...
**日期**: 2026-06-17
**当前版本**: 猫剪豆问（自然情绪版）**v1.0.1**
**目录结构规范**:
- `new/(脚本)猫剪豆问（自然情绪版）v1.0.1.json`（当前推荐插件包）
- `new/猫剪豆问（自然情绪版）v1.0.1.json`（当前推荐引擎）
- `new/(脚本)猫剪豆问（自然情绪版）v1.0_修复括号识别备份.json`（v1.0 备份）
- `new/猫剪豆问（自然情绪版）v1.0_修复括号识别备份.json`（v1.0 备份）
- `new/(脚本)猫剪豆问（自然情绪版）v1.3.2.json` 等 v1.1 ~ v1.3.2 历史文件（保留备份）
- `js/new/(脚本)猫剪豆问（自然情绪版）v1.0.1_obj{0,1}.js`（脚本调阅）
- `js/new/猫剪豆问（自然情绪版）v1.0.1.js`（引擎调阅）

**已完成事项**:
1. 用户反馈 v1.1 ~ v1.3.2 均出现 OOM，已回退到稳定版 v1.0
2. 用户反馈括号 1234 不能识别，排查定位根因：脚本未将特殊括号转换为 `<<括号X>>` 标记
3. 备份 v1.0 脚本和引擎文件
4. 实现脚本侧 v1.0.1：
   - 新增 `preprocessSpecialBrackets(text)` 函数
   - `【内容】` → `<<括号1>>内容`
   - `「内容」` → `<<括号3>>内容`
   - `『内容』` → `<<括号4>>内容`
   - 排除纯数字序号如 `【1】`
5. 实现引擎侧 v1.0.1：同步版本号
6. 同步生成 `js/new/` 调阅文件并运行 `node --check` 语法验证
7. 更新 TODO.md 变更记录和会话摘要
8. 推送远程仓库
