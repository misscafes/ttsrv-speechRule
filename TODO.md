...
**日期**: 2026-06-17
**当前版本**: 猫剪豆问（自然情绪版）**v1.3.2**
**目录结构规范**:
- `new/(脚本)猫剪豆问（自然情绪版）v1.3.2.json`（最新插件包）
- `new/猫剪豆问（自然情绪版）v1.3.2.json`（最新引擎）
- `new/(脚本)猫剪豆问（自然情绪版）v1.3.1_OOM三次修复备份.json`（v1.3.1 备份）
- `new/猫剪豆问（自然情绪版）v1.3.1_OOM三次修复备份.json`（v1.3.1 备份）
- `js/new/(脚本)猫剪豆问（自然情绪版）v1.3.2_obj{0,1}.js`（脚本调阅）
- `js/new/猫剪豆问（自然情绪版）v1.3.2.js`（引擎调阅）

**已完成事项**:
1. 用户再次报告 OOM，堆栈仍指向阅读 App JSON 序列化
2. 分析根因：长段落中大量对话的情绪标记叠加，导致返回文本过大
3. 设计 v1.3.2 方案：情绪桥接长段落降频处理
4. 备份 v1.3.1 脚本和引擎文件
5. 实现脚本侧 v1.3.2：
   - 新增 `EMOTION_BRIDGE_MAX_DIALOGS_PER_PARAGRAPH = 20`
   - 新增 `EMOTION_BRIDGE_LONG_PARAGRAPH_THRESHOLD = 1500`
   - 长段落（>1500 字且 >20 句）只对前 20 句加情绪标记
6. 实现引擎侧 v1.3.2：同步版本号
7. 同步生成 `js/new/` 调阅文件并运行 `node --check` 语法验证
...
- v1.3.2 脚本需与 v1.3.2 引擎配套使用
- 短段落情绪桥接完全不受影响
- 超长段落只给前 20 句加情绪标记，稳定性优先
- 若仍 OOM，可继续降低 `EMOTION_BRIDGE_LONG_PARAGRAPH_THRESHOLD` 或 `EMOTION_BRIDGE_MAX_DIALOGS_PER_PARAGRAPH`
- 表演链路仍默认关闭
