...
- **问题根因**：JSON 的 `code` 字段中，JavaScript 字符串内的 `\n` 被错误地只写了一个反斜杠（JSON 转义 `\n` → 实际换行符 0x0A），导致 Rhino 解析时遇到跨行字符串字面量，报 `EvaluatorException: 字符串文字没有限制`
- **修复内容**：使用状态机遍历 JavaScript 代码，精确识别字符串字面量（`'"'` / `"'` / `` ` ``），将其中所有实际换行符（0x0A）替换为 `\n` 转义序列，共修复约 2039 处跨行字符串
- **影响范围**：主要涉及日志拼接、提示词模板、API 返回处理等含换行符的字符串
- 保留 v2.100 全部特性（按书名隔离上下文、别名合并、data.json 智能备份、零 HTTP 依赖）
- 文件名：`多角色朗读2.101【修复JSON换行符转义】.json`
...
- 文件名：`多角色朗读2.100【按书名隔离上下文+别名合并发音人轮询+增强别名校验版v77+备用模型接力】.json`
...
- **本次工作**：
  ...
  - **修复 v2.100 JSON 换行符转义错误**（v2.101）：Rhino 报 `字符串文字没有限制`，原因是 `code` 字段中大量 `\n` 只写了一个反斜杠，JSON 解析后变成实际换行符，导致 JavaScript 字符串字面量跨行
- **主目录结构**：
  - `dist/多角色朗读2.94.json` — 当前主版本构建产物
  - `多角色朗读2.101【修复JSON换行符转义】.json` — 本次新建（修复 v2.100 导入/运行报错）
  - `js/` — 各版本提取的调阅文件
  - `历史版本/` — 归档的旧版本 json
  - `调试记录/` — 调试/分析文件（含 `bug.txt` — 本次报错堆栈）
  - `工具脚本/` — extract-js.js
- **注意事项**：
  - v2.101 基于 v2.100，仅修复 JSON 换行符转义问题，功能与 v2.100 完全一致
  - 修复范围：使用状态机遍历 `code` 字段，精确修复字符串字面量内的跨行换行符（共约 2039 处）
  - 用户导入 v2.100 时 Rhino 报错 `字符串文字没有限制 (<Unknown>#2786)`，根因是 `updateContext` 等函数中 `"\n"` 被 JSON 解析为实际换行符
  - 建议用户直接使用 v2.101，丢弃 v2.100
...
- [ ] 如需功能迭代，在 v2.94 / v2.101 / v2.86 基础上增量开发
