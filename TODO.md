...
### 2026-05-30 本次会话（新增 v2.106，正式人名做主名 + 称号过滤 + 修复别名丢失 + 插件同步问题）
- **当前最新版本**：v2.106（朗读规则）/ v6.70（角色管理插件）/ v2.94（主版本）
- **本次工作**：
...
- **主目录结构**：
  - `多角色朗读2.106【正式人名做主名+称号过滤+修复别名丢失+同步shuming+别名合并发音人轮询+增强别名校验版v77+备用模型接力】.json` — 本次新建（最新可用版本）
  - `ttsrv-plugin-角色管理6.70.json` — 插件修复版本
  - `js/` — 各版本提取的调阅文件
- **注意事项**：
  - v2.106 主名策略：AI 可更换主名，但如果建议的主名是称号/身份（如张掌门、大皇子、黑衣人），自动回退到原主名
  - 正式人名（如张三）可以替换昵称（如小张）成为主名
  - 称呼、称号、身份、昵称等只能作为别名
