...
- **当前最新版本**：v2.94（主版本）/ v2.86（2.85+备用模型分支）
- **本次工作**：将 2.94 的备用模型接力模块完整注入到 2.85 中，生成 v2.86
- **主目录结构**：
  - `dist/多角色朗读2.94.json` — 当前主版本构建产物
  - `多角色朗读2.86发音人轮询+增强别名校验v77+备用模型接力.json` — 本次新建
  - `js/` — 各版本提取的调阅文件（含 2.86 新增）
  - `历史版本/` — 归档的旧版本 json
- **目录整理**：
  - 旧模型 `2.85...json` → `历史版本/`
  - 调试/分析文件 → `调试记录/`
  - `extract-js.js` → `工具脚本/`（支持从上级目录读取json）
  - 主目录仅保留当前最新版 json + 项目文档
- **注意事项**：
  - v2.86 是基于 2.85 的备用模型增强版，保留了 2.85 的全部原有功能
  - 备用模型通过 `miyue.txt` 中 `#!#` 分隔符配置，向后完全兼容

## 当前任务
- [x] v2.94 稳定版保持为主版本
- [x] 在 2.85 基础上注入备用模型接力模块，生成 v2.86
- [ ] 如需功能迭代，在 v2.94 或 v2.86 基础上增量开发
