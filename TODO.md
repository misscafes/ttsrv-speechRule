...
**日期**: 2026-06-18
**当前版本**: 猫剪豆问（自然情绪版）**v1.0.2**
**目录结构规范**:
- `new/(脚本)猫剪豆问（自然情绪版）v1.0.2.json`（当前推荐插件包）
- `new/猫剪豆问（自然情绪版）v1.0.2.json`（当前推荐引擎）
- `new/(脚本)猫剪豆问（自然情绪版）v1.0.1_修复括号读出备份.json`（v1.0.1 备份）
- `new/猫剪豆问（自然情绪版）v1.0.1_修复括号读出备份.json`（v1.0.1 备份）
- `new/(脚本)猫剪豆问（自然情绪版）v1.3.2.json` 等 v1.1 ~ v1.3.2 历史文件（保留备份）
- `js/new/(脚本)猫剪豆问（自然情绪版）v1.0.2_obj{0,1}.js`（脚本调阅）
- `js/new/猫剪豆问（自然情绪版）v1.0.2.js`（引擎调阅）

**已完成事项**:
1. 用户反馈 v1.0.1 会读出「括号3」等语音标记
2. 排查定位根因：v1.0.1 脚本将特殊括号转换为 `<<括号X>>内容`（无引号），引擎只会在对话文本中消费 `<<tag>>`，旁白/普通文本中的标记被当成普通文本读出
3. 备份 v1.0.1 脚本和引擎文件
4. 实现脚本侧 v1.0.2：移除 `preprocessSpecialBrackets` 预处理，避免生成无引号包裹的 `<<tag>>`
5. 实现引擎侧 v1.0.2：
   - 移除会提前剥离所有括号的旧正则
   - 修正括号到发音人标签映射：`【】`→括号1、`「」`→括号3、`『』`→括号4
   - 统一用引号包裹成对话，确保 `<<tag>>` 能被正确消费
6. 同步生成 `js/new/` 调阅文件
7. 更新 TODO.md 变更记录和会话摘要
8. 推送远程仓库
9. 更新逻辑说明文档 `猫剪豆问-v1.10-逻辑说明.md` 为 v1.0.2 自然情绪版内容，并备份原 v1.10 版本

**注意事项**:
- v1.0.2 脚本需与 v1.0.2 引擎配套使用
- 该修复基于 v1.0.1，未引入 v1.1+ 的表演链路，避免 OOM
- 括号 2（在线音效）仍保持原有逻辑
- 音频截断问题（`emotion=sad` + 逗号处截断）尚未修复，待进一步验证
