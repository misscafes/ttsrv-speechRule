...
**日期**: 2026-06-17
**当前版本**: 猫剪豆问（自然情绪版）**v1.1**
**目录结构规范**:
- `new/(脚本)猫剪豆问（自然情绪版）v1.1.json`（最新插件包）
- `new/猫剪豆问（自然情绪版）v1.1.json`（最新引擎）
- `new/(脚本)猫剪豆问（自然情绪版）v1.0_表演链路增强备份.json`（v1.0 备份）
- `new/猫剪豆问（自然情绪版）v1.0_表演链路增强备份.json`（v1.0 备份）
- `js/new/(脚本)猫剪豆问（自然情绪版）v1.1_obj{0,1}.js`（脚本调阅）
- `js/new/猫剪豆问（自然情绪版）v1.1.js`（引擎调阅）

**已完成事项**:
1. 分析 v1.0 表演链路效果不明显的原因：覆盖范围窄、指令模板化、缺少场景温度主导
2. 设计 v1.1 增强方案：场景温度主导 + 全句表演指令覆盖 + 关键句深推 + 旁白表演化
3. 备份 v1.0 脚本和引擎文件
4. 实现脚本侧表演链路增强：
   - 扩展场景温度判断（旁白参与，新增悬疑/欢快）
   - 新增文本细节推断（重音、停顿、气口、句式）
   - 重写 `buildPerformancePrompt`，所有对话都带表演指令
   - 新增 `buildNarrationPerformancePrompt`，旁白带专用指令
   - 对话/旁白处理逻辑改为全句覆盖
5. 实现引擎侧增强：表演指令 `unshift` 到 `context_texts` 最前、保留非 `NEEDS_CONTEXT_TEXTS` voice 的表演指令、增强日志
...
**注意事项**:
- v1.1 脚本需与 v1.1 引擎配套使用
- `PERFORMANCE_INTENSITY` 默认 2（明显），可在脚本 obj0.js 顶部调整为 1（温和）或 3（强烈）
- 当前表演指令仍通过 `context_texts` 传递，若 v1.1 仍不明显，下一版可考虑直接把指令注入待合成文本前缀
