...
**当前版本**: 多角色朗读 **v2.125**
**目录结构规范**:
- `多角色朗读2.125【参考2.87加速版优化+ES5兼容+别名缓存加速】.json`（当前推荐主规则）
...
**已完成事项**:
1. 用户要求参考 `多角色朗读2.87【加速版+1】.json` 优化主脚本 v2.124
2. 对比分析 2.87 与 2.124 的差异，识别可吸收的加速优化点
3. 复制 v2.124 为 v2.125 并同步更新 name / version / code 内部版本注释
4. 调整配置参数向 2.87 加速版靠拢：WAIT_API_RESULT_COUNT、超时、xiawen/shouci、SEQ_ADD_RATIO、NEXT_CHAPTER_COUNT
5. 重写 `getAllCharacterNamesAndAliases` 为 ES5 兼容实现，移除 Set / 箭头函数 / filter
6. 新增别名 API 列表缓存机制，减少角色记录重复遍历
7. 优化 `processCharacter` 别名合并后的内存映射刷新逻辑，避免冗余重建
8. 移植 2.87 `voteBatchAliasAnalyzeResult` 批量别名投票函数
9. 运行 `extract-js.js` 同步 `js/` 调阅文件
10. 更新 `TODO.md` 变更记录和会话摘要
11. 推送远程仓库（GitHub + cnb.cool）

**注意事项**:
- v2.125 完全保留 v2.124 的所有功能模块
- `voteBatchAliasAnalyzeResult` 已移植但未完全接入 `processCharacter` 主流程，属预埋接口
- 后续可继续吸收 2.87 的 sharedState 跨轮次累积、章节缓存覆盖写入与全文定位回退等深层优化
