### ttsrv-replaces5.json 去 18+ 版本（2026-07-23）
- **目标文件**: `new/新脚本/ttsrv-replaces5.json`
- **源文件**: `new/新脚本/ttsrv-replaces4.json`
- **备份**: 原 `ttsrv-replaces4.json` 保留未动，新增 `ttsrv-replaces5.json` 作为去 18+ 版本
- **问题描述**: 用户需要一份不包含 18+ 内容的替换规则文件
- **根因**: `ttsrv-replaces4.json` 中包含两个 18+ 规则分组：`18+音效后置`（17 条规则）和 `18+音效前置`（11 条规则）
- **改动**:
  - 复制 `ttsrv-replaces4.json` 为 `ttsrv-replaces5.json`；
  - 删除 `18+音效后置` 和 `18+音效前置` 两个分组；
  - 保留其余 8 个分组：`音效前置2`、`置顶音效🎯🐾`、`1-音效 后置`、`去掉前面多余`、`2-音效 前置`、`3-直接替换（无$0）`、`至底恢复双引号`、`最底拟声词`，共 875 条规则。
- **功能保持**: 保留所有非 18+ 的音效、转场、拟声词、双引号恢复等规则。
- **验证**:
  - 使用 Node.js 统计确认 18+ 分组已移除，剩余分组及规则数量正确；
  - 使用 Python `json.load` 验证 JSON 可解析；
  - 运行 `node extract-js.js` 同步生成 `js/new/新脚本/ttsrv-replaces5.js` 调阅文件。

### 猫剪豆问 v1.28 修复章节标题“第1章”误读为“第十一章”（2026-07-23）
- **目标文件**: `new/(脚本)猫剪豆问（自然情绪版）v1.28.json`、`new/猫剪豆问（自然情绪版）v1.28.json`
- **源文件**: `new/(脚本)猫剪豆问（自然情绪版）v1.27.json`、`new/猫剪豆问（自然情绪版）v1.27.json`
- **备份**: 按版本递增规范，基于 v1.27 生成 v1.28 新文件，原 v1.27 文件保留
- **问题描述**: 用户反馈章节语音合成时，“第1章”会被读成“第十一章”；阿拉伯数字在章节标题中被 TTS 引擎错误识别
- **根因**: `preprocessChapterTitle()` 识别到章节标题后，直接将原文（如“第1章”）包装给括号3发音人朗读，未对阿拉伯数字做预处理；猫箱-VV 引擎对单独出现的“1”识别异常，输出为“十一”
- **改动**:
  - 新增 `convertArabicNumberToChinese(text)` 函数，将文本中的阿拉伯数字转换为中文数字，支持 0-9999；
  - 在 `preprocessChapterTitle()` 识别到章节标题后，先调用 `convertArabicNumberToChinese()` 转换数字，再包装为 `“<<括号3>>...”` 进行朗读；
  - 同时更新朗读脚本对象 0 与独立引擎的 `preprocessChapterTitle`，保证行为一致；
  - 修正独立引擎注释版本号历史遗留问题（原注释仍为 v1.22），统一为 v1.28；
  - 统一文件名、JSON 顶层 `name`/`version` 与 code 内部版本号全部为 v1.28。
- **功能保持**: 保留 v1.27 的换书修复、CNB 混元3 API、进度指针、章节缓存、无引号多行匹配、异常引号修复、加权随机发音人分配、跨书 `voice_usage_count.json`、切书防御与角色合并备份等能力。
- **验证**:
  - 使用 Node.js 单元测试验证 `convertArabicNumberToChinese()` 对“第1章/第11章/第123章/第1005章/卷2”等输入转换正确；
  - 使用 `node --check` 验证 v1.28 脚本对象 0、对象 1 与独立引擎 JS 无语法错误；
  - 使用 Python `json.load` 验证 JSON 可解析；
  - 运行 `node extract-js.js` 同步生成 `js/new/...v1.28...` 调阅文件。
...
- **备份**: 按版本递增规范，基于 2.134 生成 2.135 新文件，原 2.134 文件保留
- **问题描述**: 
  1. 旁白前的对话会分配给旁白读，而不是角色。例如：`“我下班啦。”孔楠拿着包起身...` 中 `“我下班啦。”` 被旁白读出。
  2. 连续对话也不分配发音人，由旁白读出。例如连续多段 `“...”` 对话中部分段落被旁白处理。
- **根因一**: `handleText` 中半角双引号转中文双引号的处理位于短对话去引号正则之后，导致 `"我下班啦"` 这类半角双引号短对话先被去掉引号，无法被 `fx` 识别为对话，最终误标为旁白。
- **修复一**: 将半角双引号统一转换逻辑提前到短对话去引号正则之前，确保所有 `"..."` 先变成 `“...”` 再进入后续处理。
- **根因二**: `handleText` 中“在线音效双引号内匹配”的普通关键词替换会在匹配项前后插入 `\n`，把 `“我下班啦。”` 拆成多行；`fx` 的分割正则 `.` 不匹配换行，导致整段对话无法被识别，拆散后的片段被当作旁白。
- **修复二**: 在双引号内跳过普通关键词替换，双引号外的独立场景音效匹配仍正常执行，避免破坏对话结构。
- **验证**: 
  - 使用 `node --check` 验证 2.135 JS 文件无语法错误
  - 使用 Python `json.load` 验证 JSON 可解析
  - 运行 `node extract-js.js` 同步生成 `js/多角色朗读2.135...js` 调阅文件
  - 用用户示例与半角双引号示例做分割测试，确认对话不再被误标为旁白

### 猫剪豆问 v1.25 统一脚本、替换引擎与独立引擎版本号（2026-07-09）
- **目标文件**: `new/(脚本)猫剪豆问（自然情绪版）v1.25.json`、`new/猫剪豆问（自然情绪版）v1.25.json`
- **源文件**: `new/(脚本)猫剪豆问（自然情绪版）v1.24.json`、`new/猫剪豆问（自然情绪版）v1.22.json`
- **备份**: 按版本递增规范，基于 v1.24 生成 v1.25 新文件，原 v1.24 文件保留；独立引擎由 v1.22 升级版本号生成
- **背景**: 用户反馈猫剪豆问各组件版本号必须保持统一，包括替换引擎与独立引擎；v1.24 将对象 1 版本号恢复为 v1.13 导致脚本 v1.24 与引擎 v1.13 不一致
- **改动**:
  - 基于 v1.24 脚本生成 v1.25，对象 0（朗读脚本）升级到 v1.25；
  - 将 JSON 数组中对象 1“有效（替换对象）引擎+”的 `name`/`version` 统一改为 v1.25，不再保持 v1.13；
  - 同步升级独立引擎文件 `new/猫剪豆问（自然情绪版）v1.25.json`，由 v1.22 升级版本号而来；
  - 统一文件名、JSON 顶层 `name`/`version` 与 code 内部版本号全部为 v1.25。
- **功能保持**: v1.25 的朗读脚本与引擎功能与 v1.23/v1.24 完全一致，包括加权随机发音人分配、跨书 `voice_usage_count.json` 使用次数记录、手动固定加成等。
- **验证**: `node --check` 验证 v1.25 脚本、替换引擎、独立引擎 JS 无语法错误；Python `json.load` 验证 JSON 可解析；`extract-js.js` 成功生成 `js/new/...v1.25...` 调阅文件。

### 猫剪豆问 v1.24 修正替换引擎版本号标注错误（2026-07-09）
...
### 猫剪豆问 v1.23 优化发音人分配：加权随机 + 跨书使用次数（2026-07-09）
...
### 多角色朗读 2.134 修复半角双引号对话识别不稳定（2026-07-09）
...
- **参考文件跳段修复核心逻辑分析**:
  1. **文本预处理**: 清理 【】、『』 等特殊括号，将拟声词从引号中剥离，避免服务端对非常规文本拒绝合成
  2. **双重追加句号补救**: 主重试循环 3 次内每次失败都在当前文本末尾追加句号；主循环全部失败后，再用"原始文本 + 句号"额外补救 3 次
  3. **独立随机设备标识**: 每次 ws.maoxiang 调用使用新的 aid/device_id，避免连接/限流冲突
  4. **不对音频做最小字节数校验**: 只要返回非空即视为成功，减少"误判跳段"
- **v1.22 引擎优化内容（在保留 v1.21 全部能力基础上）**:
  1. 改进重试标点策略：首次重试清理末尾不规范标点后补一个句号，第 2 次及以后递增追加句号（如 "。" → "。。" → "。。。"），融合参考文件的"多次追加句号"优势
  2. 新增"强化标点补救"阶段：7 次常规重试全部失败后、最终 15 秒长等待抢救前，先用"原始文本 + 两个句号"快速尝试一次
  3. 保留 v1.21 的跨段对话继承、长文本 800 字符切分、最小音频字节数检查、协议异常特殊等待、最终抢救重试、纯标点段过滤等机制
  4. 同步更新 JSON 顶层 `name` / `version` 与 code 头部注释为 v1.22
  5. 脚本文件同步升级到 v1.22，保持脚本+引擎版本配套
- **影响**:
  - 因缺标点、末尾标点不规范导致服务端截断/返回空音频的概率进一步降低
  - 对 502/504/101/连接异常等可重试错误的兼容行为不变
  - 脚本+引擎需配套使用 v1.22
- **当前文件**:
  - `new/(脚本)猫剪豆问（自然情绪版）v1.22.json`
  - `new/猫剪豆问（自然情绪版）v1.22.json`
- **JS 调阅文件同步**:
  - `js/new/(脚本)猫剪豆问（自然情绪版）v1.22_obj0.js`
  - `js/new/(脚本)猫剪豆问（自然情绪版）v1.22_obj1.js`
  - `js/new/猫剪豆问（自然情绪版）v1.22.js`
- **验证**:
  - `node --check` 验证 v1.22 引擎与脚本 JS 无语法错误
  - `node extract-js.js` 重新生成对应 JS 调阅文件

# 待办事项（TODO）

### 猫剪豆问 v1.20 回退到 v1.19（2026-07-03）
- **操作**: 因 v1.20 引擎存在问题，按用户要求直接回退到现有 v1.19 版本
- **处理方式**: 将 `new/猫剪豆问（自然情绪版）v1.20.json` 重命名为 `new/猫剪豆问（自然情绪版）v1.20_回退备份.json`，保留 v1.19 脚本+引擎作为当前可用版本
- **当前可用文件**:
  - `new/(脚本)猫剪豆问（自然情绪版）v1.19.json`
  - `new/猫剪豆问（自然情绪版）v1.19.json`
- **注意事项**: v1.20 备份文件不再使用，如需恢复其内容（千问官方扩展等）可手动改名回 `.json`

### 猫剪豆问 v1.19 「」括号3改由旁白发音、不分配独立发音人（2026-07-03）
- **版本升级**: v1.18 → v1.19（脚本 + 引擎）
- **需求背景**: 用户要求「」括号3 内容改由旁白发音、不分配括号3独立发音人；章节标题仍保留括号3独立发音人
- **改动内容**:
  1. 按项目规范基于 v1.18 创建 v1.19 JSON 源文件（脚本 + 引擎），v1.18 原文件保留
  2. 脚本 `preprocessSpecialBrackets` 与引擎预处理：`「内容」` 不再转换为 `"内容"`，改为 `text.replace(/「([^「」]*?)」/g, '$1')`（脚本）/ `text.replace(/「([^「」]+?)」/g, '$1')`（引擎），直接去除「」括号，内容作为普通旁白文本，自然走 `narratorCfg` 旁白发音，不再分配括号3独立发音人
  3. 章节标题 `preprocessChapterTitle` 保持不变：仍返回 `'"<<括号3>>' + 标题 + '"'`，继续走括号3独立发音人
  4. 同步更新 JSON 顶层 `name` / `version` 与 `code` 内部版本注释为 v1.19
  5. 运行 `node extract-js.js` 重新生成 `js/new/...v1.19...` 调阅文件
  6. 使用 `node --check` 验证 JS 无语法错误，使用 Node `JSON.parse` 验证 JSON 可解析
  7. 校验 `<<括号3>>` 在脚本/引擎 code 中均仅剩章节标题 1 处，确认「」不再产生括号3对话
- **影响**:
  - 「」包裹的内容（如内心独白、强调、注释）改用旁白音色朗读，不再使用括号3独立发音人
  - 章节标题朗读行为不变，仍用括号3独立发音人
  - 「」内容去除括号后并入前后旁白区间一起朗读（若需独立成段可后续调整）
- **当前文件**:
  - `new/(脚本)猫剪豆问（自然情绪版）v1.19.json`
  - `new/猫剪豆问（自然情绪版）v1.19.json`
- **JS 调阅文件同步**:
  - `js/new/(脚本)猫剪豆问（自然情绪版）v1.19_obj{0,1}.js`
  - `js/new/猫剪豆问（自然情绪版）v1.19.js`
- **注意事项**:
  - v1.19 脚本需与 v1.19 引擎配套使用
  - 若希望章节标题也改用旁白，可后续将 `preprocessChapterTitle` 的 `<<括号3>>` 一并改掉
...
- **问题背景**: 用户安装 2.88 后朗读出现 `EvaluatorException: 无音频数据 (Close:1000)`，日志显示系统提示音效后紧跟“薇薇旁白”。判断为默认开启的表演指令 `[[emo:情绪|表演指令]]` 与当前使用的 TTS 引擎不兼容，且旁白默认兜底强制加 `neutral` 也可能导致部分 voice 返回空音频。
- **改动内容**:
  1. 按项目规范备份 2.88 文件：`参考/多角色朗读2.88【加速版+1修复2+情绪模块】_表演指令不兼容备份.json`
  2. 将 `ENABLE_PERFORMANCE_PROMPT` 默认值从 `1` 改为 `0`，避免旧引擎无法解析带 `|` 的表演指令
  3. 将 `DEFAULT_NARRATION_EMOTION` 默认值从 `"平静"` 改为 `""`，不再强制给每个旁白加 `[[emo:neutral]]`
  4. 统一 4 处版本号/名称：文件名、JSON 顶层 `name`/`version`、`SpeechRuleJS.name`/`SpeechRuleJS.version` 全部为 2.89/64
  5. 运行 `node extract-js.js` 同步生成 `js/参考/多角色朗读2.89【加速版+1修复2+情绪模块修复表演指令不兼容】_obj0.js`
  6. 使用 `node --check` 验证 JS 无语法错误，使用 Python `json.load` 验证 JSON 可解析
- **影响**:
  - 2.89 保留 2.88 的情绪桥接、本地情绪修正、跨段情绪继承、段落主情绪稳定等能力
  - 默认不再输出带表演指令的 `[[emo:情绪|...]]` 双段格式，降低旧引擎不兼容概率
  - 旁白不再默认强制加 `neutral`，只有当本地情绪推断或继承命中时才加情绪前缀
- **当前文件**:
  - `参考/多角色朗读2.89【加速版+1修复2+情绪模块修复表演指令不兼容】.json`
- **JS 调阅文件同步**:
  - `js/参考/多角色朗读2.89【加速版+1修复2+情绪模块修复表演指令不兼容】_obj0.js`
- **注意事项**:
  - 如需使用表演指令，可手动将 `ENABLE_PERFORMANCE_PROMPT` 改回 `1`，但需确认当前 TTS 引擎支持 `[[emo:情绪|表演指令]]` 双段格式
  - 如需恢复旁白默认情绪兜底，可手动将 `DEFAULT_NARRATION_EMOTION` 改回 `"平静"`
...
- **根因分析**:
  1. 重写 2.130 切书段时，把 `if (dataJsonContent && ...)` 提前关闭，但保留了原来关闭该 if 的 `}`
  2. 造成后续两个 `}` 中有一个没有对应的 `{`，Rhino 解析时认为外层 try 缺少 catch/finally
- **改动内容**:
  1. 按项目规范备份 2.130 并创建 2.131 JSON 源文件：`多角色朗读2.131【修复切书语法错误+修复切书回默认书籍】.json`
  2. 删除多余大括号，恢复正确的 try-catch 嵌套结构
  3. 同步更新文件名、JSON 顶层 `name`/`version`、code 头部注释、`SpeechRuleJS.name`/`SpeechRuleJS.version` 为 2.131
  4. 运行 `node extract-js.js` 重新生成对应 JS 调阅文件
  5. 使用 `node --check` 验证 2.131 JS 文件无语法错误
- **影响**:
  - 2.130 存在语法错误，请勿使用；请使用 2.131
  - 保留 2.130 的切书逻辑修复：优先使用真实书名、拒绝切回默认/未知书
- **当前文件**:
  - `多角色朗读2.131【修复切书语法错误+修复切书回默认书籍】.json`
- **JS 调阅文件同步**:
  - `js/多角色朗读2.131【修复切书语法错误+修复切书回默认书籍】.js`
- **注意事项**:
  - 2.130 为存在语法错误的版本，仅作为历史备份保留，实际使用请选择 2.131

### 多角色朗读 2.130：修复切书回默认书籍 + 统一内外版本号/名称 + 添加自然情绪（2026-06-25）
- **版本升级**: 2.129 → 2.130
- **重要纠正**: 之前判断「多角色朗读规则没有切书逻辑」是错误的。`SpeechRuleJS` 主流程中存在「书籍切换与角色备份」段，同样读取 `data.json` 的 `bookName` 并与 `cunfang.txt` 比较，存在与猫剪豆问类似的回默认/未知书籍问题
- **问题背景**: 切换新书后，`data.json` 若未正确写入新书 `bookName`，`newBookName` 会变成 `"未知书名"` 或 `"default_book"`，随后覆盖 `cunfang.txt`，导致下次朗读切回默认/未知书籍
- **改动内容**:
  1. 按项目规范备份 2.129 并创建 2.130 JSON 源文件：`多角色朗读2.130【修复切书逻辑+统一内外版本号+添加自然情绪】.json`
  2. 统一 4 处版本号/名称：文件名、JSON 顶层 `name`/`version`、code 头部注释、`SpeechRuleJS.name`/`SpeechRuleJS.version`
  3. 重写「书籍切换与角色备份」段：
     - 同时读取 `data.json` 和 `cunfang.txt`，优先使用非 `"未知书名"` / `"default_book"` 的真实书名
     - 若 `data.json` 书名为空/默认/未知，但 `cunfang.txt` 有真实书名，则拒绝切回，并记录 `[多角色换书防御]` 日志
     - 仅当明确检测到新的真实书名时才执行换书流程
     - 换书时同步更新 `cunfang.txt`、`gengxin.json`、`liebiao.json`
  4. 修复 `bookData` 为空时第二步代码访问 `bookData.bookUrl` 的潜在异常
  5. 运行 `node extract-js.js` 重新生成对应 JS 调阅文件
- **影响**:
  - 多角色朗读规则不会再因 `data.json` 缺失 `bookName` 而把 `cunfang.txt` 覆盖为 `"未知书名"` 或 `"default_book"`
  - 与猫剪豆问 v1.17 在书名保存行为上保持一致
- **当前文件**:
  - `多角色朗读2.130【修复切书逻辑+统一内外版本号+添加自然情绪】.json`
- **JS 调阅文件同步**:
  - `js/多角色朗读2.130【修复切书逻辑+统一内外版本号+添加自然情绪】.js`
- **注意事项**:
  - 2.130 历史注释中保留的 `v2.124` / `v2124_*` 等标记是功能修改标记，不是需要统一的版本号
...
3. 命无言 APK 内置日志查看器方案 A（第三版，修复只有 FA 日志）：
   - 用户测试第二版：对话框能弹出，但只显示 Firebase 等系统日志，没有 TTS/JS 日志。
   - 排查发现 TTS 朗读时 `java.log()` 调用 `Lim/t.e()`，而 `Lim/t.e()` 只有在书源调试窗口设置了 `Lim/t;->b` 回调时才会继续输出到 Logcat；日常朗读 `Lim/t;->b` 为 null，所以 TTS 日志不会进入 `tc/l0.o0()`。
   - 把缓存逻辑移到 `Lim/t.e()` 方法开头（`monitor-enter p0` 之后），无条件缓存所有 JS 日志。
   - 回编译并签名生成 `新反编译/命无言/i·阅读 尝鲜版[3.26.062019]_log_fullfix_已签名.apk.1`。
4. 更新 `MEMORY.md`：
   - 添加工具/程序查找顺序约定（项目根目录 → C 盘 → 下载）。
   - 更新命无言 APK 内置日志查看器状态为「方案 A 第三版已构建，待测试」。
5. 更新 `TODO.md` 变更记录和会话摘要。
...
│   ├── i·阅读 尝鲜版[3.26.062019]_log_fix_已签名.apk.1       <- 第二版（只有系统日志）
│   └── i·阅读 尝鲜版[3.26.062019]_log_fullfix_已签名.apk.1   <- 第三版（同时缓存 JS 日志）
├── js/new/                                        <- JS 调阅文件
├── yinpin/
│   └── ttsrv-replaces4.json                       <- 项目仓库音效规则镜像
├── MEMORY.md
...
**当前版本**:
- 猫剪豆问脚本：`new/(脚本)猫剪豆问（自然情绪版）v1.19.json`
- 猫剪豆问引擎：`new/猫剪豆问（自然情绪版）v1.19.json`
- 多角色朗读规则：`多角色朗读2.131【修复切书语法错误+修复切书回默认书籍】.json`

**本次完成事项**:
1. 按用户要求将存在问题的猫剪豆问 v1.20 引擎回退到 v1.19：
   - 将 `new/猫剪豆问（自然情绪版）v1.20.json` 重命名为 `new/猫剪豆问（自然情绪版）v1.20_回退备份.json`。
   - 当前可用版本恢复为 v1.19，脚本与引擎保持配套。
2. 更新 `PROJECT_STATUS.md`：在 v1.19 最新改动中记录 v1.20 回退说明，并在会话摘要和注意事项中补充提示。
3. 更新 `TODO.md`：新增 v1.20 回退条目，并写入本次会话摘要。
4. 执行 `git add . && git commit -m "..." && git push origin master`，已同步推送到 GitHub 与 cnb.cool。

**主目录结构（相关）**:
```
/workspace/
├── new/
│   ├── (脚本)猫剪豆问（自然情绪版）v1.19.json   <- 当前最新脚本
│   ├── 猫剪豆问（自然情绪版）v1.19.json         <- 当前可用引擎
│   ├── 猫剪豆问（自然情绪版）v1.20_回退备份.json <- v1.20 废弃备份
│   └── （v1.11~v1.18 历史版本保留）
├── js/new/                                        <- JS 调阅文件
├── MEMORY.md / PROJECT_STATUS.md / TODO.md
└── extract-js.js
...
**本次完成事项**：
1. 诊断用户反馈的“章节语音合成时，第1章被读成第十一章”问题。
2. 确认根因：猫剪豆问 `preprocessChapterTitle()` 将章节标题原样送入 TTS，猫箱-VV 引擎对阿拉伯数字“1”识别异常。
3. 新增 `convertArabicNumberToChinese()` 函数，将章节标题中的阿拉伯数字转为中文数字（0-9999）。
4. 同时更新朗读脚本对象 0 与独立引擎的 `preprocessChapterTitle()`，转换后再包装为括号3发音人朗读。
5. 修正独立引擎历史遗留的注释版本号问题（原仍为 v1.22），统一为 v1.28。
6. 按版本递增规范基于 v1.27 创建 v1.28 新文件，保留原 v1.27 文件作为备份。
7. 统一文件名、JSON 顶层 `name`/`version` 与 code 内部版本号为 v1.28。
8. 使用 `node --check` 验证 v1.28 脚本对象 0、对象 1 与独立引擎 JS 无语法错误。
9. 使用 Python `json.load` 验证 JSON 可解析。
10. 运行 `node extract-js.js` 同步生成 `js/new/...v1.28...` 调阅文件。
11. 更新 `PROJECT_STATUS.md` 和 `TODO.md`。
12. 执行 `git add . && git commit && git push origin master`，成功推送到 GitHub 与 cnb.cool。

**主目录结构（相关）**：
```
C:/date/ttsrv-speechRule/
├── new/                                          # 猫剪豆问脚本+引擎（当前 v1.28）
│   ├── (脚本)猫剪豆问（自然情绪版）v1.28.json
│   └── 猫剪豆问（自然情绪版）v1.28.json
├── 多角色朗读2.136【融合零成本智谱API】.json      # 当前最新朗读规则
├── js/new/                                       # v1.28 JS 调阅文件
├── PROJECT_STATUS.md / TODO.md / MEMORY.md
└── extract-js.js
```

**注意事项**：
- v1.28 保留 v1.27 的换书修复、CNB 混元3 API、加权随机发音人分配等全部能力。
