...
- **注意事项**：
  ...
  - 修复范围：精确修复 v2.99 引入的 2 处跨行字符串（`updateContext` 中的 `"\n"`）
  - 用户导入 v2.100 时 Rhino 报错 `字符串文字没有限制 (<Unknown>#2786)`，根因是 `"\n"` 在 JSON 中只写了一个反斜杠，被解析为实际换行符
