...
**日期**: 2026-06-17
**当前版本**: 猫剪豆问（自然情绪版）**v1.2**
**目录结构规范**:
- `new/(脚本)猫剪豆问（自然情绪版）v1.2.json`（最新插件包）
- `new/猫剪豆问（自然情绪版）v1.2.json`（最新引擎）
- `new/(脚本)猫剪豆问（自然情绪版）v1.1_OOM修复备份.json`（v1.1 备份）
- `new/猫剪豆问（自然情绪版）v1.1_OOM修复备份.json`（v1.1 备份）
- `js/new/(脚本)猫剪豆问（自然情绪版）v1.2_obj{0,1}.js`（脚本调阅）
- `js/new/猫剪豆问（自然情绪版）v1.2.js`（引擎调阅）

**已完成事项**:
1. 接收用户 OOM 崩溃日志，定位根因：v1.1 全句表演指令覆盖导致返回文本膨胀
2. 设计 v1.2 修复方案：缩短指令 + 限制覆盖 + 段落前缀数量/长度兜底
3. 备份 v1.1 脚本和引擎文件
4. 实现脚本侧 v1.2 修复：
   - 缩短 `buildPerformancePrompt` 和 `buildNarrationPerformancePrompt` 所有输出
   - 平静句只保留情绪标记，不再强制加表演指令
   - 文本细节只给强情绪句追加 1 条
   - 新增 `_perfPrefixCount` / `_perfMaxPrefixCount` 限制前缀数量
   - 新增 `_perfMaxResultLength` 限制返回文本总长度
   - 单条指令长度封顶（对话 50 字 / 旁白 40 字）
5. 实现引擎侧 v1.2 修复：
   - 表演指令放入 `context_texts` 前截断到 120 字
   - 日志输出指令长度
...
**注意事项**:
- v1.2 脚本需与 v1.2 引擎配套使用
- 请先测试是否还出现 OOM，再评估表演效果
- 若 OOM 解决但表演效果回落，可在内存允许范围内逐步提高 `_perfMaxPrefixCount`（默认 6）
- `PERFORMANCE_INTENSITY` 在 v1.2 中只影响强情绪句的指令数量
