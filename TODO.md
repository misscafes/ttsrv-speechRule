...
**日期**: 2026-06-16
**当前版本**: 猫剪豆问引擎 **v1.8** / 猫剪豆问脚本 **v1.12**
**目录结构规范**:
- `new/猫剪豆问（优化版）v1.8.json`（引擎，最新）
- `new/(脚本)猫剪豆问（优化版）v1.12.json`（脚本，最新，含2个对象：obj0 主脚本、obj1 音效替换）
- `new/新脚本/ttsrv-replaces4.json`（音效替换规则，已修复章节转场正则）
- `js/new/猫剪豆问（优化版）v1.8.js`（引擎调阅）
- `js/new/(脚本)猫剪豆问（优化版）v1.12_obj{0,1}.js`（脚本调阅）
...
8. 备份 v1.11 脚本为 `new/(脚本)猫剪豆问（优化版）v1.11_fayinren显示全称备份.json`
9. 生成 `new/(脚本)猫剪豆问（优化版）v1.12.json`
10. 同步提取 `js/new/(脚本)猫剪豆问（优化版）v1.12_obj{0,1}.js` 调阅文件
11. 修复 `new/新脚本/ttsrv-replaces4.json` 章节转场正则：
    - 扩展规则 ① 匹配范围，支持 `回/话`、省略 `第`、无分隔符直接接标题等格式
    - 删除无效占位规则 ②（pattern 为中文说明，不是正则）
12. 备份原 `ttsrv-replaces4.json` 为 `new/新脚本/ttsrv-replaces4_备份_修复章节匹配.json`
13. 更新 TODO.md 变更记录和会话摘要
14. 推送远程仓库（GitHub + cnb.cool）

**注意事项**:
- v1.12 脚本与 v1.8 引擎配套使用
...
- `fayinren.json` 现在会显示全称（如 `女主01 灵动女主`），但仅限猫剪豆问自己使用；若同时加载 `多角色朗读2.124` 主规则，其读取 `fayinren.json` 可能异常
