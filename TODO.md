...
2. 修复多角色朗读规则 v2.128：
   ...
   - 修复 `SpeechRuleJS` 对象内部 `name` / `version` 与顶层字段不一致
3. 修复猫剪豆问脚本 v1.0.5：
...
4. 修复猫剪豆问引擎 v1.0.5：
   ...
   - 跨段孤儿对话也加入情绪标记识别与清理
5. 备份原文件并按规范升级版本号
6. 同步生成 `js/` 与 `js/new/` 下调阅文件
7. 更新 `TODO.md` 变更记录和会话摘要
8. 执行 `git add . && git commit -m "v2.128+v1.0.5：修复情绪桥接标记偶发残留被朗读" && git push origin master`，已同步到 GitHub 与 cnb.cool

**注意事项**:
- v2.128 朗读规则建议与 v1.0.5 引擎/脚本配套使用
