...
**日期**: 2026-06-17
**当前版本**: 猫剪豆问（自然情绪版）**v1.0（已回退）**
**目录结构规范**:
- `new/(脚本)猫剪豆问（自然情绪版）v1.0.json`（当前推荐插件包）
- `new/猫剪豆问（自然情绪版）v1.0.json`（当前推荐引擎）
- `new/(脚本)猫剪豆问（自然情绪版）v1.3.2.json` 等 v1.1 ~ v1.3.2 历史文件（保留备份）
- `js/new/(脚本)猫剪豆问（自然情绪版）v1.0_obj{0,1}.js`（脚本调阅）
- `js/new/猫剪豆问（自然情绪版）v1.0.js`（引擎调阅）

**已完成事项**:
1. 用户反馈 v1.1 ~ v1.3.2 均出现 OOM，决定回退到稳定版 v1.0
2. 确认 v1.0 脚本和引擎文件均存在且完整
3. 确认 v1.0 JS 调阅文件存在
4. 分析用户提供的 v1.0 日志：
   - 文本 `　　亲兵们挡在她面前，死死地架住她。` 合成成功但音频在逗号处截断
   - 参数：`emotion=sad`、`emotion_scale=4`、`voice=zh_female_vv_uranus_bigtts`
   - 初步判断为 TTS 模型对该情绪/voice/标点组合的输出异常
5. 更新 TODO.md 记录回退说明和音频截断问题分析
6. 推送远程仓库

**注意事项**:
- v1.0 为当前推荐稳定版本
- v1.1 ~ v1.3.2 作为历史备份保留，不推荐继续使用
- 音频截断问题待进一步验证：可尝试修改 emotion 为 neutral、降低 emotion_scale、或替换逗号为句号来定位根因
- 如需修复音频截断，将基于 v1.0 创建 v1.0.1
