...
- **当前文件**:
  - `new/(脚本)猫剪豆问（自然情绪版）v1.0.3.json`
  - `new/猫剪豆问（自然情绪版）v1.0.3.json`
- **JS 调阅文件同步**:
  - `js/new/(脚本)猫剪豆问（自然情绪版）v1.0.3_obj{0,1}.js`
  - `js/new/猫剪豆问（自然情绪版）v1.0.3.js`
- **注意事项**:
  - v1.0.3 脚本需与 v1.0.3 引擎配套使用
...
**日期**: 2026-06-19
**当前版本**: 猫剪豆问（自然情绪版）**v1.0.3**
**目录结构规范**:
- `new/(脚本)猫剪豆问（自然情绪版）v1.0.3.json`（当前推荐插件包）
- `new/猫剪豆问（自然情绪版）v1.0.3.json`（当前推荐引擎）
...
- `js/new/(脚本)猫剪豆问（自然情绪版）v1.0.3_obj{0,1}.js`（脚本调阅）
- `js/new/猫剪豆问（自然情绪版）v1.0.3.js`（引擎调阅）
...
4. 备份 v1.0.2 脚本和引擎文件
5. 实现脚本侧 v1.0.3：
   - 在 `getLatestCharacterRecords()` 中增加 `gengxin_book.txt` 书籍匹配检查
   - 同书时合并追加新角色，非当前书或旧版 `gengxin.json` 时直接清理，避免覆盖
   - 在 `saveBookCharacters()` 中保留 `usageCount` / `fixedVoice`，并新增 `shuming.{bookName}.json` 书籍级备份
6. 实现引擎侧 v1.0.3：同步版本号和头部注释
7. 同步生成 `js/new/` v1.0.3 调阅文件
8. 更新 TODO.md 变更记录和会话摘要
9. 推送远程仓库（GitHub + cnb.cool）

**注意事项**:
- v1.0.3 脚本需与 v1.0.3 引擎配套使用
- 该修复基于 v1.0.2，未引入 v1.1+ 的表演链路，避免 OOM
- 括号语音标记修复（v1.0.2）已保留
- 若仍有角色列表异常，可检查 `shuming.{bookName}.json` 备份是否完整
