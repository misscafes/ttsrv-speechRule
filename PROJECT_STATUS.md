...
| 组件 | 版本 | 文件 |
|------|------|------|
| 猫剪豆问（脚本+引擎） | **v1.28** | `new/(脚本)猫剪豆问（自然情绪版）v1.28.json`、`new/猫剪豆问（自然情绪版）v1.28.json` |
| 多角色朗读规则 | **v2.136** | `多角色朗读2.136【融合零成本智谱API】.json` |
| 多角色朗读规则（参考目录） | **v2.89** | `参考/多角色朗读2.89【加速版+1修复2+情绪模块修复表演指令不兼容】.json` |

---

## 最新改动（ttsrv-replaces5.json 去 18+ 版，2026-07-23）

1. 基于 `new/新脚本/ttsrv-replaces4.json` 生成 `new/新脚本/ttsrv-replaces5.json`
2. 删除两个含 18+ 内容的规则分组：`18+音效后置`（17 条）、`18+音效前置`（11 条）
3. 保留其余 8 个分组（音效前置2、置顶音效、1-音效后置、去掉前面多余、2-音效前置、3-直接替换、至底恢复双引号、最底拟声词），共 875 条规则
4. 使用 Python `json.load` 验证 JSON 可解析
5. 运行 `node extract-js.js` 同步生成 `js/new/新脚本/ttsrv-replaces5.js` 调阅文件

## 最新改动（猫剪豆问 v1.28，2026-07-23）

1. 基于 v1.27 创建 v1.28，修复用户反馈的“章节语音合成时，第1章被读成第十一章”问题
2. 根因：`preprocessChapterTitle` 将章节标题原样送入 TTS，猫箱-VV 引擎对阿拉伯数字“1”识别异常，读成“十一”
3. 修复：新增 `convertArabicNumberToChinese()` 函数，在章节标题被括号3发音人朗读前，将其中的阿拉伯数字转为中文数字（如“第1章”→“第一章”、“第11章”→“第十一章”、“第123章”→“第一百二十三章”）
4. 同时更新独立引擎与朗读脚本对象 0 的 `preprocessChapterTitle`，确保两处行为一致
5. 修正独立引擎 `猫剪豆问（自然情绪版）v1.27.json` 遗留的注释版本号（原为 v1.22），统一为 v1.28
6. 统一文件名、JSON 顶层 `name`/`version` 与 code 内部版本号为 v1.28
7. 使用 `node --check` 验证 v1.28 脚本对象 0、对象 1 与独立引擎 JS 无语法错误，Python `json.load` 验证 JSON 可解析
8. 运行 `node extract-js.js` 同步生成 `js/new/...v1.28...` 调阅文件
...
1. 基于 2.134 创建 2.135，修复用户反馈的“对话前旁白分配错误”与“连续对话不分配发音人”问题
2. 根因一：`handleText` 中半角双引号转中文双引号的处理位于短对话去引号正则之后，导致 `"我下班啦"` 这类半角双引号短对话先被去掉引号，无法被 `fx` 识别为对话，最终误标为旁白
3. 修复一：将半角双引号统一转换逻辑提前到短对话去引号正则之前，确保所有 `"..."` 先变成 `“...”` 再进入后续处理
4. 根因二：`handleText` 中“在线音效双引号内匹配”的普通关键词替换会在匹配项前后插入 `\n`，把 `“我下班啦。”` 拆成多行；`fx` 的分割正则 `.` 不匹配换行，导致整段对话无法被识别，拆散后的片段被当作旁白
5. 修复二：在双引号内跳过普通关键词替换，双引号外的独立场景音效匹配仍正常执行，避免破坏对话结构
6. 统一文件名、JSON 顶层 `name`/`version`、`SpeechRuleJS.name`/`SpeechRuleJS.version` 为 2.135
7. 使用 `node --check` 验证 2.135 JS 文件无语法错误，Python `json.load` 验证 JSON 可解析
8. 运行 `node extract-js.js` 同步生成 `js/多角色朗读2.135...js` 调阅文件

## 最新改动（猫剪豆问 v1.25，2026-07-09）

1. 基于 v1.24 创建 v1.25，解决 v1.23/v1.24 中脚本与替换引擎版本号不统一的问题
2. v1.24 把对象 1（替换引擎）版本号恢复为 v1.13，导致脚本 v1.24 与引擎 v1.13 混用，用户反馈应统一版本号
3. v1.25 将对象 1（替换引擎）的版本号同步升级为 **v1.25**，与对象 0（朗读脚本）完全一致
4. 同步升级独立引擎文件 `new/猫剪豆问（自然情绪版）v1.25.json`（由 v1.22 升级版本号而来），保持脚本+引擎版本配套
5. 功能与 v1.23/v1.24 完全一致：发音人加权随机分配、跨书 `voice_usage_count.json` 使用次数记录、手动固定加成等
6. 统一文件名、JSON 顶层 `name`/`version` 与 code 内部版本号全部为 v1.25
7. 使用 `node --check` 验证 v1.25 脚本、替换引擎、独立引擎 JS 无语法错误，使用 Python `json.load` 验证 JSON 可解析
8. 运行 `node extract-js.js` 同步生成 `js/new/...v1.25...` 调阅文件

## 最新改动（猫剪豆问 v1.24，2026-07-09）
