...
| 组件 | 版本 | 文件 |
|------|------|------|
| 猫剪豆问（脚本+引擎） | **v1.13** | `new/(脚本)猫剪豆问（自然情绪版）v1.13.json`、`new/猫剪豆问（自然情绪版）v1.13.json` |
| 多角色朗读规则 | **v2.129** | `多角色朗读2.129【基于126保留情绪修复回退127别名校验】.json` |

---

## 最新改动（v1.13，2026-06-25）
...
- [ ] 观察 v1.13 换书清空缓存后，切换章节卡住问题是否彻底解决
...
```
new/                          # 猫剪豆问脚本+引擎（当前版本 v1.13）
  (脚本)猫剪豆问（自然情绪版）v1.13.json
  猫剪豆问（自然情绪版）v1.13.json
...
- **当前版本**：猫剪豆问 v1.13 / 多角色朗读 v2.129
- **主目录结构**：`new/`（猫剪豆问）、`js/`（调阅文件）、`yinpin/`（音效规则）
- **已完成事项**：
  - 排查到此前 v1.13 修复仅更新 `js/` 调阅文件，未同步 JSON 源文件，导致 TTS Server 加载的仍是 v1.12
  - 按规范创建 v1.13 JSON 备份并同步修复（清空旧书章节缓存），运行 `node extract-js.js` 重新生成 JS
