...
| 多角色朗读规则 | **v2.132** | `多角色朗读2.132【融合v88.7完整别名审计+保留情绪缓存备用模型】.json` |
...
- [ ] 安装 2.132 后测试别名审计与识别：观察 `tts_debug_log.txt` 中 `[别名审计]` / `[图谱推理]` / `[别名确认]` / `[别名冲突]` 等日志
...
多角色朗读2.132...json        # 朗读规则（当前版本 v2.132）
...
## 会话摘要（最后更新：2026-07-08）

- **当前版本**：猫剪豆问 v1.19 / 多角色朗读 v2.132 / 多角色朗读（参考目录）v2.89
- **主目录结构**：`new/`（猫剪豆问）、`js/`（调阅文件）、`yinpin/`（音效规则）、`参考/`（参考目录）
- **已完成事项**：
  - 基于 2.131 创建 2.132，完整融合参考文件 `参考/多角色朗读2.85发音人轮询+增强别名检验v88.7.json` 的别名审计、语义证据、图谱推理与发音人稳定机制
  - 在配置区新增 v88.7 开关变量与规则提示函数，默认关闭
  - 移植 156 个独立函数，覆盖别名清洗、候选角色生成、N-gram/上下文窗口/语义证据/图谱推理、发音人稳定器、置信度计算等模块
  - 重构 `CharacterManager.prototype` 系列方法，增强别名识别与审计能力
  - 重构 `analyzeCharacter` 与 `checkAliasByApi`，接入 v88.7 的语义证据与备用模型投票逻辑
  - 在 `handleText` 主流程接入发音人稳定器，保留情绪模块、章节缓存、备用模型、切书修复能力
  - 统一 2.132 文件名、JSON 顶层 `name`/`version`、code 内部 `SpeechRuleJS.name`/`SpeechRuleJS.version`
  - 使用 `node --check` 验证 2.132 JS 无语法错误，Python `json.load` 验证 JSON 可解析
  - 运行 `node extract-js.js` 同步生成 `js/多角色朗读2.132...js` 调阅文件
...
- **注意事项**：
  ...
  - 所有输出必须为中文
  - 2.132 新增 v88.7 别名审计与发音人稳定机制，默认关闭相关开关，安装后如需启用请按需调整配置区开关
  - 2.132 保留 2.131 的切书修复、章节缓存、情绪模块、备用模型接力能力
  - 猫剪豆问 v1.20 已废弃，请勿使用；当前可用版本为 v1.19
