...
| 多角色朗读规则 | **v2.130** | `多角色朗读2.130【修复切书逻辑+统一内外版本号+添加自然情绪】.json` |

---

## 最新改动（多角色朗读 2.130，2026-06-25）
...
4. 同步生成 2.130 JSON 源文件与 JS 调阅文件

## 上一版本改动（v1.17，2026-06-25）
...
- [ ] 安装 v1.17 + 2.130 后测试切换新书，观察 `tts_debug_log.txt` 中 `[书名]` / `[换书检测]` / `[换书]` / `[多角色换书]` 日志是否正确
...
多角色朗读2.130...json        # 朗读规则（当前版本 v2.130）
...
- **注意事项**：
  ...
  - 多角色朗读 2.130 的切书逻辑与猫剪豆问 v1.17 保持行为一致
