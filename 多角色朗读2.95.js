...
// ===================== 情绪模块（v2.0 - 提取自多角色朗读2.95完整版）=====================
// 包含：开关配置、情绪配置项、工具函数、默认标签注入、GENSHIN角色注入、值提取、标签拼接、日志输出
...
// 子开关：AI自动分析（默认关闭，避免API消耗）
var ENABLE_EMOTION_AI = 0;
...
// 动态生成 UI 用的 items 字符串（从 EMOTION_KEY_MAP 构建，去重）
var EMOTION_ITEMS = (function() {
  var parts = ['无: "无"'];
  var seen = { "无": true };
  for (var key in EMOTION_KEY_MAP) {
    if (EMOTION_KEY_MAP.hasOwnProperty(key)) {
      var cn = EMOTION_KEY_MAP[key];
      if (!seen[cn]) {
        seen[cn] = true;
        parts.push(cn + ': "' + cn + '"');
      }
 ...
 * ES5兼容的数组扁平化函数
 * 递归将嵌套数组展平为一维数组
 * @param {Array} arr - 可能包含嵌套的数组
 * @returns {Array} 扁平化后的一维数组
 */
var EmotionForceFlattenArray = function(arr) {
  var result = [];
  for (var i = 0; i < arr.length; i++) {
    var item = arr[i];
    if (Object.prototype.toString.call(item) === '[object Array]') {
      result = result.concat(EmotionForceFlattenArray(item));
    } else {
      result.push(item);
    }
  }
...
// ===================== 情绪字段注入 =====================
/**
 * 为默认标签配置中的emotion字段赋值（对应2.95源码 line 4430-4435）
 * 在tagsData的默认标签定义中，emotion字段引用EMOTION_FIELD_CONFIG
 * 此函数返回emotion字段的配置对象，用于内嵌到标签定义中
 * @returns {Object} emotion字段配置
 */
function getDefaultEmotionField() {
  return {
    label: EMOTION_FIELD_CONFIG.label,
    hint: EMOTION_FIELD_CONFIG.hint,
    items: EMOTION_FIELD_CONFIG.items,
    default: EMOTION_FIELD_CONFIG.default
  };
}

/**
 * 为已存在的标签注入emotion字段
 * 对应2.95源码 line 4486: if (ENABLE_EMOTION) tagsData[voiceTag].emotion = EMOTION_FIELD_CONFIG;
 * @param {Object} tagConfig - 单个标签的配置对象
 * @param {boolean} enableEmotion - 是否开启情绪模块
 * @returns {Object} 注入后的标签配置
 */
function injectEmotionToTag(tagConfig, enableEmotion) {
  if (enableEmotion && tagConfig) {
    tagConfig.emotion = EMOTION_FIELD_CONFIG;
  }
  return tagConfig;
}

/**
 * 为GENSHIN角色标签注入emotion配置（含personality字段）
 * 对应2.95源码 line 4471-4495
 * 与老版本不同：新标签创建时同时包含personality和emotion两个字段
 * @param {Object} tagsData - 标签配置对象
 * @param {Object} genshinCharacters - GENSHIN角色映射
 * @param {Object} personalityConfig - 性格配置对象
 * @param {boolean} enableEmotion - 是否开启情绪模块
 * @returns {Object} 注入后的tagsData
 */
function injectGenshinEmotionField(tagsData, genshinCharacters, personalityConfig, enableEmotion) {
  for (var name in genshinCharacters) {
    if (genshinCharacters.hasOwnProperty(name)) {
      var voiceTag = genshinCharacters[name].voice.toString();
      // 1. 若标签已存在，在原有配置上新增性格+情绪选项
      if (tagsData[voiceTag]) {
        tagsData[voiceTag].personality = personalityConfig;
        if (enableEmotion) tagsData[voiceTag].emotion = EMOTION_FIELD_CONFIG;
      }
      // 2. 若标签不存在，新建配置（含性格选择框+情绪选择框）
      else {
        var _newTagCfg = { personality: personalityConfig };
        if (enableEmotion) _newTagCfg.emotion = EMOTION_FIELD_CONFIG;
        tagsData[voiceTag] = _newTagCfg;
      }
    }
  }
  return tagsData;
 ...
 * 对应2.95源码 line 4553-4569（GENSHIN）和 line 4621-4637（duihua）
 * 两条路径的提取逻辑完全一致，此处统一为一个函数
...
function extractEmotionValue(tagData, enableEmotion) {
  ...
  if (Object.prototype.toString.call(rawEmotion) === '[object Array]') {
    var flatArr = EmotionForceFlattenArray(rawEmotion);
    ...
    // 兼容配置对象格式：{label, hint, items, default}
...
/**
 * 构建duihua标签的情绪后缀
 * 对应2.95源码 line 4639-4640:
 *   var emotionVal = duihuaEmotion !== "" && duihuaEmotion !== "无" ? duihuaEmotion : "";
 *   var emotionWhole = emotionVal ? ("|" + emotionVal) : "";
 * 格式：|情绪值（如"|开心"），无情绪时为空字符串
 * @param {Object} tagData - 标签数据
 * @param {boolean} enableEmotion - 是否开启情绪模块
 * @returns {string} 情绪后缀字符串
...
/**
 * 从处理结果列表中提取并输出情绪信息
 * 对应2.95源码 line 6401-6408
 * @param {Array} list - 处理结果列表，每项含tag字段
 * @param {boolean} enableEmotion - 是否开启情绪模块
 */
function logEmotionFromResults(list, enableEmotion) {
  ...
  for (var k = 0; k < list.length; k++) {
    ...
    // 统一从 item.emotion 读取（手动/桥接/AI分析的结果都在这里）
    if (item.emotion) {
      emotionVal = EMOTION_KEY_MAP[item.emotion] || item.emotion;
      sourceStr = item.emotionSource ? ("(" + item.emotionSource + ")") : "";
    } else {
      // 回退：尝试从tag显示名的|后缀解析（兼容旧逻辑）
      if (tagStr.indexOf("|") !== -1) {
        emotionVal = tagStr.substring(tagStr.lastIndexOf("|") + 1);
      }
    }

    if (!emotionVal || emotionVal === "无") continue;
    var roleName = tagStr || "对话";
    console.log("【情绪】" + roleName + "：" + emotionVal + sourceStr);
  }
}
...
// ===================== 情绪桥接与AI分析模块 =====================
// AI情感分析超时（毫秒）
var EMOTION_ANALYZE_TIMEOUT = 8000;
...
function normalizeEmotionForMaoxiang(emo) {
  if (!emo) return "";
  emo = String(emo).trim();
  if (EMOTION_KEY_MAP[emo]) return emo;
  var key = EMOTION_NAME_TO_KEY[emo];
  if (key) return key;
  for (var k in EMOTION_KEY_MAP) {
    if (EMOTION_KEY_MAP.hasOwnProperty(k) && k.toLowerCase() === emo.toLowerCase()) {
      return k;
    }
  }
  return "";
...
function extractEmotionBridgeInfo(text) {
  ...
  if (!m) {
    m = s.match(/{{(?:emo|emotion)s*[:=]s*([^}|,:：，]+)s*(?:[}|,:：，]s*([0-9.]+))?s*}}/i);
  }
  if (!m) return { emotion: "", scale: 0, prompt: "", source: "" };
  return {
...
function findStringByKeys(obj, keys, depth) {
  ...
  for (var i = 0; i < keys.length; i++) {
    var val = safeReadField(obj, keys[i]);
...
function findEmotionScaleInRequest(request) {
  if (!request || typeof request !== 'object') return 4;
  var raw = findStringByKeys(request, ["emotionScale", "emotion_scale", "scale", "emotionStrength", "emotion_strength"],
    0);
  return normalizeEmotionScale(raw || 4);
}
...
function resolvePostModuleEmotion(request, rawText) {
  var enabled = isModuleEmotionEnabled() && ENABLE_EMOTION_BRIDGE;
  var bridge = extractEmotionBridgeInfo(rawText);
  var cleanedText = stripEmotionBridgeMarkers(rawText);
  var manualEmotion = "";
  var manualScale = 4;
  try {
    manualEmotion = normalizeEmotionForMaoxiang(ttsrv.tts.data["emotion"] || "");
    manualScale = normalizeEmotionScale(ttsrv.tts.data["emotionScale"] || "4");
  } catch (e) {}
...
function buildEmotionContextPrompt(emotionName, scale, customPrompt) {
  ...
  var key = normalizeEmotionForMaoxiang(emotionName);
  if (!key || key === "neutral") return "";
  var displayName = EMOTION_KEY_MAP[key] || emotionName;
...
function analyzeSentimentWithEmotionList(text, context, emotionList) {
  ...
  if (!emotionList || emotionList.length === 0) return null;

  var cacheKey = (text.length > 80 ? text.substring(0, 80) : text) + "_" + emotionList.join(",").length;
  if (sentimentCache[cacheKey] !== undefined) return sentimentCache[cacheKey];
  if (!ENABLE_EMOTION_AI) return null;
  ...
  (context && context.trim() ? "小说上下文片段：" + context + "\n    " : "\n    ") +
    ...
    var apiConfig;
    try {
      apiConfig = DualKeyManager.getNextNameAnalyzeKey();
    } catch (e) {
      console.log("[AI情感] DualKeyManager未就绪，跳过分析");
      return null;
    }
    if (!apiConfig || !apiConfig.key) {
      console.log("[AI情感] 无可用API密钥，跳过分析");
      return null;
    }
      ...
      try {
        ...
        if (numMatch) {
          ...
          if (idx >= 0 && idx < emotionList.length) {
            ...
            if (finalVal) {
              console.log("[AI] 情绪分析结果: " + (EMOTION_KEY_MAP[finalVal] || finalVal));
            } else {
        ...
        console.log("[AI情感] 请求异常: " + e);
      }
      if (retry < 1) {
        try { java.lang.Thread.sleep(500); } catch (e) {}
      }
    }
    return null;
  }
  ...
  // ========== 日志增强 ==========
  function logEmotionBridge(emotionInfo) {
    if (!emotionInfo || !emotionInfo.emotion) return;
    if (emotionInfo.source === "bridge") {
      console.log("[情绪桥接] 从文本标记提取到情绪: " + (EMOTION_KEY_MAP[emotionInfo.emotion] || emotionInfo.emotion) + " 强度:" +
        emotionInfo.scale);
    } else if (emotionInfo.source === "module") {
      console.log("[情绪桥接] 从上游模块获取到情绪: " + (EMOTION_KEY_MAP[emotionInfo.emotion] || emotionInfo.emotion) + " 强度:" +
        emotionInfo.scale);
    } else if (emotionInfo.source === "manual") {
      console.log("[情绪桥接] 使用手动配置情绪: " + (EMOTION_KEY_MAP[emotionInfo.emotion] || emotionInfo.emotion) + " 强度:" +
        emotionInfo.scale);
    }
  }

  function logAiEmotionStart(text) {
    if (ENABLE_EMOTION_AI) {
      console.log("[AI] 开始分析文本情绪... 文本长度:" + String(text || "").length);
    }
  }
  ...
  var ENABLE_MODEL_SAME_PERSON_EVIDENCE_GUARD =
  1; // \u6a21\u578b\u540c\u4eba\u8bc1\u636e\u542b\u5171\u540c/\u5bf9\u8bdd\u7b49\u8bcd\u65f6\u62e6\u622a // 过滤“众人/四名修士”等群体名
  ...
  var BackupModelManager = (function() {
    ...
    function loadBackupFromContent(backupContent, defaultConfig) {
      ...
      if (backupContent) {
        ...
        if (backupPool.length > 0) {
          ...
          backupPendingLog = "【🟡备用模型】配置已加载，模型：" + backupConfig.model + "，密钥末尾4位：" + backupConfig.key.slice(-4) +
            "，地址：" + (backupConfig.endpoint || defaultConfig.endpoint);
        } else {
    ...
    return {
      ...
      deactivateBackup: function() { backupActive = false;
        backupPendingLog = null; },
      ...
      return function() {
        ...
        try {
          ...
          if (!needWaitMultiResult) {
            ...
            console.log("【" + (scene === "nameAnalyze" ? "🔴🔴🔴✅ 姓名分析" : "🔵🔵🔵✅ 别名校验") + "成功！】 单结果模式，立即使用，模型：" +
              apiConfig.model + "，密钥末尾4位：" + apiConfig.key.slice(-4));
            if (hasWakedUp.compareAndSet(false, true)) {
              countDownLatch.countDown();
            }
          } else {
            // 多结果模式：打印当前进度
            console.log("【" + (scene === "nameAnalyze" ? "🔴🔴🔴 姓名分析" : "🔵🔵🔵 别名校验") + "成功" + currentSuccessNum +
              "/" + targetSuccessCount + "个】 模型：" + apiConfig.model + "，密钥末尾4位：" + apiConfig.key.slice(-4));
            // 达到目标成功数，唤醒主线程
  ...
  function voteModelRelations(successResults) {
    ...
    for (var i = 0; i < successResults.length; i++) {
      ...
      for (var j = 0; j < rels.length; j++) {
        ...
        if (!bucket[key]) bucket[key] = { a: r.a, b: r.b, type: r.type, evidence: [], votes: 0, maxConfidence: 0, seq: r
            .seq || "" };
        bucket[key].votes++;
    ...
    for (var k in bucket) {
      ...
      if (item.votes >= MODEL_RELATION_MIN_VOTES || item.maxConfidence >= 80) {
        out.push({ a: item.a, b: item.b, type: item.type, evidence: item.evidence.join(" | "), seq: item.seq,
          confidence: item.maxConfidence, votes: item.votes });
      }
    }
    return out;
  }
  ...
  function voteNameAnalyzeResult(successResults, dialogTextMap) {
    ...
    if (enableMerge) {
      ...
      console.log("【🔴🔴🔴 投票别名合并】已" + (enableMerge ? "开启" : "关闭") + "，内存映射表共" + Object.keys(nameToMainNameMap).length +
        "条记录");
    }
    // ========== 优化结束 ==========
  ...
  function voteAliasAnalyzeResult(successResults) {
    ...
    if (enableMerge) {
      ...
      console.log("【🔵🔵🔵 别名投票别名合并】已" + (enableMerge ? "开启" : "关闭") + "，内存映射表共" + Object.keys(nameToMainNameMap)
        .length + "条记录");
    }
    // ========== 优化结束 ==========
  ...
  function voteAliasRefineResult(successResults) {
    ...
    console.log("【🔵🔵🔵✅ 别名清洗投票完成】 主名：" + selectedMainName + "，确认别名数：" + finalConfirmedAliases.length + "，剔除别名数：" +
      finalRemovedAliases.length + "，基于" + successResults.length + "个API结果");
    return finalResult;
  }
  ...
  var GRAPH_RULE_SOURCE = "tts-rule-93-v93";
  ...
  function graphBuildChapterKey(text) {
    ...
    if (typeof graphCurrentChapterIndex !== "undefined" && graphCurrentChapterIndex !== "") return "chapter:" +
      graphCurrentChapterIndex;
    return "chapter:unknown";
  }
  ...
  function graphSetCurrentChapterKey(bookUrl, chapterIndex) {
    try {
      ...
      var realSwitch = (oldIndex !== "" && newIndex !== "" && String(oldIndex) !== String(newIndex)) || (oldBookUrl !==
        "" && newBookUrl !== "" && oldBookUrl !== newBookUrl);
  ...
  function graphIsAliasGroupName(name) {
    ...
    if (
      /^(\u4f17\u4eba|\u4f17\u4fee\u58eb|\u4f17\u5f1f\u5b50|\u8bf8\u4eba|\u8bf8\u4fee|\u4e8c\u4eba|\u4e24\u4eba|\u4e09\u4eba|\u56db\u4eba|\u51e0\u4eba|\u6570\u4eba|\u4e00\u884c\u4eba|\u4e00\u7fa4\u4eba|\u5176\u4ed6\u51e0\u4eba|\u5176\u4ed6\u4fee\u58eb|\u5728\u5750\u4fee\u58eb)$/
      .test(name)) return true;
    if (
      /[\u4e00\u4e8c\u4e24\u4e09\u56db\u4e94\u516d\u4e03\u516b\u4e5d\u5341\u6570\u51e00-9]+(\u540d|\u4e2a|\u4f4d|\u4eba).*(\u4fee\u58eb|\u5973\u5b50|\u7537\u5b50|\u5f1f\u5b50|\u8001\u8005|\u5927\u6c49|\u5c11\u5e74|\u5c11\u5973|\u5112\u751f|\u6c49\u5b50|\u6cd5\u58eb|\u957f\u8001|\u50e7\u4eba|\u4eba)/
      .test(name)) return true;
    if (
      /^(\u9ad8\u77ee|\u4e00\u9ad8\u4e00\u77ee|\u4e00\u80d6\u4e00\u7626|\u4e00\u7537\u4e00\u5973|\u4e24\u7537|\u4e24\u5973).*(\u4fee\u58eb|\u7537\u5b50|\u5973\u5b50|\u6cd5\u58eb|\u8001\u8005|\u4eba)/
      .test(name)) return true;
    if (
      /(\u4e8c\u4eba|\u4e24\u4eba|\u4e09\u4eba|\u56db\u4eba|\u51e0\u4eba|\u6570\u4eba|\u4f17\u4eba|\u4e00\u884c\u4eba|\u4e00\u7fa4\u4eba)$/
      .test(name)) return true;
    return false;
  }
  ...
  function graphSamePersonEvidenceBlockReason(a, b, evidence) {
    ...
    evidence = graphSafeString(evidence || "", 300);
    if (!evidence) return "";
    var hasExplicitAlias =
      /(\u53c8\u53eb|\u4e5f\u53eb|\u540d\u53eb|\u5168\u540d|\u5c0f\u540d|\u5916\u53f7|\u5316\u540d|\u7ef0\u53f7|\u88ab\u79f0\u4e3a|\u88ab\u79f0\u4f5c|\u672c\u540d|\u539f\u540d|\u771f\u540d|\u5373\u4e3a|\u4e3a\u540c\u4e00\u4eba|\u540c\u4e00\u4eba|\u540c\u4e00\u540d|\u540c\u4e00\u4f4d)/
      .test(evidence) || graphEvidenceHasStrongSamePersonPhrase(a, b, evidence);
    var hasPluralOrInteraction =
      /(\u5171\u540c|\u4e00\u8d77|\u4e00\u540c|\u540c\u4e3a|\u5747\u4e3a|\u7686\u4e3a|\u4e8c\u4eba|\u4e24\u4eba|\u4e09\u4eba|\u51e0\u4eba|\u6570\u4eba|\u53cc\u65b9|\u5f7c\u6b64|\u4e92\u76f8|\u540c\u884c|\u4ea4\u8c08|\u5bf9\u8bdd|\u5546\u8bae|\u8c0b\u5212|\u8ba8\u8bba|\u5408\u4f5c|\u5173\u7cfb|\u540c\u5c5e|\u5e76\u5217|\u548c|\u4e0e|\u53ca|\u8ddf)/
      .test(evidence);
    if (hasPluralOrInteraction && !hasExplicitAlias)
    return "\u5171\u540c/\u4e92\u52a8\u8bc1\u636e\u4e0d\u662f\u540c\u4eba";
    return "";
  }
  ...
  function graphGetPairStats(stats, a, b) {
    ...
    if (!stats[key]) {
      stats[key] = { a: a, b: b, chapterCount: 0, sameSentence: 0, adjacentSpeaker: 0, directInteraction: 0,
          listedTogether: 0, explicitRelation: 0, positiveMention: 0, modelPositive: 0, modelNegative: 0,
        updatedAt: "" };
    }
    return stats[key];
  }
  ...
  function graphCurrentChapterId() {
    return graphSafeString((typeof graphCurrentChapterIndex !== "undefined" && graphCurrentChapterIndex) || (
      typeof graphRemoteChapterIndex !== "undefined" && graphRemoteChapterIndex) || "unknown", 80);
  }
  ...
  function graphIsPositiveClosureReason(reason) {
    reason = graphSafeString(reason || "", 80);
    return reason === "model_same_person" || reason === "explicit_same_person_statement" || reason ===
      "alias_refine_confirmed" || reason === "positive_chain_closed";
  }

  function graphEdgeHasClosureReason(edge, positive) {
    if (!edge || !edge.reasons) return false;
    for (var i = 0; i < edge.reasons.length; i++) {
      if (positive ? graphIsPositiveClosureReason(edge.reasons[i]) : graphIsNegativeClosureReason(edge.reasons[i]))
        return true;
    }
    return false;
  }
  ...
  CharacterManager.prototype.setAliasGraphBook = function(bookName, bookUrl) {
    ...
    graphRemoteLog("graph_book_cache", { bookKey: bookKey, positiveFile: this.aliasPositiveGraphFile,
      negativeFile: this.aliasNegativeGraphFile, cooccurFile: this.aliasCooccurStatsFile });
  };
  ...
  CharacterManager.prototype.recordPositiveAliasEdge = function(a, b, score, reason, extra) {
    ...
    if (mergeBlockReason) {
      ...
      graphRemoteLog("alias_merge_blocked", { stage: "positive_edge", newName: graphNormalizeName(a),
        mainName: graphNormalizeName(b), reason: mergeBlockReason, sourceReason: reason });
      return;
    }
    ...
    if (graphAddWeightedEdge(this.aliasPositiveGraph, a, b, score, reason, extra || "")) {
      ...
      graphRemoteLog("graph_positive_edge", { a: graphNormalizeName(a), b: graphNormalizeName(b), score: score,
        reason: reason, extra: graphSafeString(extra, 180) });
    }
  };
  ...
  CharacterManager.prototype.recordNegativeAliasEdge = function(a, b, score, reason, extra) {
    ...
    if (graphAddWeightedEdge(this.aliasNegativeGraph, a, b, score, reason, extra || "")) {
      ...
      graphRemoteLog("graph_negative_edge", { a: graphNormalizeName(a), b: graphNormalizeName(b), score: score,
        reason: reason, extra: graphSafeString(extra, 180) });
    }
  };
  ...
  CharacterManager.prototype.applyNegativeTriadClosure = function(a, b, reason) {
    ...
    for (var i = 0; i < neighbors.length; i++) {
      ...
      for (var p = 0; p < pairs.length; p++) {
        if (graphMarkChapterEvidenceOnce(this.aliasCooccurStats, pairs[p][0], pairs[p][1],
          "triad_interaction_closed") && graphAddWeightedEdge(this.aliasNegativeGraph, pairs[p][0], pairs[p][1],
            GRAPH_TRIAD_CLOSED_SCORE, "triad_interaction_closed", extra)) {
          pairAdded++;
        }
      }
      if (pairAdded) {
  ...
  CharacterManager.prototype.applyPositiveChainClosure = function(a, b, reason) {
    ...
    function tryAdd(x, y, via) {
      ...
      if (graphAddWeightedEdge(that.aliasPositiveGraph, x, y, GRAPH_POSITIVE_CHAIN_SCORE, "positive_chain_closed",
          "正链:" + x + "=" + via + "=" + y)) {
        added++;
  ...
  CharacterManager.prototype.applyModelRelationEvidence = function(relations, chapterFullContent) {
    if (!ENABLE_ALIAS_GRAPH || !ENABLE_MODEL_RELATION_EVIDENCE || !relations || !Array.isArray(relations) || relations
      .length === 0) return { positive: 0, negative: 0 };
    if (!this.aliasCooccurStats) this.aliasCooccurStats = {};
    var pos = 0;
    var neg = 0;
    var detailLimit = parseInt(MODEL_RELATION_DETAIL_LIMIT, 10);
    if (isNaN(detailLimit) || detailLimit < 0) detailLimit = 40;
    var observe = { total: relations.length, accepted: 0, skipped: 0, positive: 0, negative: 0, unknown: 0,
    samples: [] };
    ...
    for (var i = 0; i < relations.length; i++) {
      ...
      if (type.indexOf("same") !== -1 || type.indexOf("alias") !== -1 || type.indexOf("同一") !== -1 || type.indexOf(
          "别名") !== -1 || type.indexOf("化名") !== -1 || type.indexOf("昵称") !== -1) {
        var sameBlockReason = graphSamePersonEvidenceBlockReason(a, b, evidence);
        if (sameBlockReason) {
          st.modelNegative = Number(st.modelNegative || 0) + 1;
          if (graphMarkChapterEvidenceOnce(this.aliasCooccurStats, a, b, "model_same_person_blocked") &&
            graphAddWeightedEdge(this.aliasNegativeGraph, a, b, 4, "model_same_person_blocked", sameBlockReason + (
              evidence ? ":" + evidence : ""))) {
            neg++;
          }
          observe.accepted++;
          ...
          graphRemoteLog("model_relation_blocked", { a: a, b: b, type: type, confidence: confidence || 0,
            reason: sameBlockReason, evidence: graphSafeString(evidence, 180) });
          addModelRelationSample(r, a, b, type, confidence, evidence, "\u62e6\u622a", "\u8f6c\u53cd\u5411",
            "model_same_person_blocked");
          continue;
        }
        st.modelPositive = Number(st.modelPositive || 0) + 1;
        if (graphMarkChapterEvidenceOnce(this.aliasCooccurStats, a, b, "model_same_person") && graphAddWeightedEdge(
            this.aliasPositiveGraph, a, b, 4.5, "model_same_person", evidence || "模型判定同一人")) {
          pos++;
      ...
      } else if (type.indexOf("relation") !== -1 || type.indexOf("relationship") !== -1 || type.indexOf(
        "different") !== -1 || type.indexOf("interaction") !== -1 || type.indexOf("together") !== -1 || type.indexOf(
          "关系") !== -1 || type.indexOf("不同") !== -1 || type.indexOf("互动") !== -1 || type.indexOf("并列") !== -1) {
        st.modelNegative = Number(st.modelNegative || 0) + 1;
        var reason = type.indexOf("interaction") !== -1 || type.indexOf("互动") !== -1 ? "model_direct_interaction" : (
          type.indexOf("together") !== -1 || type.indexOf("并列") !== -1 ? "model_listed_together" : (type.indexOf(
              "relation") !== -1 || type.indexOf("relationship") !== -1 || type.indexOf("关系") !== -1 ?
            "model_relationship" : "model_different_person"));
        if (graphMarkChapterEvidenceOnce(this.aliasCooccurStats, a, b, reason) && graphAddWeightedEdge(this
            .aliasNegativeGraph, a, b, 4, reason, evidence || "模型判定非同一人")) {
          neg++;
    ...
    if (pos || neg) {
      ...
      graphRemoteLog("model_relation_apply", { positive: pos, negative: neg, relationCount: relations.length,
  ...
  CharacterManager.prototype.updateAliasGraphsFromCache = function(dialogList, chapterFullContent, modelRelations) {
    ...
    var scanKey = dialogList.length + ":" + graphHash(text.substring(0, 3500) + "#" + JSON.stringify(dialogList)
      .substring(0, 1500));
    ...
    function addPos(a, b, score, reason, extra) {
      ...
      if (graphAddWeightedEdge(this.aliasPositiveGraph, a, b, score, reason, extra)) {
        ...
        remoteGraphEdgeLog("graph_positive_edge", { a: a, b: b, score: score, reason: reason, extra: graphSafeString(
            extra, 160) });
      }
    }
    ...
    function addNeg(a, b, score, reason, extra) {
      ...
      if (graphAddWeightedEdge(this.aliasNegativeGraph, a, b, score, reason, extra)) {
        ...
        remoteGraphEdgeLog("graph_negative_edge", { a: a, b: b, score: score, reason: reason, extra: graphSafeString(
            extra, 160) });
      }
    }
    ...
    for (var x = 0; x < names.length; x++) {
      for (var y = x + 1; y < names.length; y++) {
        ...
        var positiveAliasWords =
          "(\u53c8\u540d|\u522b\u540d|\u672c\u540d|\u539f\u540d|\u771f\u540d|\u5316\u540d|\u7ef0\u53f7|\u5916\u53f7|\u5168\u540d|\u5c0f\u540d|\u540d\u4e3a|\u540d\u53eb|\u53c8\u53eb\u505a|\u4e5f\u53eb\u505a|\u88ab\u79f0\u4e3a|\u88ab\u79f0\u4f5c)";
        var positiveGapBefore =
          "[\u7684\s\u3000,\uff0c\u3001:\uff1a\u201c\u201d\u2018\u2019\u300a\u300b\u3010\u3011\\(\\)\uff08\uff09]{0,6}";
        var positiveGapAfter =
          "[\u662f\u53eb\u4e3a\u4f5c\s\u3000,\uff0c\u3001:\uff1a\u201c\u201d\u2018\u2019\u300a\u300b\u3010\u3011\\(\\)\uff08\uff09]{0,8}";
        ...
        if (sameSentence > 0) {
          ...
          if (!positiveHit && sameSentence >= COOCUR_NEG_SENTENCE_MIN) {
            addNeg.call(this, A, B, Math.min(2.5, sameSentence * 0.45), "same_sentence_cooccur", "同句共现" +
              sameSentence + "次");
          }
        }
      }
    ...
    var modelSummary = this.applyModelRelationEvidence ? this.applyModelRelationEvidence(modelRelations || [], text) :
    { positive: 0, negative: 0 };
    this.saveAliasGraphData();
    this.saveAliasCooccurStats();
    graphShortLog("共现" + coHits + " 正" + (posEdges + (modelSummary.positive || 0)) + " 反" + (negEdges + (modelSummary
      .negative || 0)));
    graphRemoteLog("cooccur_scan_done", { names: names.length, cooccurHits: coHits, positiveEdges: posEdges + (
        modelSummary.positive || 0), negativeEdges: negEdges + (modelSummary.negative || 0),
      modelRelations: modelRelations ? modelRelations.length : 0, dialogCount: dialogList.length });
  };

  CharacterManager.prototype.buildAliasEvidenceHint = function(newName, chapterFullContent, currentDialogueText, gender,
    age) {
    ...
    for (var i = 0; i < this.characterRecords.length; i++) {
      ...
      for (var a = 0; a < aliases.length; a++) {
        ...
        if (ps > maxPos) { maxPos = ps;
          posReasons = graphGetEdgeReasons(this.aliasPositiveGraph, newName, al); }
        if (ns > maxNeg) { maxNeg = ns;
          negReasons = graphGetEdgeReasons(this.aliasNegativeGraph, newName, al); }
        var st = this.aliasCooccurStats ? this.aliasCooccurStats[graphPairKey(newName, al)] : null;
        if (st && (!bestStats || Number(st.sameSentence || 0) + Number(st.adjacentSpeaker || 0) > Number(bestStats
            .sameSentence || 0) + Number(bestStats.adjacentSpeaker || 0))) bestStats = st;
      }
      if (maxPos >= GRAPH_POSITIVE_HINT_MIN) positives.push({ name: rec.name, score: maxPos, reasons: posReasons });
      var coUseful = bestStats && (Number(bestStats.sameSentence || 0) >= COOCUR_NEG_SENTENCE_MIN || Number(bestStats
          .adjacentSpeaker || 0) >= COOCUR_NEG_ADJACENT_MIN || Number(bestStats.directInteraction || 0) > 0 ||
        Number(bestStats.listedTogether || 0) > 0 || Number(bestStats.explicitRelation || 0) > 0);
      if (maxNeg >= GRAPH_NEGATIVE_SOFT_BLOCK || coUseful) negatives.push({ name: rec.name, score: maxNeg,
        reasons: negReasons, stats: bestStats });
    }
    ...
    if (positives.length > 0) {
      ...
      for (var p = 0; p < Math.min(5, positives.length); p++) {
        lines.push("- " + newName + " → " + positives[p].name + "：分" + Number(positives[p].score || 0).toFixed(1) +
          "，因" + (positives[p].reasons || []).join("|"));
      }
    }
    if (negatives.length > 0) {
      ...
      for (var n = 0; n < Math.min(8, negatives.length); n++) {
        ...
        lines.push("- " + newName + " ≠ " + ne.name + "：" + level + "，反分" + Number(ne.score || 0).toFixed(1) + "，同句" +
          Number(st2.sameSentence || 0) + "，相邻" + Number(st2.adjacentSpeaker || 0) + "，因" + (ne.reasons || []).join(
            "|"));
      }
    }
    graphRemoteLog("alias_graph_hint", { newName: newName, graphHint: lines.join("\n"), positiveCount: positives
        .length, negativeCount: negatives.length });
    return lines.join("\n");
  };
  ...
  CharacterManager.prototype.selectVoiceByGlobalRandom = function(candidates, label) {
    ...
    try { console.log("【发音人】" + label + " " + selectedVoice + " 全局" + this.globalVoiceUsage[selectedVoice]); } catch (
      e) {}
    try {
        ...
        for (var name in GENSHIN_CHARACTERS) {
          if (GENSHIN_CHARACTERS.hasOwnProperty(name)) {
            ...
            if (info.gender === gender && info.age === similarAge) {
              var isUsed = usedVoiceMap.hasOwnProperty(info.voice) || mainRoleVoiceBlacklist.hasOwnProperty(info
                .voice);
              var isAvailable = this.isVoiceAvailable(info.voice);
              if (!isUsed && isAvailable) {
                candidates.push({
  ...
  CharacterManager.prototype.analyzeCharacter = function(fullText, characterId, allDialogues) {
    ...
    function parseNameAnalyzeResponse(response) {
      ...
      if (!apiResponse.choices || !apiResponse.choices[0] || !apiResponse.choices[0].message || !apiResponse.choices[
          0].message.content) {
  ...
  CharacterManager.prototype.getAllCharacterNamesAndAliases = function(targetGender) {
    var allNamesSet = {}; // 用对象模拟Set自动去重：键=主名/别名，值=true
    ...
    for (var i = 0; i < apiLimitedRecords.length; i++) {
      ...
      // 2. 添加别名（去重，且不与主名重复）
      if (record.aliases && record.aliases.trim()) {
        var _rawAliases = record.aliases.split("|");
        var aliasList = [];
        for (var _ai = 0; _ai < _rawAliases.length; _ai++) {
          var _alias = _rawAliases[_ai].trim();
          if (_alias && _alias !== mainName) {
    ...
    var nameListForApi = [];
    for (var _nameKey in allNamesSet) {
      if (allNamesSet.hasOwnProperty(_nameKey)) {
        nameListForApi.push({ name: _nameKey });
      }
    }
  ...
  CharacterManager.prototype.checkAliasByApi = function(newName, chapterFullContent, newCharacterGender,
    ...
    if (this.updateAliasGraphsFromCache) {
      try {
        ...
        this.updateAliasGraphsFromCache(graphCache.dialogList || [], chapterFullContent || "", graphCache
          .relationEvidence || []);
      } catch (graphCacheErr) {}
    }
    graphRemoteSetChapter(graphBuildChapterKey(chapterFullContent || this.contextHistory2 || ""), "别名校验");
    var graphEvidenceHint = this.buildAliasEvidenceHint ? this.buildAliasEvidenceHint(newName, chapterFullContent,
      currentDialogueText, newCharacterGender, "") : "";
    graphRemoteLog("alias_check_start", { newName: newName.trim(), mode: mode, localRoleCount: nameListForApi.length,
      hasGraphHint: !!graphEvidenceHint });
    ...
    prompt += "【输入信息】\n" +
      ...
      "【待分析小说上下文】\n" + String(this.contextHistory2 || '').slice(-1000) + text2 + String(next100Chars || '').slice(0,
        500) + "\n\n" +
      "【新名字】是\n" + newName.trim() + "\n" +
      "---\n" +
      "【输出要求】\n" +
    ...
    if (finalResult && finalResult.isAlias && finalResult.mainName) {
      ...
      if (aliasBlockReason) {
        aliasShortLog("\u5408\u5e76\u62e6\u622a " + graphNormalizeName(newName) + "\u2192" + graphNormalizeName(
          finalResult.mainName));
        graphRemoteLog("alias_merge_blocked", { stage: "alias_check", newName: graphNormalizeName(newName),
          mainName: graphNormalizeName(finalResult.mainName), reason: aliasBlockReason });
  ...
  CharacterManager.prototype.refineAliasGroupByApi = function(mainRecord, newName, chapterFullContent,
    currentDialogueText) {
    ...
    if (finalResult && finalResult.isSamePerson) {
      ...
      if (refineBlockReason) {
        aliasShortLog("\u6e05\u6d17\u62e6\u622a " + graphNormalizeName(newName) + "\u2192" + graphNormalizeName(
          finalResult.mainName || mainName));
        graphRemoteLog("alias_merge_blocked", { stage: "alias_refine", newName: graphNormalizeName(newName),
          mainName: graphNormalizeName(finalResult.mainName || mainName), reason: refineBlockReason });
  ...
  CharacterManager.prototype.processCharacter = function(fullText, characterId, allDialogues, chapterFullContent) {
    ...
    if (bieming !== 0 && newCharacterName !== "未知") {
      ...
      if (!existingRecord) {
        ...
        if (aliasCheckResult && !aliasCheckResult.isAlias && aliasCheckResult.mainName && this
          .recordNegativeAliasEdge) {
          this.recordNegativeAliasEdge(newCharacterName, aliasCheckResult.mainName, 3, "alias_api_rejected",
            aliasCheckResult.reason || "别名API拒绝");
        }
        ...
        if (aliasCheckResult && aliasCheckResult.isAlias && aliasCheckResult.mainName) {
          ...
          if (processAliasBlockReason) {
            aliasShortLog("\u5408\u5e76\u62e6\u622a " + graphNormalizeName(newCharacterName) + "\u2192" +
              graphNormalizeName(aliasCheckResult.mainName));
            graphRemoteLog("alias_merge_blocked", { stage: "process", newName: graphNormalizeName(newCharacterName),
              mainName: graphNormalizeName(aliasCheckResult.mainName), reason: processAliasBlockReason });
          ...
          if (targetMainRecord) {
            ...
            if (refineResult && refineResult.isSamePerson && refineResult.mainName) {
              ...
              if (normalizedRefine) {
                ...
                this.saveRecords();
                if (this.recordPositiveAliasEdge) this.recordPositiveAliasEdge(newCharacterName, targetMainRecord
                  .name, 3.5, "alias_refine_confirmed", aliasCheckResult.reason || "别名清洗确认");
                if (typeof graphRemoteLog === "function") graphRemoteLog(
                "alias_merge_confirmed", { newName: newCharacterName, mainName: targetMainRecord.name,
                  aliases: finalAliasList });
              }
            }
    ...
    if (!targetMainRecord) {
      ...
      if (targetMainRecord.genderAgeHistory.length >= CONFIG.reEvaluateThreshold) this.reEvaluateCharacter(
        targetMainRecord);
  ...
  var SpeechRuleJS = {
    name: "多角色朗读2.95（修复缓存中断+定位优化+情绪日志+备用模型接力+ES5兼容+情绪桥接AI分析整合版）",
    id: "mingwuyan",
    author: "命無言、萌新改",
    version: 95,
    zdfp: 1,
    ...
    tagsData: (function() {
      ...
      // 为 GENSHIN_CHARACTERS 所有标签注入性格+情绪配置（使用情绪模块函数）
      var _personalityConfig = {
        label: "角色性格",
        hint: "选择角色的性格特质（独立配置，不影响其他选项）"
      };
      tagsData = injectGenshinEmotionField(tagsData, GENSHIN_CHARACTERS, _personalityConfig, ENABLE_EMOTION);

      // 为所有已有标签注入情绪配置（确保旁白、对话、发音人等都能读取情绪）
      if (ENABLE_EMOTION) {
        for (var _tagKey in tagsData) {
          if (tagsData.hasOwnProperty(_tagKey)) {
            var _cfg = tagsData[_tagKey];
            if (_cfg && typeof _cfg === 'object' && !_cfg.emotion) {
              _cfg.emotion = EMOTION_FIELD_CONFIG;
            }
          }
        }
      }
    ...
    getTagName: function(tag, tagData) {
      ...
      if (genshinTagKey !== "") {
        ...
        // 情绪部分（使用情绪模块函数）
        var emotionWhole = buildGenshinEmotionSuffix(tagData, ENABLE_EMOTION);
      ...
      // 2. duihua标签处理（括号完全匹配，复用GENSHIN逻辑）
        ...
        // 最终拼接（括号不变）
        // 情绪部分（使用情绪模块函数）
        var emotionWhole = buildDuihuaEmotionSuffix(tagData, ENABLE_EMOTION);
    ...
    handleText: function(text, tagsData) {
      ...
      for (var i = 0; i < allLocalSoundTags.length; i++) {
        ...
        if (tagsData && tagsData[tagKey] && tagsData[tagKey].audioName) {
          ...
          for (var j = 0; j < flatConfig.length; j++) {
            ...
            // 正则关键词处理
            if (inputValue.startsWith("@") || inputValue.startsWith("＜") || inputValue.startsWith("＞") || inputValue
              .startsWith("<")) {
              allRegexList.push({
                originKW: inputValue,
      ...
      for (var i = 0; i < allLocalSoundTags.length; i++) {
        ...
        for (var r = 0; r < regexList.length; r++) {
          ...
          regexMarkedText = regexMarkedText.replace(rkw.regex, function(match) {
            ...
            // 原符号转义逻辑保留
            var replacedContent = SpeechRuleJS.replaceTargetContentSymbols(finalContent);
            var newContentWithTag = "{{" + tagName + "_" + replacedContent + "}}" + finalContent + "{{" +
              tagName + "结束}}";
      ...
      if (soundKeywords.length > 0 && text.includes("“")) {
        ...
        text = text.replace(quotedReg, function(match) {
          ...
          for (var r = 0; r < parsedKWs.regexKWs.length; r++) {
            ...
            if (rkw.flag === "<" || rkw.flag === "＜" || rkw.flag === "＞" || rkw.flag === ">" || rkw.flag ===
              "@") { // 半角<、全角＜开头的正则，直接跳过
              continue;
            }
            // --------------------------------------------------------------------------------------------------
          ...
          if (repeatCheck.isRepeat) {
            ...
            for (var g = 0; g < parsedKWs.normalKWGroups.length; g++) {
              ...
              var singleKWReg = new RegExp('(^|\\s|[' + commonPunctuation + '])(' + childrenAlt + ')([' +
                commonPunctuation + ']{0,2})?' + '(?=\\s|[' + commonPunctuation + ']|["”]|$)', 'g');
              result = result.replace(singleKWReg, function(match, prefix, matchedChildKW, punc) {
                if (match.includes("〖")) return match;
                var afterPunc = text.substring(text.indexOf(match) + match.length, text.indexOf(match) +
                  match.length + 1);
                if (afterPunc && !commonPunctuation.includes(afterPunc) && afterPunc !== " " &&
                  afterPunc !== "" && afterPunc !== "\"" && afterPunc !== "”") {
                  return match;
                }
          ...
          if (repeatCheck.isRepeat) {
            ...
            for (var g = 0; g < parsedKWs.normalKWGroups.length; g++) {
              ...
              var repeatKWReg = new RegExp(
                '(^|\\s|["“]|[' + commonPunctuation + '])(' + childrenAlt + ')([' + commonPunctuation +
                ']{0,1})?' + '\\2' +
              ...
              text = text.replace(singleKWReg, function(match, prefix, matchedChildKW, punc) {
                if (match.includes("〖")) return match;
                var afterPunc = text.substring(text.indexOf(match) + match.length, text.indexOf(match) + match
                  .length + 1);
      ...
      try {
        if (text.includes("“")) {
          ...
          if (dataJsonContent && dataJsonContent.trim() !== "") {
            ...
            try {
              // 【修复缓存中断】保留data.json直连，并尝试HTTP补充后续章节
              var rawBookUrlForGraph = bookData.bookUrl ? String(bookData.bookUrl) : '';
              var currentChapterIndex = typeof bookData.durChapterIndex !== 'undefined' ? bookData.durChapterIndex :
                -1;

              // 如果配置需要且信息足够，通过HTTP API补充后续章节
              if (rawBookUrlForGraph && currentChapterIndex >= 0 && NEXT_CHAPTER_COUNT > 0) {
                var bookUrl = encodeURIComponent(rawBookUrlForGraph);
                for (var i = 1; i <= NEXT_CHAPTER_COUNT; i++) {
                  ...
                  try {
                    ...
                    if (chapterResponse && chapterResponse.body()) {
                      ...
                      if (chapterData.isSuccess && chapterData.data) {
                        fullChapterContent += "\n" + String(chapterData.data).toString();
                      } else {
                        break;
                      }
                    } else {
              ...
              if (typeof characterManager !== 'undefined' && characterManager && characterManager
                .setAliasGraphBook) {
              ...
              if (characterManager.contextHistory2 && characterManager.contextHistory2.length >= 10) {
                historyTail = characterManager.contextHistory2.slice(-30).trim();
              }

              var historyPos = -1;
              if (historyTail) {
                historyPos = fullChapterContent.indexOf(historyTail);
              }
              ...
              // 定位最终匹配位置：优先选择 historyPos 之后的匹配（朗读向前推进）
              if (currentMatchPositions.length > 0) {
                if (historyPos !== -1) {
                  var minDistance = Infinity;
                  var closestPos = -1;
                  // 第一轮：只在 historyPos 之后（含）找最近匹配
                  for (var p = 0; p < currentMatchPositions.length; p++) {
                    var pos = currentMatchPositions[p];
                    if (pos >= historyPos) {
                      var distance = pos - historyPos;
                  ...
                  // 第二轮：如果后面没有，再往前找最后一个（容错）
                  if (closestPos === -1) {
                    for (var p = currentMatchPositions.length - 1; p >= 0; p--) {
                      if (currentMatchPositions[p] <= historyPos) {
        ...
        if (updateExists) {
          if (jsonFileContent.trim() !== "") {
            try {
              ...
              this.characterManager.contextHistory = oldManager.contextHistory ? oldManager.contextHistory
              .toString() : "";
              ...
              for (var i = 0; i < this.characterManager.characterRecords.length; i++) {
                ...
                if (record && record.voice) {
                  ...
                  this.characterManager.voiceUsageMap[voiceTag] = (this.characterManager.voiceUsageMap[voiceTag] ||
                    0) + 1;
                }
              }
      ...
      (function extractFayinrenPersonalityAuto() {
        ...
        } else if (duihuaRoles.length === 0) {} else if (duihuaPersonalities.length === 0) {}
        // =============================================================================
      ...
      for (var i = 0; i < text.length; i++) {
        ...
        if (tmpStr.indexOf("〖") !== -1 || tmpStr.indexOf("{{") !== -1 || tmpStr.indexOf("###PROTECTED_") !== -1)
          continue;
        if (tmpStr.indexOf("“") === 0) {
          ...
          if (match && match[1]) {
            ...
            for (var _di = 0; _di < allDialogues.length; _di++) {
              if (allDialogues[_di].id === dialogueId) {
                isIdDuplicate = true;
                break;
              }
            }
      ...
      for (var i = 0; i < text.length; i++) {
        ...
        for (var j = 0; j < allLocalSoundTags.length; j++) {
          ...
          var tagReg = new RegExp("(\\{\\{" + escapedTagName + "_([\\s\\S]*?)\\}\\}([\\s\\S]*?)\\{\\{" +
            escapedTagName + "结束\\}\\})", 'g');
        ...
        if (originalTextLine) {
          ...
          // ========== 情绪统一处理（手动/桥接/AI）==========
          if (ENABLE_EMOTION && originalItem.tag !== "narration") {
            var _emotionInfo = { emotion: "", scale: 4, source: "none" };
            ...
            // 2. 标签手动配置（桥接没有时生效）
            if (!_emotionInfo.emotion && tagsData) {
              var _tagCfg = tagsData[originalItem.tag];
              var _manualCn = extractEmotionValue(_tagCfg, ENABLE_EMOTION);
              if (_manualCn && _manualCn !== "无") {
                var _manualKey = EMOTION_NAME_TO_KEY[_manualCn];
                if (_manualKey) {
                  _emotionInfo.emotion = _manualKey;
                  _emotionInfo.source = "manual";
                }
              }
            }

            // 3. AI自动分析（桥接和手动都没有时才生效）
            if (!_emotionInfo.emotion && ENABLE_EMOTION_AI) {
              var _shouldAnalyze = originalItem.text.indexOf("“") !== -1;
              if (_shouldAnalyze) {
                logAiEmotionStart(originalItem.text);
                var _candidateKeys = [];
                for (var _ek in EMOTION_KEY_MAP) {
                  if (EMOTION_KEY_MAP.hasOwnProperty(_ek) && _ek !== "neutral") {
                    _candidateKeys.push(_ek);
                  }
                }
                var _analyzed = analyzeSentimentWithEmotionList(originalItem.text, "", _candidateKeys);
                if (_analyzed) {
                  _emotionInfo.emotion = _analyzed;
                  _emotionInfo.source = "ai";
                }
              }
            }

            if (_emotionInfo.emotion) {
              originalItem.emotion = _emotionInfo.emotion;
              originalItem.emotionScale = _emotionInfo.scale || 4;
              originalItem.emotionSource = _emotionInfo.source;
              // 清理文本中的桥接标记
              if (_emotionInfo.text && _emotionInfo.text !== originalItem.text) {
                originalItem.text = _emotionInfo.text;
              }
            }
          }
          // ========== 情绪统一处理 结束 ==========
      ...
      // 输出情绪模块开关状态（仅变化时输出）
      logEmotionStatus(ENABLE_EMOTION);
      // 输出处理结果总览
      for (var k = 0; k < list.length; k++) {
        var item = list[k];
        ...
        // 从最终tag中提取情绪（最后一个|后面的部分）

      }
      // 输出情绪信息
      logEmotionFromResults(list, ENABLE_EMOTION);
      return list;
    },
  ...
  // 输出情绪/备用模型初始状态（启动时仅显示一次，后续仅变化时再输出）
