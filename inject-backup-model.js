...
// ===================== 1. 读取源文件 =====================
const srcFile = '多角色朗读2.85发音人轮询+增强别名校验v77.json';
const dstFile = '多角色朗读2.86发音人轮询+增强别名校验v77+备用模型接力.json';
const data = JSON.parse(fs.readFileSync(srcFile, 'utf8'));
let code = data.code;

// ===================== 2. 更新元信息 =====================
data.name = '多角色朗读2.86发音人轮询+增强别名校验v77+备用模型接力';
data.version = 86;

// code内部的name和version
code = code.replace(
  /name: "多角色朗读2\.85【测试3\+3】重构别名"/,
  'name: "多角色朗读2.86【测试3+3】重构别名"'
);
code = code.replace(
  /version: 64,/,
  'version: 86,'
...
// ===================== 3. 注入 BackupModelManager + _lastBackupStatus + logBackupInitStatus =====================
// 插入位置：var roleToRootIdMap = {}; 之后，// ===================== 核心：双场景独立密钥轮换管理 之前

const backupManagerBlock = `
...
// ===================== 4. 在 DualKeyManager.loadKeyFile 中注入 #!# 备用模型解析 =====================
// 找到 pools.aliasAnalyze.list = parseSingleGroup(aliasContent); 之后的空行，插入备用模型解析

const loadKeyFileInjection = `          pools.nameAnalyze.list = parseSingleGroup(nameContent);
...
code = code.replace(
  /          pools\.nameAnalyze\.list = parseSingleGroup\(nameContent\);\n          pools\.aliasAnalyze\.list = parseSingleGroup\(aliasContent\);\n\n/,
  loadKeyFileInjection
);

// ===================== 5. 在 concurrentApiRequest 失败分支注入备用模型接力 =====================
// 替换最后的 } else { return { success: false, data: null, errors: errors }; }\n}
...
// 找到 printAvailableVoices(); 并在其后添加 logBackupInitStatus();
code = code.replace(
