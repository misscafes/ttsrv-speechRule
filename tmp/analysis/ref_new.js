...
function isNameMatch(record, targetName) {
    ...
    var allNames = uniqueArr(
        aliases.split("|").filter(function(s) { return s && s.trim(); }).concat([mainName])
    );
...
function handleBookSwitch() {
    ...
    var rawContent = "";
    try {
        rawContent = java.readExternalFile(charMainPath);
    } catch (err) {
        rawContent = "[]";
    }
    try {
    ...
    var newRaw = "[]";
    try {
        newRaw = java.readExternalFile(newBackupPath);
...
function readDialogCache() {
    try {
        ...
        var safeList = Array.isArray(raw.dialogList) ? raw.dialogList.filter(function(item) {
            return item && typeof item.dialogContent === "string";
        }) : [];
...
function matchDialogFromCache(currentDialogText) {
    ...
    function tryMatch(items, idx) {
        var lines = items.dialogContent.split("\n").filter(function(l) { return l.trim() !== ""; });
        for (var i = 0; i < lines.length; i++) {
            if (cleanDialogText(lines[i]) === cleanCurrent) {
                matchedResult = {
                    name: items.name || "未知",
    ...
    if (ci >= 0 && ci < dialogList.length) tryMatch(dialogList[ci], ci);
    if (!matchedResult) {
        for (var offset = 1; offset <= MAX_FORWARD_OFFSET; offset++) {
            var ti = currentIndex - 1 - offset;
            if (ti < 0) break;
            if (tryMatch(dialogList[ti], ti)) break;
        }
    }
    if (!matchedResult) {
...
function handleSpecialQuoteCases(originalText) {
    ...
    var finalGender = matchResult.gender;
    var finalAge = matchResult.age;
...
(function() {
    ...
    if (dialogs.length === 0) { saveCurrentToHistory(originalText); return text; }
