...
var shouci = 500;//   首次使用缓存字数，只在首次使用。
...
CharacterManager.prototype.analyzeCharacter = function(fullText, characterId, allDialogues, chapterFullContent) {
  ...
  var useChapterContent = chapterFullContent || fullText || "";
  var segments = buildNameAnalyzeSegments(useChapterContent, allDialogues);
  var fullBatchContent = "【01对话前上文原文（1500字）】\n" + segments.beforeContent + "\n\n【01对话至最后对话之间的全部原文（含旁白叙述）】\n" + segments.middleContent + "\n\n【最后对话后下文原文（1500字）】\n" + segments.afterContent;

  // 对话列表提取改用 middleContent，避免后文引号污染 dialogContent
  var extractSource = segments.middleContent || "";
  var dialogTextMap = {};
  var seqReg = /【(\d{2})】[\s\S]*?(["“])([\s\S]*?)(["”])/g;
  var seqMatch;
  while ((seqMatch = seqReg.exec(extractSource)) !== null) {
    var seq = seqMatch[1];
  ...
  var prompt = 
"你是一个严谨的对话归因分析师。请按以下逐步推理法判断每句对话的说话人：\n" +
"S1（通读全局）：先通读所有对话及其上下文，列出本段共出现几个人物，标注人物之间的关系（谁对谁说话、谁是上级/下属、谁是说话对象）。\n" +
"S2（匹配主名）：对每个编号对话，分析出的说话人名字如果在本地已存角色库中存在或与之有别名/昵称/称谓关系，请使用角色库中该角色的「主名」作为 name——这能保证同一人物在全文中的称呼一致。\n" +
"S3（精确定位提示词）：对每个编号对话，在上文中寻找明确的提示词——若出现「XX说/道/喊/吼/问/笑/开口/冷冷道/叹了口气道」等，则该XX就是直接说话人。\n" +
"S4（引号内外区分）：对话双引号“”内部的人名是被谈论对象，绝不能当作说话人——说话人一定在引号外面。\n" +
"S5（无提示词时溯源）：若前文无提示词，看对话交替模式——两人对话中通常轮流出现，三人以上需结合上下文、动作描述判断说话人。\n" +
"S6（排除错位）：每分析完一段，整体复查一遍，检查是否存在序号错位、同名不同人、或说话人连续多句无提示词导致的误判，并自行修正。\n" +
"S7（不确定就留空）：如果确实无法判断，不要乱填；name可以用上下文中的特征称呼（如“络腮胡大汉”），gender和age保持。\n\n" +
...
"4. 如果姓名不是太能确定，也就是说没有明确说明，先空着，不要产生幻觉。\n" +
"**【输出要求】**\n" +
...
"\n\n";
  ...
  function buildNameAnalyzeRequest(apiConfig) {
    var allRolesList = characterManager.getAllCharacterNamesAndAliases(null);
    var rolesInfo = allRolesList && allRolesList.length > 0
    ...
    var requestData = {
      ...
      messages: [
        ...
        { role: 'user', content: rolesInfo + "⚠️ 请对照上方角色库：如果说话人与库中某角色是同一人（包括别名/昵称/称谓），请使用该角色的「主名」作为 name，不要另起新名。如果确实是新角色，才能使用对话中出现的具体称呼。\n\n" + "【上文历史内容】\n" + this.contextHistory2 + "\n【当前待分析对话内容】\n" + fullBatchContent }
      ],
      thinking_mode: false,
  ...
  for (var seqKey in batchResult) {
    if (batchResult.hasOwnProperty(seqKey)) {
      var name = batchResult[seqKey].name;
...
CharacterManager.prototype.processCharacter = function (fullText, characterId, allDialogues, chapterFullContent) {
  var analysis = this.analyzeCharacter(fullText, characterId, allDialogues, chapterFullContent);
  if (!analysis) {
    return null;
  }
  ...
  if (bieming !== 0 && newCharacterName !== "未知" && Object.keys(pendingAliasNames).length > 0 && pendingAliasContext) {
    ...
    var contextText = ctx.chapterFullContent ? String(ctx.chapterFullContent).slice(0, 8000) : ((ctx.contextHistory2 || '') + '\n' + (ctx.text2 || '') + '\n' + (ctx.next100Chars || ''));
    var nameSeqText = '';
    for (var i = 0; i < namesToCheck.length; i++) {
      nameSeqText += '【' + padZero(i + 1, 2) + '】' + namesToCheck[i] + '\n';
    }
  ...
  var batchPrompt = "你是专业的小说人物别名识别AI。请首先通读下方【整章小说原文】全文，理解每个人物的互动关系（谁对谁说话、谁的动作描述、旁白如何指代人物），" +
  "再逐一判断【待分析名字列表】中每个名字是否为【本地已存角色列表】中某个角色的别名。\n" +
  "【核心原则】\n" +
  "1. 别名是同一人物的不同称呼，必须能在原文中找到明确证据（如：张三被称为「小张」，王处长同时被称作「王建国」）\n" +
  "2. 不要仅凭字面相似度（同姓、相近字数）推断为别名，必须有上下文支撑\n" +
  "3. 同一个名字在原文中可能指代不同人物→不是别名\n" +
  "4. 泛指称谓（如「兄弟」「大哥」）能明确归属某角色时才是别名，否则不是\n" +
  "5. 不确定的默认判定为非别名\n\n" +
  promptPart +
  ...
  "【待分析名字列表】\n" + nameSeqText + "\n\n" +
  "【输出要求】\n" +
  "输出一个JSON，键为序号（如\"01\"），值为 {isAlias: true/false, mainName: \"匹配主名或\", reason: \"(置信度百分比)或\"}。\n" +
...
// 清洗上下文文本，防止中文引号／对话序号被正则捕获，污染 dialogContent
function sanitizeContextText(text) {
    if (!text) return "";
    return text
        .replace(/【\d{1,2}】/g, "〖$&〗")  // 【01】→ 〖【01】〗避免被 seqReg 捕获
        .replace(/“/g, "＜")                // 左引号转义
        .replace(/”/g, "＞");              // 右引号转义
}

// ===================== 新增：姓名分析三段原文上下文构建函数 =====================
// 从章节全文中提取前后文，中间内容复用generateBatchSeqContent保证批量分析
function buildNameAnalyzeSegments(chapterFullContent, allDialogues) {
    var CHAPTER_BEFORE_CHARS = 1500;
    var CHAPTER_AFTER_CHARS = 1500;
    var emptySegments = { beforeContent: "", middleContent: "", afterContent: "" };

    if (!allDialogues || allDialogues.length === 0) {
        return emptySegments;
    }

    // 中间内容：复用已验证的generateBatchSeqContent，确保所有对话带序号
    var middleContent = generateBatchSeqContent(allDialogues, next100Chars);

    // 前后文：从章节原文中按【数字】模式定位
    var chapText = chapterFullContent || "";
    var firstNumMatch = chapText.match(/【\d{2}】/);
    var firstPos = firstNumMatch ? firstNumMatch.index : 0;

    // 找最后一个【数字】位置
    var lastPattern = /【\d{2}】/g;
    var lastMatch, lastPos = firstPos, lastMatchLen = 0;
    while ((lastMatch = lastPattern.exec(chapText)) !== null) {
        lastPos = lastMatch.index;
        lastMatchLen = lastMatch[0].length;
    }
    var lastEnd = lastPos + lastMatchLen;
    var nlAfter = chapText.indexOf("\n", lastEnd);
    if (nlAfter !== -1) lastEnd = nlAfter;
    else lastEnd = chapText.length;

    // ① 第一条对话往上1500字（清除【数字】和中文引号，避免被正则误匹配）
    var beforeStart = Math.max(0, firstPos - CHAPTER_BEFORE_CHARS);
    var beforeContent = chapText.substring(beforeStart, firstPos);
    beforeContent = sanitizeContextText(beforeContent);

    // ② 中间内容（generateBatchSeqContent已处理，直接使用）
    // middleContent 已赋值

    // ③ 最后一条对话往后1500字（清除【数字】和中文引号，避免被正则误匹配）
    var afterEnd = Math.min(chapText.length, lastEnd + CHAPTER_AFTER_CHARS);
    var afterContent = chapText.substring(lastEnd, afterEnd);
    afterContent = sanitizeContextText(afterContent);
