package ct;

import jx.w;
import ry.z;
import yx.p;

/* JADX INFO: compiled from: r8-map-id-47d7031f5d78718531f48f1c7640d5b20aa4ef3796b143da36b974009fbbf949 */
/* JADX INFO: loaded from: classes3.dex */
public final class j extends qx.i implements p {
    public final /* synthetic */ n X;

    /* JADX INFO: renamed from: i, reason: collision with root package name */
    public final /* synthetic */ int f5245i;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public /* synthetic */ j(n nVar, ox.c cVar, int i10) {
        super(2, cVar);
        this.f5245i = i10;
        this.X = nVar;
    }

    @Override // qx.a
    public final ox.c create(Object obj, ox.c cVar) {
        int i10 = this.f5245i;
        n nVar = this.X;
        switch (i10) {
            case 0:
                return new j(nVar, cVar, 0);
            default:
                return new j(nVar, cVar, 1);
        }
    }

    @Override // yx.p
    public final Object invoke(Object obj, Object obj2) {
        int i10 = this.f5245i;
        w wVar = w.f15819a;
        z zVar = (z) obj;
        ox.c cVar = (ox.c) obj2;
        switch (i10) {
            case 0:
                ((j) create(zVar, cVar)).invokeSuspend(wVar);
                break;
            default:
                ((j) create(zVar, cVar)).invokeSuspend(wVar);
                break;
        }
        return wVar;
    }

    @Override // qx.a
    public final Object invokeSuspend(Object obj) {
        int i10 = this.f5245i;
        w wVar = w.f15819a;
        n nVar = this.X;
        switch (i10) {
            case 0:
                lb.w.j0(obj);
                jx.l lVar = qq.g.f25386a;
                qq.g.e(nVar.f5255u0, nVar.f5254t0);
                break;
            default:
                lb.w.j0(obj);
                jx.l lVar2 = qq.g.f25386a;
                qq.g.f(nVar.f5255u0, nVar.f5254t0);
                break;
        }
        return wVar;
    }
}
