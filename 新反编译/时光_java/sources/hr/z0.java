package hr;

import io.legado.app.ui.book.read.page.entities.TextChapter;

/* JADX INFO: compiled from: r8-map-id-47d7031f5d78718531f48f1c7640d5b20aa4ef3796b143da36b974009fbbf949 */
/* JADX INFO: loaded from: classes3.dex */
public final class z0 extends qx.i implements yx.p {
    public final /* synthetic */ TextChapter X;

    /* JADX INFO: renamed from: i, reason: collision with root package name */
    public final /* synthetic */ int f12968i;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public /* synthetic */ z0(TextChapter textChapter, ox.c cVar, int i10) {
        super(2, cVar);
        this.f12968i = i10;
        this.X = textChapter;
    }

    @Override // qx.a
    public final ox.c create(Object obj, ox.c cVar) {
        int i10 = this.f12968i;
        TextChapter textChapter = this.X;
        switch (i10) {
            case 0:
                return new z0(textChapter, cVar, 0);
            case 1:
                return new z0(textChapter, cVar, 1);
            default:
                return new z0(textChapter, cVar, 2);
        }
    }

    @Override // yx.p
    public final Object invoke(Object obj, Object obj2) {
        int i10 = this.f12968i;
        jx.w wVar = jx.w.f15819a;
        ry.z zVar = (ry.z) obj;
        ox.c cVar = (ox.c) obj2;
        switch (i10) {
            case 0:
                ((z0) create(zVar, cVar)).invokeSuspend(wVar);
                break;
            case 1:
                ((z0) create(zVar, cVar)).invokeSuspend(wVar);
                break;
            default:
                ((z0) create(zVar, cVar)).invokeSuspend(wVar);
                break;
        }
        return wVar;
    }

    @Override // qx.a
    public final Object invokeSuspend(Object obj) {
        int i10 = this.f12968i;
        jx.w wVar = jx.w.f15819a;
        TextChapter textChapter = this.X;
        switch (i10) {
            case 0:
                lb.w.j0(obj);
                j1.X.getClass();
                j1.f12832v0 = textChapter;
                break;
            case 1:
                lb.w.j0(obj);
                j1.X.getClass();
                j1.f12831u0 = textChapter;
                break;
            default:
                lb.w.j0(obj);
                j1.X.getClass();
                j1.f12833w0 = textChapter;
                break;
        }
        return wVar;
    }
}
