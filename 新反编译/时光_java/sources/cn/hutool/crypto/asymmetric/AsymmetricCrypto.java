...
public class AsymmetricCrypto extends AbstractAsymmetricCrypto<AsymmetricCrypto> {
    ...
    private byte[] doFinal(byte[] bArr, int i10) throws BadPaddingException, IllegalBlockSizeException, IOException {
        int length = bArr.length;
        return length <= i10 ? getCipher().doFinal(bArr, 0, length) : doFinalWithBlock(bArr, i10);
    }

    private byte[] doFinalWithBlock(byte[] bArr, int i10) throws BadPaddingException, IllegalBlockSizeException, IOException {
        int length = bArr.length;
        b bVar = new b();
        int i11 = 0;
        int i12 = length;
        while (i12 > 0) {
            ...
            bVar.write(getCipher().doFinal(bArr, i11, iMin));
            i11 += iMin;
            i12 = length - i11;
        }
        return bVar.h();
    ...
    private Cipher initMode(int i10, Key key) throws InvalidKeyException, InvalidAlgorithmParameterException {
