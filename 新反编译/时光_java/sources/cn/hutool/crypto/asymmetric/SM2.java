...
public class SM2 extends AbstractAsymmetricCrypto<SM2> {
    ...
    /* JADX INFO: renamed from: cn.hutool.crypto.asymmetric.SM2$1, reason: invalid class name */
    /* JADX INFO: compiled from: r8-map-id-47d7031f5d78718531f48f1c7640d5b20aa4ef3796b143da36b974009fbbf949 */
    ...
    private CipherParameters getCipherParameters(KeyType keyType) throws Throwable {
        int i10 = AnonymousClass1.$SwitchMap$cn$hutool$crypto$asymmetric$KeyType[keyType.ordinal()];
        if (i10 == 1) {
            d.N(this.publicKeyParams, "PublicKey must be not null !", new Object[0]);
            return this.publicKeyParams;
        }
        if (i10 != 2) {
            return null;
        }
        d.N(this.privateKeyParams, "PrivateKey must be not null !", new Object[0]);
        return this.privateKeyParams;
    }

    private SM2Engine getEngine() throws Throwable {
        if (this.engine == null) {
            d.N(this.digest, "digest must be not null !", new Object[0]);
    ...
    private SM2Signer getSigner() throws Throwable {
        if (this.signer == null) {
            d.N(this.digest, "digest must be not null !", new Object[0]);
    ...
    public byte[] decrypt(byte[] bArr, CipherParameters cipherParameters) throws Throwable {
        this.lock.lock();
        ...
        try {
            try {
                ...
                throw new CryptoException((Throwable) e11);
            }
        } finally {
            this.lock.unlock();
        }
    ...
    public byte[] encrypt(byte[] bArr, CipherParameters cipherParameters) throws Throwable {
        this.lock.lock();
        SM2Engine engine = getEngine();
        try {
            try {
    ...
    public byte[] getQ(boolean z11) {
        return this.publicKeyParams.getQ().getEncoded(z11);
    }
    ...
    public byte[] sign(byte[] bArr, byte[] bArr2) throws Throwable {
        this.lock.lock();
        ...
        try {
            ...
            } catch (org.bouncycastle.crypto.CryptoException e11) {
                throw new CryptoException((Throwable) e11);
            }
        } catch (Throwable th2) {
            this.lock.unlock();
    ...
    public boolean verify(byte[] bArr, byte[] bArr2, byte[] bArr3) throws Throwable {
        this.lock.lock();
        SM2Signer signer = getSigner();
        try {
            ...
            signer.update(bArr, 0, bArr.length);
            boolean zVerifySignature = signer.verifySignature(bArr2);
            this.lock.unlock();
    ...
    public byte[] decrypt(byte[] bArr, KeyType keyType) throws CryptoException {
        ...
        c.z("Decrypt is only support by private key");
        return null;
    }
    ...
    public byte[] encrypt(byte[] bArr, KeyType keyType) throws CryptoException {
        if (KeyType.PublicKey == keyType) {
            return encrypt(bArr, (CipherParameters) new ParametersWithRandom(getCipherParameters(keyType)));
        }
        c.z("Encrypt is only support by public key");
        return null;
    }
