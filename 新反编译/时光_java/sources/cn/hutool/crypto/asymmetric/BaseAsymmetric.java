...
public class BaseAsymmetric<T extends BaseAsymmetric<T>> implements Serializable {
    ...
    public Key getKeyByType(KeyType keyType) {
        ...
        if (i10 == 1) {
            ...
            a.v("Private key must not null when use it !");
            return null;
        }
        if (i10 != 2) {
        ...
        a.v("Public key must not null when use it !");
        return null;
    }
    ...
    public T setKey(Key key) throws Throwable {
        d.N(key, "key must be not null !", new Object[0]);
