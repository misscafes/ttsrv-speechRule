package xn;

import android.content.Context;
import vp.q0;

/* JADX INFO: compiled from: r8-map-id-05bfbbe9086a2edb9eee68032a6875ae8b29a17573f56b596f68c5a5f5b16892 */
/* JADX INFO: loaded from: classes2.dex */
public final class t extends cr.i implements lr.p {

    /* JADX INFO: renamed from: i, reason: collision with root package name */
    public final /* synthetic */ int f28275i;

    /* JADX INFO: renamed from: v, reason: collision with root package name */
    public final /* synthetic */ x f28276v;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public /* synthetic */ t(x xVar, ar.d dVar, int i10) {
        super(2, dVar);
        this.f28275i = i10;
        this.f28276v = xVar;
    }

    @Override // cr.a
    public final ar.d create(Object obj, ar.d dVar) {
        switch (this.f28275i) {
            case 0:
                return new t(this.f28276v, dVar, 0);
            case 1:
                return new t(this.f28276v, dVar, 1);
            default:
                return new t(this.f28276v, dVar, 2);
        }
    }

    @Override // lr.p
    public final Object invoke(Object obj, Object obj2) {
        wr.w wVar = (wr.w) obj;
        ar.d dVar = (ar.d) obj2;
        switch (this.f28275i) {
            case 0:
                t tVar = (t) create(wVar, dVar);
                vq.q qVar = vq.q.f26327a;
                tVar.invokeSuspend(qVar);
                return qVar;
            case 1:
                t tVar2 = (t) create(wVar, dVar);
                vq.q qVar2 = vq.q.f26327a;
                tVar2.invokeSuspend(qVar2);
                return qVar2;
            default:
                t tVar3 = (t) create(wVar, dVar);
                vq.q qVar3 = vq.q.f26327a;
                tVar3.invokeSuspend(qVar3);
                return qVar3;
        }
    }

    @Override // cr.a
    public final Object invokeSuspend(Object obj) {
        int i10 = this.f28275i;
        vq.q qVar = vq.q.f26327a;
        x xVar = this.f28276v;
        switch (i10) {
            case 0:
                br.a aVar = br.a.f2655i;
                l3.c.F(obj);
                Context contextN = xVar.n();
                if (contextN != null) {
                    q0.H(contextN, "该章节没有缓存的音频文件\n请先使用朗读功能播放该章节");
                }
                break;
            case 1:
                br.a aVar2 = br.a.f2655i;
                l3.c.F(obj);
                Context contextN2 = xVar.n();
                if (contextN2 != null) {
                    q0.H(contextN2, "无法获取章节内容");
                }
                break;
            default:
                br.a aVar3 = br.a.f2655i;
                l3.c.F(obj);
                sr.c[] cVarArr = x.f28294i1;
                xVar.n0().j(0, xVar.n0().c(), Boolean.TRUE);
                break;
        }
        return qVar;
    }
}
