...
- **中文输出检查**：每次打开 Kimi 后，必须第一时间检测并确保所有输出为中文，包括内部思考、工具调用说明、代码注释、文件内容等。如发现非中文输出，立即切换为中文。
...
- 2.130 修复尝试：同时读取 `data.json` 和 `cunfang.txt`，优先使用非 `"未知书名"` / `"default_book"` 的真实书名；若 `data.json` 无效但 `cunfang.txt` 有真实书名，则拒绝切回；仅当明确检测到新的真实书名时才执行换书流程。
- 2.130 问题：重写切书段时不慎遗留多余大括号，导致 Rhino 报错 `try 被检测到没有 catch 或 finally`。
- 2.131 修复：删除多余大括号，恢复正确 try-catch 嵌套；使用 `node --check` 验证 JS 无语法错误。保留 2.130 的切书逻辑修复。
- 待用户安装 2.131 后验证是否彻底解决。
- **注意**：2.130 存在语法错误，请勿使用。
