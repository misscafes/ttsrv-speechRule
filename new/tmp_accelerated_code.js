...
"\n\n";
  ...
  // ✅ 保留之前的修改：不写入dialog_cache.json，不覆盖外部脚本结果
...
CharacterManager.prototype.processCharacter = function (fullText, characterId, allDialogues, chapterFullContent) {
  ...
  // ✅ 核心修改：单名字实时别名分析（缓存/API结果统一触发）
  if (bieming !== 0 && newCharacterName !== "未知") {
    // 立即检查本地角色库，不存在则实时调用别名分析
    if (!this.findCharacterRecord(newCharacterName)) {
      console.log("【🔵🔵🔵 触发单别名分析】发现新名字【" + newCharacterName + "】，开始校验");
      // 传入完整上下文（和原批量逻辑完全一致）
      this.singleAliasAnalyze(
        newCharacterName,
        this.contextHistory2,
        text2,
        next100Chars
    ...
    // 分析完成后重新查询（可能已合并别名）
    targetMainRecord = this.findCharacterRecord(newCharacterName);
  }

  // 以下原有逻辑100%保留（发音人分配、角色管理等）
  if (!targetMainRecord) {
...
// 新增：单个名字实时别名分析函数（复用原API/解析/投票逻辑，不攒批、实时触发）
CharacterManager.prototype.singleAliasAnalyze = function(targetName, contextHistory, currentText, nextChars) {
  if (!targetName || targetName.trim() === "" || targetName === "未知") {
    return null;
  }
  targetName = targetName.trim();
  
  // 前置检查：本地已存在该角色，直接跳过
  if (this.findCharacterRecord(targetName)) {
    console.log("【🔵🔵🔵 单别名分析】跳过：本地已存在角色【" + targetName + "】");
    return null;
  }
  ...
  var contextText = (contextHistory || '') + '\n' + (currentText || '') + '\n' + (nextChars || '');
  var nameSeqText = '【01】' + targetName + '\n';

  // 生成单名字专属Prompt（保留原严谨/宽松模式）
  ...
  var singlePrompt = "你是专业的小说人物别名识别AI。请基于小说上下文和角色关系，判断【待分析名字】是否为【本地已存角色列表】中某个角色的别名。\n" +
    promptPart +
    
    
    "别名类型包括：字号/表字、绰号/外号、身份称谓、关系称谓、谐音/简称、笔名/化名。排除同名不同人、临时NPC、全新角色、未明确指向的泛指称谓。如果两个名字里只有一个字不同，要结合上下文分析，两个名字是亲戚亲属关系的可能性更大，不要轻易判断是作者笔误。\n" +
    "【本地已存角色列表】\n" + JSON.stringify(allRolesList) + "\n\n" +
    "【小说上下文】\n" + contextText + "\n\n" +
    "【待分析名字】\n" + nameSeqText + "\n\n" +
    "【输出要求】\n" +
    "输出严格JSON格式：{\"01\": {isAlias: true/false, mainName: \"匹配主名或null\", reason: \"置信度百分比或null\"}}。\n" +
    "必须覆盖序号01，优先根据上下文语境判断，不要仅基于字面相似度。";

  // 请求构建函数（100%复用原批量逻辑）
  function buildSingleRequest(apiConfig) {
    var requestData = {
      model: apiConfig.model,
      messages: [
  ...
  while (currentRound < maxRetryRound && !finalResult) {
    ...
    if (accumulatedCount.get() > 0) {
      if (needWaitMultiResult && accumulatedResults.length > 0) {
        var nameMap = { "01": targetName };
        var voteResult = voteBatchAliasAnalyzeResult(accumulatedResults, nameMap);
        finalResult = voteResult ? voteResult["01"] : null;
      } else if (accumulatedResults.length > 0) {
        finalResult = accumulatedResults[0].data["01"];
      }
      if (finalResult) break;
    }

    if (!finalResult && currentRound < maxRetryRound) {
      sleep(250);
    }
  }

  // 结果处理：立即写入本地角色库（仅修改characterRecords.json）
  if (finalResult && finalResult.isAlias && finalResult.mainName) {
    var mainRecord = this.findCharacterRecord(finalResult.mainName);
    if (mainRecord) {
      var aliases = mainRecord.aliases ? mainRecord.aliases.split("|").map(function(a) { return a.trim(); }) : [];
      if (aliases.indexOf(targetName) === -1) {
        aliases.push(targetName);
        mainRecord.aliases = aliases.join("|");
        this.saveRecords();
        console.log("【🔵🔵🔵✅ 单别名分析成功】【" + targetName + "】已合并到主角色【" + finalResult.mainName + "】");
      }
    }
  } else {
    console.log("【🔵🔵🔵 单别名分析完成】【" + targetName + "】判定为新角色，后续自动分配发音人");
  }
...
function generateBatchSeqContent(dialoguesList, nextContent) {
  ...
  // 2. ✅ 下文仅做基础清理，不参与引号修正和序号添加（仅作为API上下文）
  var cleanedNextContent = (nextContent || "")
      .replace(/【\d+】/g, "") // 移除下文原有旧序号
      .replace(/(.[\u4e00-\u9fa5]+音效.)/g, "") // 清除下文音效
      .replace(/[『「【〈〉〔‘’〕】」』]/g, ""); // 清除下文特殊括号
  
  // ===================== 第二步：仅对当前对话执行引号修正 =====================
  var processedDialogues = cleanedDialogues
      .replace(/(.[\u4e00-\u9fa5]+音效.)/g, "") // 清除当前对话音效
      .replace(/【\d\d?】/g, "") // 再次清理当前对话残留序号
      .replace(/(”[^“”]*\n)([^“”\n]+”)/g, "$1“$2") // 补换行后缺失的左引号
      .replace(/(“[^“”\n]+)(\n[^“”]*“)/g, "$1”$2") // 补换行前缺失的右引号
      .replace(/[『「【〈〉〔‘’〕】」』]/g, ""); // 清除当前对话特殊括号
  
  // ===================== 第三步：仅基于当前对话计算添加序号的数量 =====================
  var allLeftQuotes = processedDialogues.match(/“/g);
  var totalQuoteCount = allLeftQuotes ? allLeftQuotes.length : 0;
  var stopAddIndex;
  // 原比例逻辑完全保留：总引号数≤5全部加，超过5按SEQ_ADD_RATIO计算
  ...
  // ===================== 第四步：仅给当前对话添加序号 =====================
  var dialoguesWithSeq = processedDialogues.replace(/“/g, function (match) {
      seqCounter++;
  ...
  // ===================== 第五步：拼接最终结果（下文无任何序号） =====================
  return dialoguesWithSeq + cleanedNextContent;
}
   ...
   //   //console.log("\n" + AUDIO_CONFIG.logPrefix + "===== 解析yinxiao.json中的%标记（开头%提内容，结尾%提网址） =====");
      ...
      for (var i = 0; i < text.length; i++) {
          ...
          if (originalTextLine) {
              ...
              // ========== 新增：手动发音人标注优先匹配（最高优先级） ==========
              // 匹配格式："<<发音人标签>>对话内容" 或 "【序号】<<发音人标签>>对话内容"
              var manualVoiceMatch = restoredText.match(/^“(【\d+】)?<<([^>]+)>>/);
              if (manualVoiceMatch && manualVoiceMatch[2]) {
                var manualVoiceTag = manualVoiceMatch[2].trim();
                // 移除标注和序号，保留纯对话内容
                var cleanText = restoredText.replace(/^“(【\d+】)?<<[^>]+>>/, "“");
                // 验证发音人是否在可用列表中
                var isVoiceAvailable = this.characterManager.isVoiceAvailable(manualVoiceTag);
                
                if (isVoiceAvailable) {
                  // 兼容duihua动态发音人（带id的自定义角色）
                  if (roleToRootIdMap.hasOwnProperty(manualVoiceTag)) {
                    var rootId = roleToRootIdMap[manualVoiceTag] || "0";
                    originalItem = { 
                      text: cleanText.replace(/^(“?)【\d+】/, "$1").toString(), 
                      tag: "duihua", 
                      id: rootId 
                    };
                  } else {
