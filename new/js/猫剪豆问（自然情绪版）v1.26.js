...
// 猫剪豆问引擎 v1.22（自然情绪版）：融合猫箱-VV大军"完全版+4"跳段修复策略，重试时递增追加句号并增加强化标点补救阶段；保留v1.21的跨段对话、长文本切分、协议异常等待、最终抢救重试、纯标点段过滤等能力
try {
...
for (var i = 0; i < segments.length; i++) {
    ...
    for (var attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        // v1.22 优化：融合猫箱-VV大军策略，首次重试清理末尾不规范标点后补一个句号，后续递增追加句号，提高服务端接受概率
    ...
    if (audio && audio.length > 0) {
        ...
        // v1.22 新增：最终长等待抢救前，先用强化标点（多个句号）快速补救一次（参考猫箱-VV大军"追加句号"策略的强化版）
