// ===================== 朗读脚本 v1.26（自然情绪版）：配合 CNB 混元3 接口，融合猫箱-VV大军"完全版+4"跳段修复策略；「」内容改由旁白发音（去除「」括号作为旁白文本）、不分配括号3独立发音人（章节标题仍走括号3）；修复切回旧书角色丢失（角色备份改合并写入，防止主记录被污染时覆盖丢失；saveCurrentBookName 加 default_book 保护） =====================
