// ===================== 朗读脚本 v1.16（自然情绪版）：诊断版，简化日志以排查换书问题：统一日志写入外部文件，方便无日志APK排查 =====================
...
function getBookNameSafely() {
    ...
    log("【书名】data.json 内容长度=" + dataStr.length + "，解析书名=" + bookName);
    // 若 data.json 中没有书名，回退到 cunfang.txt 中保存的当前书
    if (!bookName) {
        try {
            var saved = readCurrentBookName();
            if (saved) bookName = saved;  // v1.12 修复：不再排除 default_book，避免生成随机书名
            log("【书名】data.json 无书名，回退到 cunfang.txt：" + bookName);
        } catch (e) {}
    }
    if (!bookName) bookName = "default_book";  // v1.12 修复：使用固定默认书名
    log("【书名】最终书名=" + bookName);
