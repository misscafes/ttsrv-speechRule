...
// ❌ 不再使用 var ROLE_LIST_FILE = EXT_DIR + "jiaoseliebiao-list.json";
...
// ⚡️ 修改点：从 fayinren.json 读取可用发音人，不再依赖 jiaoseliebiao-list.json
var enabledTags = {};
try {
    var fayinrenContent = java.readExternalFile(EXT_DIR + "fayinren.json");
    if (fayinrenContent && fayinrenContent.trim() !== "") {
        var fayinrenList = JSON.parse(fayinrenContent);
        if (isArray(fayinrenList)) {
            for (var i = 0; i < fayinrenList.length; i++) {
                enabledTags[fayinrenList[i]] = true;
            }
        } else {
            // 文件内容格式不对，回退启用全部硬编码发音人
...
// ⚡️ 修改点：不再自动生成 fayinren.json（删除原有保存代码）
...
function updateCharacterRecords(dialogList, chapterContext) {
    ...
    for (var i = 0; i < MAX_RETRY; i++) {
        ...
        var aliasBody = JSON.stringify({
            model: cfg.model, 
            messages: [{ role: "system", content: "严格遵守格式要求，仅输出JSON" }, { role: "user", content: aliasPrompt }],
            temperature: 0.1, 
            thinking_mode: false, 
            thinking: { type: "disabled" }, 
            disable_think: true, 
