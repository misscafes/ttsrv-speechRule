...
// ===================== v2.136 新增：零成本 API 源配置 =====================
// API_SOURCE 可选值：
//   "official"     - 走 miyue.txt 中的智谱官方/自定义接口（默认，与原逻辑一致）
//   "cnb_guest"    - 走 CNB 智谱游客免登录接口（需 App 支持读取响应头）
//   "zhipu_guest"  - 走智谱清言 App 游客模式（完全免 key，每次自动生成 deviceId/guest token）
var API_SOURCE = "official";
var CNB_COOKIE_FILE = "cnb_cookie.json";   // CNB csrfkey 文件
var CNB_TOKEN_FILE = "cnb_token.json";     // CNB csrftoken 文件
...
var DualKeyManager = (function() {
  ...
  function getAvailableApiList(scene, needCount) {
      // v2.136：CNB 智谱游客免登录模式
      if (API_SOURCE === "cnb_guest") {
          var cnt = needCount && needCount > 0 ? needCount : MIN_CONCURRENT_COUNT;
          var arr = [];
          for (var i = 0; i < cnt; i++) {
              arr.push({
  ...
  function createSingleRequestTask(apiConfig) {
    return function() {
      ...
      try {
        ...
        // v2.136：CNB 智谱游客模式，替换请求头为 Cookie + Csrftoken
        if (apiConfig.isCnb === true) {
            var cnbToken = getCnbCsrfToken();
            ...
            if (cnbToken) requestParams.headers["Csrftoken"] = cnbToken;
            requestParams.endpoint = CNB_CONFIG.baseUrl + CNB_CONFIG.apiPath;
            requestParams.data.model = CNB_CONFIG.model;
        ...
        if ((apiConfig.isCnb === true || apiConfig.isZhipuGuest === true) && response && response.body) {
            ...
            if (apiConfig.isCnb === true && rawBody.indexOf("data:") !== -1) {
                ...
                for (var cnbI = 0; cnbI < lines.length; cnbI++) {
                    var cnbLine = lines[cnbI].trim();
                    if (cnbLine.indexOf("data:") !== 0) continue;
                    var cnbData = cnbLine.substring(5).trim();
                    if (cnbData === "[DONE]" || cnbData === "") continue;
                    try {
                        var cnbChunk = JSON.parse(cnbData);
                        var cnbDelta = cnbChunk.choices && cnbChunk.choices[0] && cnbChunk.choices[0].delta;
                        if (cnbDelta && cnbDelta.content) cnbContentParts.push(cnbDelta.content);
                    } catch (cnbE) {}
                }
                var cnbFullContent = cnbContentParts.join("");
...
var SpeechRuleJS = {
  name: "多角色朗读2.135【修复对话前旁白分配错误+连续对话分配】",
  id: "mingwuyan",
  author: "命無言、萌新改",
  version: 135,
  zdfp: 1,
  ...
  handleText: function(text, tagsData) {
    try {
      ...
      // 新增【v2.135】：半角双引号统一转为中文双引号，提前到短对话去引号之前，
      // 避免半角双引号短对话被后续 "..." 去引号正则误去掉引号，导致无法识别为对话。
 ...
 //     text = text.replace(/(^|”)([^a-zA-Z0-9\u4e00-\u9fa5\n]+)($)/g, "$1（时间转场音效）$3");
      ...
      if (soundKeywords.length > 0 && text.includes("“")) {
          ...
          text = text.replace(quotedReg, function(match) {
              ...
              // 一旦插入 \n，fx 分割时的 /(["“])(.*?)(["”])/g 无法跨行匹配，整段对话会被拆散并误标为旁白。
              // 双引号外的独立场景仍走下方的独立匹配逻辑，不受影响。
...
input = input.replace(/"([^"]*)"/g, function(match, content) {
    return "“" + content + "”";
      ...
      input = input.replace(/〖(.*?)〗/g, "\n〖括号2】$1\n").toString();
      input = input.replace(/「(.*?)」/g, "$1").toString();
      input = input.replace(/『(.*?)』/g, "\n『括号4】$1\n").toString();
