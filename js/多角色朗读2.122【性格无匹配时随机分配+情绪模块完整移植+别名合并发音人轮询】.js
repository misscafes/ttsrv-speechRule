...
CharacterManager.prototype.selectVoiceByGlobalRandom = function(candidates, label, personality) {
  ...
  if (personality && tagsData) {
      for (var idx = 0; idx < candidates.length; idx++) {
          ...
          var vp = (td && td.personality) ? String(td.personality).trim() : "";
          if (vp) {
  ...
  candidates.sort(function(a, b) {
      ...
      if (personality && tagsData) {
          ...
          if (tdA && tdA.personality) vpA = String(tdA.personality).trim();
          if (tdB && tdB.personality) vpB = String(tdB.personality).trim();
          if (vpA) {
