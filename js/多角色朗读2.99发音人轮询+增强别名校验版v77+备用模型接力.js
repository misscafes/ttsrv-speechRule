...
" + text2).slice(-CONFIG.contextHistoryLength);
  ...
  // 【v2.99新增】持久化1500字上文缓存到文件，App重启后不丢失
  try {
    var existingContext = "";
    try {
      existingContext = String(ttsrv.readTxtFile("context_1500.txt")).toString();
    } catch (e) {}
...
CharacterManager.prototype.analyzeCharacter = function(fullText, characterId, allDialogues) {
  ...
  // 【v2.99新增】优先读取持久化的1500字上文缓存，App重启后仍能利用历史语境
  try {
...
// 普通关键词：先处理重复，再处理单个（保留你原有注释）
      ...
      // -------------------------- 书籍切换与角色备份（v2.99：data.json直连+智能备份+持久化上文） --------------------------
      try {
          if (text.includes("“")) {
              var dataJsonContent = "";
              try {
                  ...
                  var cunfangReadSuccess = false;
                      ...
                      if (cunfangReadSuccess && oldBookName !== newBookName) {
                          ...
                          //    next100Chars = "";
                              ...
                              // 【v2.99新增】换书时清空持久化上文缓存，避免新书受到旧书上下文干扰
                              try {
