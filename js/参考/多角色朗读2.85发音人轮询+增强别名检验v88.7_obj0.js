...
var GRAPH_CONFLICT_VERIFY_MAX_PER_CHAPTER = 0; // v887：单章冲突校验不再限次，仅保留同证据去重
...
function CharacterManager() {
  ...
  // v887延续：mergedRecords 等 setAliasGraphBook 拿到 bookKey 后再读取，避免默认 default 污染日志。
}
...
var GRAPH_RULE_SOURCE = "tts-rule-2.85-v887";
var graphCurrentBookUrl = "";
var graphCurrentChapterIndex = "";
var graphCurrentChapterKey = "";

var graphAliasRecentChapters = []; // v81：别名校验/姓名分析用最近章节索引
...
function graphEventName(type) {
  var map = {
    ...
    "alias_check_queue_completed": "新名字别名校验队列完成",
    "speaker_cache_hit": "说话人缓存命中",
    ...
    "name_semantic_shape_passed": "姓名语义字段通过",
    ...
    "model_relation_audit_apply": "模型证据审计应用",
...
function graphSamePersonEvidenceBlockReason(a, b, evidence) {
  ...
  evidence = graphSafeString(evidence || "", 300);
  if (!evidence) return "";
  // v887延续：不再因为“共同/互动/附身/假冒”等泛词把模型same_person转反向；复杂证据交给暂停正边与冲突校验处理。
  return "";
}
...
// ===================== v887延续：direct-pair evidence gate（正向写入门槛，不做反向判死）=====================
...
CharacterManager.prototype.directPairEvidenceGate = function(a, b, reason, contextText, stage) {
  ...
  var directInReason = graphHasDirectPairEvidenceText(a, b, reason);
  ...
  if (directInContext || directInReason) {
    // v887延续：只要当前pair有强直连模板（自称/号称/本名/即/就是/被称为等），即使reason里夹带最近图谱，也优先A档放行。
    graphRemoteLog("direct_pair_gate_pass", { a: a, b: b, stage: stage, tier: "A", reason: directInContext ? "direct_pair_context" : "direct_pair_reason", directInContext: directInContext, directInReason: directInReason, bridgeRisk: bridgeRisk });
    return { allow: true, tier: "A", reason: directInContext ? "direct_pair_context" : "direct_pair_reason", directInContext: directInContext, directInReason: directInReason, bridgeRisk: bridgeRisk };
  }
  if (strictExisting) {
  ...
  if (idType && (contextText.indexOf(a) !== -1 || reason.indexOf(a) !== -1) && (contextText.indexOf(b) !== -1 || reason.indexOf(b) !== -1)) {
    graphRemoteLog("identity_substitution_evidence", { a: a, b: b, type: idType, stage: stage, triggerText: graphSafeString(reason || contextText, 220), action: "evidence_only_not_edge" });
    // 身份替代证据进入B档：不直接强正扩散，由冲突校验或后续上下文裁决。
    return { allow: false, needVerify: true, tier: "B", reason: "identity_substitution_to_verify", identityType: idType, directInContext: directInContext, directInReason: directInReason, bridgeRisk: bridgeRisk };
  }
  if (bridgeRisk) {
    return { allow: false, needVerify: false, tier: "C", reason: "只存在桥接/历史图谱/复用表/背景推断，缺少当前pair直连强证", directInContext: directInContext, directInReason: directInReason, bridgeRisk: bridgeRisk };
  }
  return { allow: false, needVerify: true, tier: "B", reason: "无当前pair直连强证，转冲突校验复核", directInContext: directInContext, directInReason: directInReason, bridgeRisk: bridgeRisk };
};
...
function graphIsNegativeClosureReason(reason) {
  // v887延续：不同人三角闭合退休。反向闭合不再以任何本地/模型反证作为新产出种子。
  return false;
}
...
CharacterManager.prototype.loadMergedRecords = function() {
  this.mergedRecords = graphReadJsonSafe(this.mergedRecordsFile || "mergedRecords.default.json", {});
  if (!this.mergedRecords || typeof this.mergedRecords !== "object") this.mergedRecords = {};
  graphRemoteLog("merged_records_backup_loaded", { file: this.mergedRecordsFile || "mergedRecords.default.json", count: Object.keys(this.mergedRecords || {}).length, bookKey: this.aliasGraphBookKey || "default" });
};

CharacterManager.prototype.saveMergedRecords = function() {
  graphWriteJsonSafe(this.mergedRecordsFile || "mergedRecords.default.json", this.mergedRecords || {});
};

CharacterManager.prototype.storeMergedRecordBackup = function(target, source, reason) {
  if (!source || !source.name) return;
  if (!this.mergedRecords) this.mergedRecords = {};
  var key = graphNormalizeName(source.name);
  if (!key) return;
  var backupRecord = {};
  ...
  this.mergedRecords[key] = {
    name: key,
    mergedInto: graphNormalizeName(target && target.name || ""),
    mergeChapter: graphCurrentChapterId(),
  ...
  try {
    ...
    if (keys.length > 200) {
      keys.sort(function(a, b) { return String((this.mergedRecords[a] || {}).mergeTime || "").localeCompare(String((this.mergedRecords[b] || {}).mergeTime || "")); }.bind(this));
      while (keys.length > 200) delete this.mergedRecords[keys.shift()];
    }
  } catch(pruneErr) {}
  this.saveMergedRecords();
  graphRemoteLog("merged_records_backup_saved", { file: this.mergedRecordsFile || "mergedRecords.default.json", name: key, mergedInto: graphNormalizeName(target && target.name || ""), bookKey: this.aliasGraphBookKey || "default", reason: graphSafeString(reason || "", 180) });
};
...
CharacterManager.prototype.buildGraphConflictVerifyPrompt = function(payload) {
  return "你是小说人物正反图谱冲突校验器。请只根据输入的图谱、共现统计、当前证据和上下文判断两个人名是否同一具体人物。\n" +
    ...
    getV887CharacterNamingAndSpeakerRules("graph_conflict") +
    "【关系描述图谱规则】\n" +
...
CharacterManager.prototype.logAliasCheckFlow = function(newName, result, graphHint, mode, recentHint) {
...
CharacterManager.prototype.splitAliasByConflict = function(a, b, reason) {
  ...
  if (!exact) {
    if (this.mergedRecords && this.mergedRecords[splitName] && this.mergedRecords[splitName].backupRecord) {
      try {
        exact = JSON.parse(JSON.stringify(this.mergedRecords[splitName].backupRecord));
        restoredFromBackup = true;
        delete this.mergedRecords[splitName];
        if (this.saveMergedRecords) this.saveMergedRecords();
      } catch(bookBackupErr) { exact = null; }
    }
    if (!exact && rec.mergedRecords && Array.isArray(rec.mergedRecords)) {
      for (var bi = rec.mergedRecords.length - 1; bi >= 0; bi--) {
        ...
        if (backup && graphNormalizeName(backup.name) === splitName) {
    ...
    if (!exact) {
      ...
      exact = {
        ...
        genderAgeHistory: rec.genderAgeHistory ? rec.genderAgeHistory.slice(-3) : []
      };
...
CharacterManager.prototype.assignVoice = function (gender, age, voiceAssignContext) {
  ...
  var normalizedAssign = graphV887NormalizeVoiceAssignGenderAge(gender, age);
...
CharacterManager.prototype.analyzeCharacter = function(fullText, characterId, allDialogues) {
  ...
  graphRemoteLog("name_analyze_narration_rule_hint", { enabled: true, rule: "模型若根据上下文判断为旁白，必须输出旁白；只有明确系统主体才输出系统。v887延续：复用表边界去重；一致性检查绝对高精度；direct-pair三档分流；绑定式角色备份；本地槽位解析分流；模型证据锚点规则；别名图谱审计建议；远程日志附全量角色快照。", hasRecentRoleHint: !!nameAnalysisRecentRoleHint });
  
  // ===================== 提前提取序号-对话原文映射，传给投票函数 =====================
  var dialogTextMap = {};
  var seqRegForMap = /【(\d{2})】[\s\S]*?(["“])([\s\S]*?)(["”])/g;
  var seqMatch;
  while ((seqMatch = seqRegForMap.exec(fullBatchContent)) !== null) {
    var mapSeq = seqMatch[1];
    var mapRawDialog = seqMatch[3];
    dialogTextMap[mapSeq] = mapRawDialog;
  }
...
getV887CharacterNamingAndSpeakerRules("name_analyze") +
...
"【旁白归属窄规则】\n" +
"如果你根据上下文语义判断某序号是旁白，则name必须输出“旁白”，gender输出“特殊”，age输出“旁白”；不要输出“系统”。只有文本明确存在系统主体（如系统提示、系统说道、系统音、系统面板等）时，才可输出“系统”。\n" +
"如果某序号是旁白，禁止把旁白段落里的物品名、法宝名、地点名、事件名、动作对象名作为name输出；旁白只能输出name=旁白。\n\n" +
"**【输出要求】**\n" +
"1. 分析文本中所有带【01】【02】【03】...序号标记的对话，每个序号对应一个结果，序号和对话一一对应，不能错位；\n" +
"2. 返回严格JSON，key为对话序号，value包含name、gender、age；必须包含文本中所有序号，不能遗漏；\n" +
"3. 如果无法确定说话人姓名，就用前后对这个人的描述作为名字；如果连描述也没有，再按性别年龄使用群众类临时名；\n" +
"4. 你还必须返回 __relations 数组，用于记录当前批次文本中能被原文锚定的人物/角色关系原子证据；没有证据时返回空数组；\n" +
"5. __relations 与姓名分析共用同一套命名规则：命中已登记主名/别名才规范成主名；未登记新称呼按正文当前称呼输出；禁止创造状态名；\n" +
"6. evidenceText必须是【当前待分析对话内容】中的原文片段，必须能在原文中找到；禁止只写‘上下文明确/可知/证明/显然/无歧义’等总结句；\n" +
...
"\n" +"【已迁移到模型的封闭结构证据】\n" +
"以下结构原先由本地封闭式正则识别，v887 起全部由你在 __relations 中按原文证据输出，再交由证据审计/别名校验裁决；没有明确原文就不要输出。\n" +
...
"输出格式示例：\n" +
"{\n" +
"  \"01\": {\"name\": \"分析出的说话人姓名\", \"gender\": \"男/女/特殊\", \"age\": \"男青年/女青年/旁白等\"},\n" +
"  \"__relations\": [\n" +
...
"\n";
  ...
  function buildNameAnalyzeRequest(apiConfig) {
    ...
    if (ENABLE_MODEL_RAW_REMOTE_LOG) {
      graphRemoteLog("name_llm_raw_request", {
        scene: "name_analyze",
        targetSeq: padZero(targetIndex + 1, 2),
  ...
  function parseNameAnalyzeResponse(response) {
    ...
    for (var i = 0; i < allDialogues.length; i++) {
      ...
      var item = parsedResult[seq];
      if (!item || !item.name || !item.gender || !item.age) {
        throw new Error("AI返回缺失序号" + seq + "或字段不完整");
      }
    }
  ...
  while ((match = seqReg.exec(fullBatchContent)) !== null) {
    ...
    if (ENABLE_NARRATOR_SYSTEM_PRESERVE_FIX && itemResult && graphNormalizeName(itemResult.name) === "旁白") {
      ...
      graphRemoteLog("speaker_system_result_trace", { seq: seq, dialog: graphSafeString(rawDialog, 120), gender: itemResult.gender || "", age: itemResult.age || "", reason: "模型原始输出系统，v81不做语义覆盖，仅记录观察" });
    }
  ...
  if (ENABLE_NAME_ANALYSIS_PARSED_RESULT_LOG) {
    ...
    graphRemoteLog("name_analysis_parsed_result", { targetSeq: padZero(targetIndex + 1, 2), characterId: graphSafeString(characterId || "", 80), returnedNames: returnedNamesForLog.slice(0, 80), specialSpeakers: specialSpeakersForLog.slice(0, 30), knownRoleReuseHits: knownRoleReuseHitsForLog.slice(0, 60), unknownNewNames: unknownNewNamesForLog.slice(0, 60), relationCount: relForParsedLog ? relForParsedLog.length : 0, relationSamples: relationSamplesForLog });
    if (unknownNewNamesForLog.length) graphRemoteLog("alias_check_queue_created", { names: unknownNewNamesForLog.slice(0, 60), total: unknownNewNamesForLog.length, targetSeq: padZero(targetIndex + 1, 2), chapterIndex: graphCurrentChapterId() });
    if (unknownNewNamesForLog.length) graphRemoteLog("alias_check_queue_completed", { names: unknownNewNamesForLog.slice(0, 60), total: unknownNewNamesForLog.length, note: "批量姓名分析阶段仅创建观察队列，逐项结果由processCharacter的校验项日志确认" });
  }
  ...
  try { if (currentResult && currentResult.name) graphRemoteLog("speaker_cache_hit", { seq: currentSeq, name: graphNormalizeName(currentResult.name), sourceStage: "batch_name_analysis_result", currentChapter: graphCurrentChapterId(), characterId: graphSafeString(characterId || "", 80) }); } catch(cacheHitErr) {}
...
CharacterManager.prototype.processCharacter = function (fullText, characterId, allDialogues, chapterFullContent) {
  ...
  var newCharacterName = graphNormalizeStateAliasName(rawNewCharacterName); // 从API解析的新角色名；v86禁止状态说明名新增音色
          ...
          if (aliasCheckResult && aliasCheckResult.isAlias && aliasCheckResult.mainName) {
              ...
              if (targetMainRecord) {
                  ...
                  if (refineResult && refineResult.isSamePerson && refineResult.mainName) {
                      ...
                      if (normalizedRefine) {
                          // 如果AI认为应该切换主名，则以清洗后的finalMainName为准；本地没有该主名时新建主记录，避免继续合并到错误宿主/傀儡记录。
                          if (normalizedRefine.mainName !== targetMainRecord.name) {
                              ...
                              if (switchedMainRecord) {
                                  ...
                                  var redirectVoice = this.assignVoice ? this.assignVoice(analysis.gender, analysis.age, { targetName: normalizedRefine.mainName, assignType: "别名清洗主名重定向新角色", sourceStage: "alias_refine_main_redirect", afterAliasCheck: true, isSpecialSpeaker: false }) : "";
                                  switchedMainRecord = {
                                      name: normalizedRefine.mainName,
                                      aliases: normalizedRefine.mainName,
                                      gender: analysis.gender,
                                      age: analysis.age,
                                      voice: redirectVoice || targetMainRecord.voice || "default",
                                      usageCount: CONFIG.resetUsageCount,
                                      chapters: graphCurrentChapterId() === "unknown" ? [] : [graphCurrentChapterId()],
                                      genderAgeHistory: [{ gender: analysis.gender, age: analysis.age }]
                                  };
                                  this.characterRecords.unshift(switchedMainRecord);
                                  targetMainRecord = switchedMainRecord;
                                  createdNewMainRecordForRedirect = true;
                              }
  ...
  if (!targetMainRecord) {
      ...
      var voice = this.assignVoice(analysis.gender, analysis.age, { targetName: newCharacterName, assignType: "新角色发音人分配", sourceStage: aliasCheckCompletedForNewName === true ? "new_role_create_after_alias" : "new_role_create_without_alias", afterAliasCheck: aliasCheckCompletedForNewName === true, aliasCheckResult: aliasCheckResultForNewName || "", isSpecialSpeaker: false });
      if (!voice) {
      ...
      // v887：已有角色每次复用都先记录本轮模型返回的性别/年龄，避免usageCount=100早退导致历史永远不累计。
      var ageHistoryUpdated = this.addGenderAgeHistoryForExistingRole ? this.addGenderAgeHistoryForExistingRole(targetMainRecord, analysis, "process_existing_role_reuse") : false;
      var fixedVoiceLockedForExisting = graphV887IsFixedVoiceRecord(targetMainRecord);
      if (fixedVoiceLockedForExisting) {
          graphV887MarkFixedVoiceRecord(targetMainRecord, targetMainRecord.fixedVoiceReason || "existing_role_fixed_voice_detected");
      }
      ...
      if (isVoiceInvalid) {
          ...
          var newVoice = this.assignVoice(analysis.gender, analysis.age, { targetName: targetMainRecord.name || newCharacterName, assignType: "已有角色发音人修复", sourceStage: "existing_role_voice_repair", afterAliasCheck: false, isSpecialSpeaker: false });
          if (newVoice) {
      ...
      if (!targetMainRecord.voice || targetMainRecord.voice === "") {
          targetMainRecord.voice = this.assignVoice(analysis.gender, analysis.age, { targetName: targetMainRecord.name || newCharacterName, assignType: "已有角色发音人补齐", sourceStage: "existing_role_voice_fill", afterAliasCheck: false, isSpecialSpeaker: false });
          if (!voice) {
...
var SpeechRuleJS = {
  name: "多角色朗读2.85发音人轮询+增强别名检验v887",
  id: "mingwuyan_v887",
  author: "命無言、萌新、M",
  version: 887,
  zdfp: 1,
                          ...
                          try {
                              ...
                              try {
                                  ...
                                  if (jsNewShumingContent && jsNewShumingContent.length > 0) {
                                      ...
                                      graphRemoteLog("character_book_cache_switch", { source: "load_new_book_to_gengxin", bookName: newBookName, fileName: newShumingFileName, recordCount: (function(){ try { return JSON.parse(newShumingContent.toString()).length || 0; } catch(e){ return 0; } })(), chapterIndex: graphCurrentChapterId() });
                                  } else {
                                      throw new Error("文件空");
                                  }
                              } catch (e) {
                                  ...
                                  graphRemoteLog("character_book_cache_switch", { source: "new_book_empty_gengxin", bookName: newBookName, fileName: newShumingFileName, recordCount: 0, chapterIndex: graphCurrentChapterId() });
                              }
                              // 3. 更新缓存书名（原有逻辑完全保留）
                              try {
...
// v887延续：mergedRecords 是 merge rollback history，不再是按name复用表。
CharacterManager.prototype.loadMergedRecords = function() {
...
// v887延续：快照组包时从真实持久角色表重新读取，避免上传空records。
...
// v887延续：身份替代日志只用于真正身份替代/魂魄/伪装类；普通别名走 alias_evidence_observed。
...
// v887延续：本地 speaker/action 槽位语义判定代码已删除；相关证据由批量姓名分析 __relations 返回。
...
// v887：保存/读取角色表前后补日志，并在保存前补recordId，给绑定式备份和快照提供稳定键。
...
// v887：voteModelRelations 统一在前置定义中完成归并与字段保留；此处不再覆盖，避免丢失 evidenceFamily/evidenceSubtype。
...
function setFixedVoice(characterName) {
  ...
  if (record) {
      ...
      graphV887MarkFixedVoiceRecord(record, "manual_setFixedVoice");
      characterManager.saveRecords();
      return "✅ 已硬锁" + charName + "发音人：" + record.voice.toString() + "。后续年龄累计不会更新年龄段，也不会重新分配发音人。";
  } else {
