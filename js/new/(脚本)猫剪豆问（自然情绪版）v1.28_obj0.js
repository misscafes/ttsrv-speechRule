// ===================== 朗读脚本 v1.28（自然情绪版）：配合 CNB 混元3 接口，融合猫箱-VV大军"完全版+4"跳段修复策略；「」内容改由旁白发音（去除「」括号作为旁白文本）、不分配括号3独立发音人（章节标题仍走括号3）；修复切回旧书角色丢失（角色备份改合并写入，防止主记录被污染时覆盖丢失；saveCurrentBookName 加 default_book 保护） =====================
...
function preprocessChapterTitle(text) {
    ...
    for (var i = 0; i < chapterPatterns.length; i++) {
        if (chapterPatterns[i].test(trimmed)) {
            ...
            var convertedText = convertArabicNumberToChinese(text);
...
function getBookNameSafely() {
    // v1.28 修复：优先使用 App 注入的 book.name（实时性最高，避免 data.json 滞后导致换书不切换）
    try {
...
function handleBookSwitch() {
    ...
    // v1.28 增强日志：输出 data.json 原始 bookName，便于诊断书名来源
    try {
