// 猫剪豆问引擎 v1.3.2（自然情绪版）：同步脚本长段落降频保护
try {
...
var MAX_CHARS_PER_SYNTHESIS = 500; // 单次合成最大字符数，超长文本自动切分
...
// 将超长文本按标点切分为多个子段，避免单次合成过长导致音频截断或服务端返回null
        ...
        if ((current + part).length <= maxChars) {
            ...
            if (part.length > maxChars) {
                ...
                for (var j = 0; j < subParts.length; j++) {
                    var sub = subParts[j];
                    if (!sub) continue;
                    if ((current + sub).length <= maxChars) {
                        current += sub;
                    } else {
                        if (current) result.push(current);
                        if (sub.length > maxChars) {
                            for (var k = 0; k < sub.length; k += maxChars) {
                                result.push(sub.substring(k, Math.min(k + maxChars, sub.length)));
                            }
                            current = '';
                        } else {
                            current = sub;
...
for (var si = 0; si < segments.length; si++) {
    ...
    if (seg.type === 'sfx' || !seg.txt || seg.txt.length <= MAX_CHARS_PER_SYNTHESIS) {
        ...
        java.log('[引擎] 长文本切分: ' + seg.txt.substring(0, 30) + '... 长度' + seg.txt.length + ' → ' + subTexts.length + '段');
...
for (var i = 0; i < segments.length; i++) {
    ...
    // v1.3：脚本侧动态生成的自然语言表演指令优先放入 context_texts，并做长度保护
    ...
    // v1.3：OOM 保护，若 extraObj 过大则精简 context_texts
    ...
    var retryCount = 0;
    var lastError = '';
    var minAudioBytes = 200; // 最小有效音频字节数，低于此值视为合成失败
    for (var attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        if (attempt < maxRetries) {
            ...
            if (isRetryable) {
                retryCount++;
                var sleepMs = 500 + attempt * 400;
                java.log('[合成] 第' + (attempt + 1) + '次失败(可重试): ' + lastError.substring(0, 80) + '，' + sleepMs + 'ms后重试 | 文本: ' + seg.txt.substring(0, 25));
