...
function saveBookCharacters(charArr) {
    try {
        ...
        if (bookName) {
            java.writeExternalFile(EXT_DIR + "shuming." + bookName + ".json", jsonData);
        }
    } catch (e) {}
}
...
function updateCharacterRecords(dialogList, chapterContext) {
    ...
    var jsonRecords = JSON.stringify(records, null, 2);
    try { java.writeExternalFile(RECORDS_FILE, jsonRecords); } catch (e) {}
    var bookName = getBookNameSafely();
    if (bookName) {
        try { java.writeExternalFile(EXT_DIR + "shuming." + bookName + ".json", jsonRecords); } catch (e) {}
    }
}
