...
var SEQ_ADD_RATIO = 0.7; // 总引号数＞5时生效：总左引号数量 × 该比例 = 最终添加序号的数量，取整数
...
var CNB_CONFIG = {
    ...
    model: "zhipu-guest(智谱游客免登录)"       // 注：实际请求已改为智谱游客模式，此处仅为日志显示的模型标签
};
...
var _cnbCsrfToken = "";
// 上次成功获取凭证的时间戳（用于 8 小时限频，避免过期 token 被一直复用）
var _cnbLastAuthTime = 0;
// 凭证最长有效期：超过该时长才在请求路径上重新获取（默认 8 小时）
var CNB_AUTH_MAX_AGE_MS = 8 * 60 * 60 * 1000;
try {
...
// ===================== 智谱清言（GLM）游客模式客户端（免登录，内置实现） =====================
// 逆向自 app-proxy 的 ZhipuClient：每次请求自动生成全新 deviceId + guest token，满足「每次不同设备信息」
// 直接写在朗读规则里，不再依赖混元太极 App 或任何 API Key。
var ZHIPU_BASE = "https://chatglm.cn/chatglm";
var ZHIPU_SIGN_SECRET = "8a1317a7468aa3ad86e997d08f3f31cb";
var ZHIPU_TEXT_ASSISTANT_ID = "65940acff94777010aa6b796";
var ZHIPU_UA = "Mozilla/5.0 (Linux; Android 15; V2352A Build/AP3A.240905.015.A2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.6478.71 Mobile Safari/537.36";
var ZHIPU_X_EXP_GROUPS = "na_android_config:exp:NA,na_4o_config:exp:4o_A,tts_config:exp:tts_config_a,na_glm4plus_config:exp:open,mainchat_server_app:exp:A,mobile_history_daycheck:exp:a,desktop_toolbar:exp:A,chat_drawing_server:exp:A,drawing_server_cogview:exp:cogview4,app_welcome_v2:exp:B,chat_drawing_streamv2:exp:A,mainchat_rm_fc:exp:add,mainchat_dr:exp:open,chat_auto_entrance:exp:A,drawing_server_hi_dream:control:A,homepage_square:exp:close,assistant_recommend_prompt:exp:3,app_home_regular_user:exp:A,mainchat_moe:exp:300,assistant_greet_user:exp:greet_user,app_welcome_personalize:exp:A,assistant_model_exp_group:exp:glm4.5,ai_wallet:exp:ai_wallet_enable";
var ZHIPU_GUEST_URL = ZHIPU_BASE + "/user-api/guest/access";
var ZHIPU_STREAM_URL = ZHIPU_BASE + "/backend-api/assistant/stream";

// MD5（直接用 Java 实现，避免手写 JS MD5）
function zhipuMd5(s) {
...
function zhipuRandomHex(nBytes) {
  var rnd = new java.security.SecureRandom();
  var out = "";
  for (var i = 0; i < nBytes; i++) {
    var b = rnd.nextInt(256) & 0xff;
    out += (b < 16 ? "0" : "") + b.toString(16);
  }
  return out;
}
function zhipuUuid16() {
  var uuid = String(java.util.UUID.randomUUID());
  return uuid.replace(/-/g, "").substring(0, 16);
}
function zhipuRandomDeviceId() {
  var uuid = String(java.util.UUID.randomUUID());
  return uuid.replace(/-/g, "");
}
// 签名三件套（与 ZhipuClient.buildSign 完全一致）
function zhipuBuildSign() {
  var now = String(new Date().getTime());
  while (now.length < 13) now += "0";
  var digits = [];
  for (var i = 0; i < now.length; i++) digits.push(parseInt(now.charAt(i), 10));
  var sum = 0;
  for (var i = 0; i < digits.length; i++) sum += digits[i];
  var secondLast = digits[digits.length - 2];
  var checksum = (sum - secondLast) % 10;
  if (checksum < 0) checksum += 10;
  var timestamp = now.substring(0, now.length - 2) + String(checksum) + now.charAt(now.length - 1);
  var nonce = zhipuRandomHex(16);
  var sign = zhipuMd5(timestamp + "-" + nonce + "-" + ZHIPU_SIGN_SECRET);
  return { timestamp: timestamp, nonce: nonce, sign: sign };
}
// 拉取 guest token（每次都会生成全新 deviceId）
function zhipuFetchGuestToken() {
  var sign = zhipuBuildSign();
  var deviceId = zhipuRandomDeviceId();
  var headers = {
    ...
    "X-Lang": "zh",
    "X-Request-Id": zhipuUuid16(),
    "X-Nonce": sign.nonce,
    "X-Sign": sign.sign,
    "X-Timestamp": sign.timestamp
  };
  var resp = ttsrv.httpPost(ZHIPU_GUEST_URL, "", headers);
  var code = (typeof resp.code === "function") ? resp.code() : (resp.code || 0);
  var text = resp.body ? String(resp.body().string() || "") : "";
  if (code !== 200 && code !== 201) throw new Error("智谱获取guest token HTTP " + code);
  var o = JSON.parse(text);
  var result = o.result || {};
  var access = result.access_token || "";
  if (!access) throw new Error("智谱获取guest token失败：" + text.substring(0, 200));
  return { access: access, deviceId: deviceId };
}
// 聊天请求头（含 token 与本次 deviceId）
function zhipuStreamHeaders(tokenObj, sign) {
  return {
    "Host": "chatglm.cn",
    ...
    "X-App-fr": "default",
    "X-Lang": "zh",
    "X-Device-Id": tokenObj.deviceId,
    "X-Device-Model": "",
    "X-Device-Brand": "",
    "X-Request-Id": zhipuUuid16(),
    "X-Exp-Groups": ZHIPU_X_EXP_GROUPS,
    "X-Timestamp": sign.timestamp,
    "X-Nonce": sign.nonce,
    "X-Sign": sign.sign,
...
// 智谱原生请求体
function zhipuBuildChatBody(text) {
  return {
    assistant_id: ZHIPU_TEXT_ASSISTANT_ID,
    conversation_id: "",
    meta_data: {
      mention_assistant_id: "", mention_assistant_name: "", mention_assistant_avatar: "",
      mention_conversation_id: "", is_test: false, input_question_type: "xxxx",
      channel: "", agent_id: "", is_greeting: false, chat_mode: "", is_networking: false,
      platform: "h5", tm: "h5", if_plus_model: false,
...
// 解析智谱 SSE（parts -> content -> type:text，按累积增量拼接，与 ProxyServer.parseZhipuSse 一致）
function zhipuParseStream(rawBody) {
    ...
    if (line.length === 0) {
      ...
      try {
        ...
        for (var p = 0; p < parts.length; p++) {
          var content = parts[p] && parts[p].content;
          if (!content) continue;
        ...
        if (full.length > prevFull.length) {
          result += full.substring(prevFull.length);
          prevFull = full;
        }
      } catch (e) {}
    } else if (line.indexOf("data:") === 0) {
      dataBuf += line.substring(5).replace(/^\s+/, "") + "\n";
    }
  }
  return result;
}
...
function getCnbCsrfToken() {
    // csrftoken 与 csrfkey 都在，且距上次获取未超过有效期，则直接复用（避免频繁请求 explore）
    if (_cnbCsrfToken && CNB_CONFIG.csrfkey) {
        if (Date.now() - _cnbLastAuthTime < CNB_AUTH_MAX_AGE_MS) return _cnbCsrfToken;
        console.log("【CNB】凭证已超 " + (CNB_AUTH_MAX_AGE_MS / 3600000) + " 小时，重新获取");
    }
    try {
...
// 截断超长文本，避免日志刷屏（保留前后各 800 字符）
function cnbLogTruncate(s, len) {
    len = len || 800;
    s = String(s == null ? "" : s);
    if (s.length <= len * 2) return s;
    return s.substring(0, len) + "\n...[已截断，共 " + s.length + " 字符]...\n" + s.substring(s.length - len);
}
// 判断 CNB 响应体是否像「鉴权失效」而非正常结果：
//   - HTML 登录/错误页（以 < 开头）或非 JSON → 视为失效
//   - 含 error/error_code/error_message/detail 字段的 JSON → 视为失效
//   - 含 choices 字段（正常完成，即便内容格式不对） → 不视为鉴权失效
function cnbResponseLooksAuthError(bodyStr) {
    if (!bodyStr) return true;
    var s = String(bodyStr).trim();
    if (s.indexOf("<") === 0) return true; // HTML 错误/登录页
    try {
        var o = JSON.parse(s);
        if (o.choices) return false; // 有 choices → 正常完成
        if (o.error || o.error_code || o.error_message || (o.detail && String(o.detail).length)) return true;
        return false; // 其它无 choices 的 JSON（少见），不轻易判定为鉴权失效
    } catch (e) {
        return true; // 非 JSON → 多半是错误页
    }
}

// 从 SSE 流式响应体（text/event-stream）拼接出完整文本：
//   逐行取 data: 事件，解析 choices[0].delta.content 并累加，忽略 [DONE] 与无 content 的事件。
function cnbConcatStreamContent(rawBody) {
    var lines = String(rawBody || "").split("\n");
    var parts = [];
    for (var i = 0; i < lines.length; i++) {
        var line = lines[i].trim();
        if (!line || line.indexOf("data:") !== 0) continue;
        var data = line.substring(5).trim(); // 去掉 "data:" 前缀
        if (data === "[DONE]" || data === "") continue;
        try {
            var o = JSON.parse(data);
            var delta = o.choices && o.choices[0] && o.choices[0].delta;
            if (delta && delta.content) parts.push(delta.content);
        } catch (e) { /* 跳过无法解析的行（如注释/心跳） */ }
    }
    return parts.join("");
}

// 判断响应体是否为 SSE 流式格式（CNB 在 stream:true 时返回）
function cnbIsStream(rawBody) {
    var s = String(rawBody || "").trim();
    return s.indexOf("data:") !== -1 || s.indexOf("\"delta\"") !== -1;
}
...
var CHARACTER_ANALYZE_RETRY_MAX = 5;
var next100Chars = "";
var shuohua = "";
var text2 = "";
var text3 = "";
...
CharacterManager.prototype.analyzeCharacter = function(fullText, characterId, allDialogues) {
  ...
  var prompt = 
"你是一个小说内容角色分析师，你要按照要求回复内容，不要回复无关的，任务是精准判断小说手稿中所有带【01】【02】序号标记的对话的说话人，每个序号对应一段对话。\n\n" +
...
"**【输出要求】**\n" +
"1. 不要返回分析过程，必须只返回json，要不然解析不了！分析文本中所有带【01】【02】【03】...序号标记的对话，每个序号对应一个结果，序号和对话一一对应，不能错位；\n" +
...
"\n\n";
  ...
  function buildNameAnalyzeRequest(apiConfig) {
    // 改用智谱游客模式（免登录）：每次请求生成全新 deviceId + guest token
    var tokenObj = zhipuFetchGuestToken();
    var sign = zhipuBuildSign();
    var userText = (typeof prompt !== "undefined" ? prompt : "") +
      "\n【上文历史内容】\n" + this.contextHistory2 +
      "\n【当前待分析对话内容】\n" + fullBatchContent;
    return {
      endpoint: ZHIPU_STREAM_URL,
      data: zhipuBuildChatBody(userText),
      headers: zhipuStreamHeaders(tokenObj, sign)
    };
  }
  function parseNameAnalyzeResponse(response) {
    // 先读取一次响应体（body 只能消费一次），后续各分支复用
    ...
    if (typeof response.code === "function") {
      ...
      if (_code === 401 || _code === 403) {
        ...
        console.log("【CNB-日志】鉴权失败 HTTP " + _code + "，响应体：\n" + cnbLogTruncate(responseBody));
        throw new Error("CNB鉴权失败(" + _code + ")，将刷新凭证重试");
      }
    }
    // 智谱游客模式：直接解析智谱原生 SSE（parts->content->type:text），构造 OpenAI 兼容结构
    var text = zhipuParseStream(responseBody);
    if (!text || text.trim() === "") {
      console.log("【智谱-日志】响应解析为空，响应体：\n" + cnbLogTruncate(responseBody));
      throw new Error("智谱响应解析为空");
    }
    var apiResponse = { choices: [{ message: { content: text } }] };
    if (!apiResponse.choices || !apiResponse.choices[0] || !apiResponse.choices[0].message || !apiResponse.choices[0].message.content) {
      console.log("【CNB-日志】API返回格式错误（无content字段），响应体：\n" + cnbLogTruncate(responseBody));
    ...
    if (jsonStart === -1 || jsonEnd === -1) {
      console.log("【CNB-日志】返回内容无有效JSON（缺失{}），content：\n" + cnbLogTruncate(content));
...
CharacterManager.prototype.processCharacter = function (fullText, characterId, allDialogues, chapterFullContent) {
  ...
  if (bieming !== 0 && newCharacterName !== "未知" && Object.keys(pendingAliasNames).length > 0 && pendingAliasContext) {
    ...
    var batchPrompt = "你是专业的小说人物别名识别AI。不要返回分析过程，必须只返回json，要不然解析不了！请基于小说上下文和角色关系，逐一判断【待分析名字列表】中每个名字是否为【本地已存角色列表】中某个角色的别名。\n" +
    promptPart +
    ...
    function buildBatchRequest(apiConfig) {
      ...
      var tokenObj = zhipuFetchGuestToken();
      var sign = zhipuBuildSign();
      var userText = "严格遵守格式要求，仅输出JSON" + "\n" + batchPrompt;
      return { endpoint: ZHIPU_STREAM_URL, data: zhipuBuildChatBody(userText), headers: zhipuStreamHeaders(tokenObj, sign) };
    }
    ...
    function parseBatchResponse(response) {
      ...
      if (!apiResponse.choices || !apiResponse.choices[0] || !apiResponse.choices[0].message) {
        console.log("【CNB-日志】响应格式错误（无choices/message），响应体：\n" + cnbLogTruncate(responseBody));
      ...
      var jsonEnd = content.lastIndexOf("}");
      if (jsonStart === -1 || jsonEnd === -1) {
        console.log("【CNB-日志】未找到JSON，content：\n" + cnbLogTruncate(content));
