// 猫剪豆问引擎 v1.28（自然情绪版）：融合猫箱-VV大军"完全版+4"跳段修复策略，重试时递增追加句号并增加强化标点补救阶段；保留v1.21的跨段对话、长文本切分、协议异常等待、最终抢救重试、纯标点段过滤等能力
try {
...
var RETRY_APPEND_PUNCTUATION = 1;  // 可重试错误时，是否在文本末尾补句号，0关闭
var ENABLE_FINAL_RETRY = 1;        // 7次常规重试全部失败后，是否进行长等待抢救重试，0关闭
var FINAL_RETRY_DELAY_MS = 15000;  // 长等待抢救重试的等待毫秒数
var SKIP_PURE_PUNCTUATION = 1;     // 是否跳过无引号包裹的纯标点段，0关闭
var PROTOCOL_EXCEPTION_FIRST_DELAY_MS = 2000; // ProtocolException/SocketException首次重试前的最小等待毫秒数
...
// 将阿拉伯数字转为中文数字（避免 TTS 把“第1章”误读为“第十一章”）
function convertArabicNumberToChinese(text) {
    var cn = ["零", "一", "二", "三", "四", "五", "六", "七", "八", "九"];
    return text.replace(/[0-9]+/g, function(numStr) {
        ...
        if (n < 100) {
            ...
            var one = n % 10;
            return cn[ten] + "十" + (one === 0 ? "" : cn[one]);
        }
        if (n < 1000) {
            var hun = Math.floor(n / 100);
            var rem = n % 100;
            if (rem === 0) return cn[hun] + "百";
            var remStr = convertArabicNumberToChinese(String(rem));
            return cn[hun] + "百" + (rem < 10 ? "零" : "") + remStr;
        }
        if (n < 10000) {
            var thou = Math.floor(n / 1000);
            var rem = n % 1000;
            if (rem === 0) return cn[thou] + "千";
            var remStr = convertArabicNumberToChinese(String(rem));
            return cn[thou] + "千" + (rem < 100 ? "零" : "") + remStr;
        }
        return numStr;
    });
}
...
function preprocessChapterTitle(t) {
    ...
    for (var i = 0; i < chapterPatterns.length; i++) {
        if (chapterPatterns[i].test(trimmed)) {
            ...
            var convertedText = convertArabicNumberToChinese(t);
            return '“<<括号3>>' + convertedText + '”';
        }
    }
    return t;
}
...
for (var i = 0; i < segments.length; i++) {
    ...
    // v1.21：跳过无引号包裹的纯标点段（避免分段残留单独句号被合成），但保留“”内的纯标点（如乌鸦叫等拟声）
    if (SKIP_PURE_PUNCTUATION === 1 && !/^“.*”$/.test(seg.txt) && /^[。，、；：！？…～.,;:!?.]+$/.test(pure)) {
        log('[合成] 跳过无引号包裹的纯标点段: ' + seg.txt);
        continue;
    }
    ...
    for (var attempt = 0; attempt <= maxRetries; attempt++) {
        ...
        // 重试时尝试在文本末尾补句号，修复因缺标点被服务端截断导致的跳段
        // v1.28 优化：融合猫箱-VV大军策略，首次重试清理末尾不规范标点后补一个句号，后续递增追加句号，提高服务端接受概率
        var sendText = seg.txt;
        if (attempt > 0 && RETRY_APPEND_PUNCTUATION === 1) {
            sendText = (sendText || "").replace(/[。，、；：！？…~.!?]*$/, "");
            for (var _apIdx = 0; _apIdx < attempt; _apIdx++) {
                sendText += "。";
            }
        }

        try {
            audio = ws.maoxiang(wsUrl, sendText, cfg.voice, AUDIO_FORMAT, SAMPLE_RATE, segRate, PITCH_VALUE, APP_KEY, TIMEOUT_MS, extra);
            if (!audio) {
                lastError = '返回null';
            ...
            lastError = String(ex);
            // v1.21：完整记录协议层异常，便于排查服务端返回的具体状态码
            if (lastError.indexOf('ProtocolException') !== -1 || lastError.indexOf('SocketException') !== -1) {
                hasProtocolError = true;
                log('[合成] 协议/连接异常(完整): ' + lastError);
            }
        }
        ...
        if (attempt < maxRetries) {
            ...
            if (isRetryable) {
                retryCount++;
                var baseDelay = hasProtocolError ? PROTOCOL_EXCEPTION_FIRST_DELAY_MS : 500;
                var sleepMs = Math.min(20000, baseDelay * Math.pow(2, attempt)) + Math.floor(Math.random() * 500);
                // v1.21：协议/连接首次失败后，额外等待让底层连接有机会重建
                if (hasProtocolError && attempt === 0) {
                    sleepMs = Math.max(sleepMs, PROTOCOL_EXCEPTION_FIRST_DELAY_MS);
                }
                log('[合成] 第' + (attempt + 1) + '次失败(可重试): ' + lastError.substring(0, 80) + '，' + sleepMs + 'ms后重试 | 文本: ' + sendText.substring(0, 25));
                try { java.lang.Thread.sleep(sleepMs); } catch(e) {}
            } else {
                log('[合成] 请求失败(不重试): ' + lastError + ' 文本: ' + sendText.substring(0, 30));
                break;
            }
        }
    }
    if (audio && audio.length > 0) {
        ...
        log('[合成] 成功(' + audio.length + '字节) voice=' + cfg.voice + ' | ' + sendText.substring(0, 25));
        out.write(audio);
    } else if (ENABLE_FINAL_RETRY === 1) {
        // v1.28 新增：最终长等待抢救前，先用强化标点（多个句号）快速补救一次（参考猫箱-VV大军"追加句号"策略的强化版）
        if (RETRY_APPEND_PUNCTUATION === 1) {
            var _reinforcedText = (seg.txt || "").replace(/[。，、；：！？…~.!?]*$/, "") + "。。";
            log('[合成] 7次常规重试失败，尝试强化标点补救: ' + _reinforcedText.substring(0, 25));
            try {
                var _reinforcedAid = String(Math.floor(1e12 + 9e12 * Math.random()));
                var _reinforcedDeviceId = String(Math.floor(1e12 + 9e12 * Math.random()));
                var _reinforcedWsUrl = BASE_URL + '?' + query + '&ssmix=&aid=' + _reinforcedAid + '&device_id=' + _reinforcedDeviceId;
                audio = ws.maoxiang(_reinforcedWsUrl, _reinforcedText, cfg.voice, AUDIO_FORMAT, SAMPLE_RATE, segRate, PITCH_VALUE, APP_KEY, TIMEOUT_MS, extra);
                if (audio && audio.length > 0) {
                    synthesisResult.result = 'success';
                    synthesisResult.bytes = audio.length;
                    synthesisResult.retries = retryCount + 1;
                    log('[合成] 强化标点补救成功(' + audio.length + '字节) voice=' + cfg.voice + ' | ' + _reinforcedText.substring(0, 25));
                    out.write(audio);
                }
            } catch (_reinforcedEx) {
                log('[合成] 强化标点补救失败: ' + String(_reinforcedEx).substring(0, 60));
                audio = null;
            }
        }
        // v1.21：7次常规重试全部失败后，长等待再抢救一次（针对服务端5xx/101握手失败后连接状态异常）
        if (!audio || audio.length === 0) {
            log('[合成] 7次常规重试失败，' + FINAL_RETRY_DELAY_MS + 'ms后进行最终抢救重试 | 文本: ' + seg.txt.substring(0, 25));
            try { java.lang.Thread.sleep(FINAL_RETRY_DELAY_MS); } catch(e) {}
            try {
                var finalAid = String(Math.floor(1e12 + 9e12 * Math.random()));
                var finalDeviceId = String(Math.floor(1e12 + 9e12 * Math.random()));
                var finalWsUrl = BASE_URL + '?' + query + '&ssmix=&aid=' + finalAid + '&device_id=' + finalDeviceId;
                audio = ws.maoxiang(finalWsUrl, seg.txt, cfg.voice, AUDIO_FORMAT, SAMPLE_RATE, segRate, PITCH_VALUE, APP_KEY, TIMEOUT_MS, extra);
                if (audio && audio.length > 0) {
                    synthesisResult.result = 'success';
                    synthesisResult.bytes = audio.length;
                    synthesisResult.retries = retryCount + 1;
                    log('[合成] 最终抢救重试成功(' + audio.length + '字节) voice=' + cfg.voice + ' | ' + seg.txt.substring(0, 25));
                    out.write(audio);
                } else {
                    throw new Error('最终抢救重试仍返回空音频');
                }
            } catch (finalEx) {
                synthesisResult.result = 'failed';
                synthesisResult.retries = retryCount + 1;
                synthesisResult.error = lastError || '未知错误';
                log('[合成] 跳过本段(最终抢救重试仍失败): ' + (lastError || String(finalEx)).substring(0, 60) + ' | ' + seg.txt.substring(0, 25));
            }
        }
    } else {
        synthesisResult.result = 'failed';
        synthesisResult.retries = retryCount;
        synthesisResult.error = lastError || '未知错误';
        log('[合成] 跳过本段(' + (retryCount > 0 ? '重试' + retryCount + '次后仍失败' : '无可重试错误') + '): ' + lastError.substring(0, 60) + ' | ' + sendText.substring(0, 25));
    }
