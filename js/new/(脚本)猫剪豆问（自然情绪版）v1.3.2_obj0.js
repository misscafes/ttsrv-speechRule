...
var ENABLE_EMOTION_BRIDGE = 1;
        ...
        // v1.3.2：情绪桥接长段落降频保护
        var EMOTION_BRIDGE_MAX_DIALOGS_PER_PARAGRAPH = 20;
        var EMOTION_BRIDGE_LONG_PARAGRAPH_THRESHOLD = 1500;
...
// ===================== 旁白情绪推断 =====================
        ...
        // v1.3.2：长段落情绪桥接降频，避免返回文本过大导致阅读 App OOM
        var effectiveDialogs = dialogs;
        if (inputLen > EMOTION_BRIDGE_LONG_PARAGRAPH_THRESHOLD && dialogs.length > EMOTION_BRIDGE_MAX_DIALOGS_PER_PARAGRAPH) {
            java.log("[情绪桥接] 段落过长(" + inputLen + "字，" + dialogs.length + "句)，只对前" + EMOTION_BRIDGE_MAX_DIALOGS_PER_PARAGRAPH + "句加情绪标记");
            effectiveDialogs = [];
            for (var _ei = 0; _ei < EMOTION_BRIDGE_MAX_DIALOGS_PER_PARAGRAPH && _ei < dialogs.length; _ei++) {
                effectiveDialogs.push(dialogs[_ei]);
            }
        }
        ...
        // 处理对话（从后往前，v1.3.2：长段落只处理 effectiveDialogs）
        for (var i = effectiveDialogs.length - 1; i >= 0; i--) {
            var d = effectiveDialogs[i];
            var emotion = "";
            var source = "";
            ...
            if (ENABLE_EMOTION_BRIDGE === 1 && !hasExistingEmotion(d.content)) {
                ...
                var inputLen = String(inputText || "").length;
                if (inputLen <= PERFORMANCE_MAX_PARAGRAPH_CHARS && (isStrong || _perfPrefixCount < _perfMaxPrefixCount)) {
