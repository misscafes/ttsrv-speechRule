...
// ===================== v2.127新增：批量别名校验pending状态 =====================
// 由 analyzeCharacter 在批量名字分析后收集未匹配本地角色的名字
// 由 processCharacter 在首次处理对话时一次性批量校验并合并别名
...
"\n\n";
  ...
  // ===================== v2.127新增：批量别名分析pending收集 =====================
  // 在批量名字分析完成后，收集所有未匹配本地角色的名字，供processCharacter批量校验
  if (bieming !== 0 && batchResult) {
    var unknownNames = [];
    var uniqueNamesSet = {};
    for (var seqKey in batchResult) {
      if (batchResult.hasOwnProperty(seqKey)) {
        if (seqKey === "__relations" || seqKey === "relations" || seqKey === "_relations") continue;
        var uName = batchResult[seqKey].name;
        if (uName && uName !== "未知" && !uniqueNamesSet[uName]) {
          uniqueNamesSet[uName] = true;
          if (!this.findCharacterRecord(uName)) {
            unknownNames.push(uName);
          }
        }
      }
    }
    ...
    if (unmatchedCount > 0) {
      ...
      pendingAliasNames = {};
      for (var ui = 0; ui < unknownNames.length; ui++) {
        pendingAliasNames[unknownNames[ui]] = true;
      }
      pendingAliasContext = {
        contextHistory2: this.contextHistory2,
        text2: fullBatchContent,
...
CharacterManager.prototype.processCharacter = function (fullText, characterId, allDialogues, chapterFullContent) {
  ...
  // ===================== v2.127新增：批量别名校验（参考2.87加速版）=====================
  // 当 analyzeCharacter 收集到未匹配本地角色的 pending 名字时，
  // 在 processCharacter 首次处理时一次性批量校验并合并别名，减少API调用次数
  if (bieming !== 0 && newCharacterName !== "未知" && Object.keys(pendingAliasNames).length > 0 && pendingAliasContext) {
    ...
    var contextText = (ctx.contextHistory2 || '') + '\n' + (ctx.text2 || '') + '\n' + (ctx.next100Chars || '');
    var nameSeqText = '';
    ...
    var batchAliasPrompt = "你是专业的小说人物别名识别AI。请基于小说上下文和角色关系，逐一判断【待分析名字列表】中每个名字是否为【本地已存角色列表】中某个角色的别名。\n" +
      promptPart +
    ...
    var batchAliasTimeout = ALIAS_ANALYZE_TIMEOUT;

    function buildBatchAliasRequest(apiConfig) {
      var requestData = {
        model: apiConfig.model,
        messages: [
          { role: "system", content: "严格遵守格式要求，仅输出JSON" },
          { role: "user", content: batchAliasPrompt }
        ],
        temperature: 0.1,
        thinking_mode: false,
        thinking: { type: "disabled" },
      ...
      var headers = {
        ...
        "Authorization": "Bearer " + apiConfig.key,
        "Connection": "keep-alive",
        "Timeout": batchAliasTimeout.toString()
      };
      return { endpoint: apiConfig.endpoint, data: requestData, headers: headers };
    }

    function parseBatchAliasResponse(response) {
      var responseBody = String(response.body().string() || "{}");
      var apiResponse = JSON.parse(responseBody);
    ...
    var aliasConcurrentResult = concurrentApiRequest(
      "aliasAnalyze",
      buildBatchAliasRequest,
      parseBatchAliasResponse,
      null,
      batchAliasTimeout
    );

    if (aliasConcurrentResult && aliasConcurrentResult.success) {
      var finalMap = null;
      if (aliasConcurrentResult.isMultiResult) {
        var nameMap = {};
        for (var i = 0; i < namesToCheck.length; i++) {
          nameMap[padZero(i + 1, 2)] = namesToCheck[i];
        }
        finalMap = voteBatchAliasAnalyzeResult(aliasConcurrentResult.data, nameMap);
      } else if (aliasConcurrentResult.data) {
        finalMap = aliasConcurrentResult.data;
      }
      ...
      if (finalMap) {
        for (var i = 0; i < namesToCheck.length; i++) {
          ...
          if (res && res.isAlias && res.mainName) {
            ...
            if (mainRecord) {
              var aliases = mainRecord.aliases ? mainRecord.aliases.split("|") : [];
              for (var ai = 0; ai < aliases.length; ai++) aliases[ai] = aliases[ai].trim();
              if (aliases.indexOf(name) === -1) {
                aliases.push(name);
                mainRecord.aliases = aliases.join("|");
                if (this.recordPositiveAliasEdge) this.recordPositiveAliasEdge(name, mainRecord.name, 3, "batch_alias_confirmed", res.reason || "批量别名校验确认");
                if (typeof graphRemoteLog === "function") graphRemoteLog("batch_alias_merge_confirmed", { newName: name, mainName: mainRecord.name, seq: seq });
              }
            }
          }
        }
...
CharacterManager.prototype.reEvaluateCharacter = function(record) {
  ...
  if (needReassign) {
      var newVoice = this.assignVoice(mostCommonGender, mostCommonAge, record.personality || "");
...
// 普通关键词：先处理重复，再处理单个（保留你原有注释）
      ...
      // -------------------------- 书籍切换与角色备份（v2.100：data.json直连+智能备份+按书名隔离上下文+别名合并） --------------------------
      try {
          if (text.includes("“")) {
              var dataJsonContent = "";
              try {
