...
var DualKeyManager = (function() {
  ...
  // 无#!#则无备用模型，向后完全兼容
  var backupConfig = null; // 由loadKeyFile()从miyue.txt的#!#段动态加载
  // 备用模型状态：false=静默（默认），true=本次已激活（用完立即复位）
  ...
  // 私有：加载并解析整个密钥文件，按##拆分双场景，按#!#拆分备用模型
  function loadKeyFile() {
      try {
          var fileContent = ttsrv.readTxtFile(keyFileName).trim();

          // ===================== 第一步：按#!#拆分备用模型段（独立分隔符，不与##或其他占位符冲突）======================
          var backupContent = "";
          var mainContent = fileContent;
          if (fileContent.indexOf("#!#") !== -1) {
              var backupSplit = fileContent.split("#!#");
              mainContent = backupSplit[0] ? backupSplit[0].trim() : "";
              backupContent = backupSplit[1] ? backupSplit[1].trim() : "";
          }

          // ===================== 第二步：按##拆分双场景（原逻辑，完全兼容）======================
          var hasSplit = mainContent.indexOf("##") !== -1;
          var nameContent, aliasContent;
          if (hasSplit) {
              // 有##：前面=姓名分析密钥，后面=别名分析密钥
              var splitArr = mainContent.split("##");
              nameContent = splitArr[0] ? splitArr[0].trim() : "";
              aliasContent = splitArr[1] ? splitArr[1].trim() : "";
          } else {
              // 无##：两个场景共用同一套密钥
              nameContent = mainContent;
              aliasContent = mainContent;
          }
          // 分别解析两个场景的密钥池
          pools.nameAnalyze.list = parseSingleGroup(nameContent);
          pools.aliasAnalyze.list = parseSingleGroup(aliasContent);

          // ===================== 第三步：解析备用模型段（取第一个有效条目）======================
          backupConfig = null; // 每次重新加载，先复位
          if (backupContent) {
              var backupPool = parseSingleGroup(backupContent);
              if (backupPool.length > 0) {
                  backupConfig = backupPool[0]; // 备用模型只取第一条
  ...
  return {
      ...
      getBackupConfig: function() { return isBackupConfigured() ? backupConfig : null; }, // 获取备用模型配置
      isBackupActive: function() { return backupActive; }, // 备用模型是否已激活
      activateBackup: function() { backupActive = true; }, // 激活备用模型
      deactivateBackup: function() { backupActive = false; backupPendingLog = null; }, // 复位备用模型（静默），同时清除待输出日志
      getBackupPendingLog: function() { return backupPendingLog; } // 获取待输出的备用模型加载日志
  };
})();
...
if (!maxConcurrent || maxConcurrent <= 0) {
  ...
  if (successCount.get() > 0) {
    ...
    // ===================== 备用模型故障切换 =====================
    // 主力全部失败，尝试启用备用模型
    var backupCfg = DualKeyManager.getBackupConfig();
    if (backupCfg) {
      console.log("【🟡备用模型】⚠️主力全部失败，启用接力！场景：" + scene + "，模型：" + backupCfg.model + "，密钥末尾4位：" + backupCfg.key.slice(-4));
      // 输出之前暂存的备用模型加载日志（仅主力失败后才显示）
      if (DualKeyManager.getBackupPendingLog) { var _plog = DualKeyManager.getBackupPendingLog(); if (_plog) console.log(_plog); }
      DualKeyManager.activateBackup();
      ...
      if (backupSuccessNum.get() > 0) {
        ...
        DualKeyManager.deactivateBackup();
...
var GRAPH_RULE_SOURCE = "tts-rule-2.85-v75";
...
function graphSamePersonEvidenceBlockReason(a, b, evidence) {
  ...
  if (!evidence) return "";
  var hasExplicitAlias = /(\u53c8\u53eb|\u4e5f\u53eb|\u540d\u53eb|\u5168\u540d|\u5c0f\u540d|\u5916\u53f7|\u5316\u540d|\u7ef0\u53f7|\u88ab\u79f0\u4e3a|\u88ab\u79f0\u4f5c|\u672c\u540d|\u539f\u540d|\u771f\u540d|\u5373\u4e3a|\u4e3a\u540c\u4e00\u4eba|\u540c\u4e00\u4eba|\u540c\u4e00\u540d|\u540c\u4e00\u4f4d)/.test(evidence);
...
CharacterManager.prototype.applyNegativeTriadClosure = function(a, b, reason) {
  ...
  for (var i = 0; i < neighbors.length; i++) {
    ...
    if (pairAdded) {
      ...
      graphRemoteLog("triad_closure", { kind: "反证", names: names, added: pairAdded, reason: reason });
    }
  }
  return added;
};
...
var SpeechRuleJS = {
      ...
      for (var name in GENSHIN_CHARACTERS) {
         ...
         //         default: moren // 独立默认值
              ...
              if (tagsData[voiceTag]) {
                  ...
                  if (ENABLE_EMOTION) tagsData[voiceTag].emotion = EMOTION_FIELD_CONFIG;
              }
              // 2. 若标签不存在，新建配置（仅含性格选择框+情绪选择框）
              else {
                  var _newTagCfg = { personality: personalityConfig };
                  if (ENABLE_EMOTION) _newTagCfg.emotion = EMOTION_FIELD_CONFIG;
  ...
  getTagName: function(tag, tagData) {
      ...
      if (genshinTagKey !== "") {
          ...
          // 情绪部分
          var genshinEmotion = "";
          if (ENABLE_EMOTION && tagData && tagData.emotion) {
      ...
      // 2. duihua标签处理（括号完全匹配，复用GENSHIN逻辑）
          ...
          if (emotionVal) console.log("【情绪】" + emotionVal);
          return rsTag;
      }
