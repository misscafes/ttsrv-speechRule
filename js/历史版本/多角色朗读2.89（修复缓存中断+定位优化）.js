...
function logEmotionFromResults(list, enableEmotion) {
    ...
    for (var k = 0; k < list.length; k++) {
        ...
        if (item.tag.indexOf("|") !== -1) {
            var _pipeIdx = item.tag.lastIndexOf("|");
            var _emotionPart = item.tag.substring(_pipeIdx + 1);
            if (_emotionPart && _emotionPart !== "无") {
                console.log("【情绪】" + _emotionPart);
            }
        }
    }
}
...
var GRAPH_RULE_SOURCE = "tts-rule-89-v89";
