...
function extractEmotionValue(tagData, enableEmotion) {
    var emotionValue = "";
    if (!enableEmotion || !tagData || !tagData.emotion) {
        return emotionValue;
    }

    if (Object.prototype.toString.call(tagData.emotion) === '[object Array]') {
        var flatArr = EmotionForceFlattenArray(tagData.emotion);
        ...
        emotionValue = (tagData.emotion + "").trim();
    }

    // 过滤"无"和空值
    if (emotionValue === "" || emotionValue === "无") {
        emotionValue = "";
    }
    return emotionValue;
}
...
function logEmotionFromResults(list, enableEmotion) {
    ...
    for (var k = 0; k < list.length; k++) {
        ...
        // 简化角色名显示
        if (!roleName || roleName === "narration") roleName = "旁白";
        else if (roleName === "duihua") roleName = "对话";
        else if (roleName === "duihuaA") roleName = "男对话";
        else if (roleName === "duihuaB") roleName = "女对话";
        else if (roleName.indexOf("localSound") === 0) roleName = "本地音效";
        else if (roleName.indexOf("括号") === 0) roleName = "括号发音";
...
var GRAPH_RULE_SOURCE = "tts-rule-90-v90";
...
"\n\n";
  ...
  if (!batchResult) {
    ...
    if (typeof BackupModelManager !== 'undefined' && BackupModelManager.getBackupConfig()) {
        console.log("【备用模型】主力全部失败，备用模型已配置");
    }
    return this.analyzeCharacterFallback(fullText, characterId);
  }
  // ========== 原有后续逻辑完全保留，零改动 ==========
...
var SpeechRuleJS = {
  name: "多角色朗读2.90（修复缓存中断+定位优化+情绪日志）",
  id: "mingwuyan",
  author: "命無言、萌新改",
  version: 64,
  zdfp: 1,
