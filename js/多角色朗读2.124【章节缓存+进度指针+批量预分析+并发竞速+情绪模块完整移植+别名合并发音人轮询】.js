...
function attachEmotionBridgeToText(text, rawEmotion) {
  ...
  try {
    ...
    // 避免重复叠加
    pureText = pureText.replace(/\[\[emo:[a-z\-]+\]\]/g, "");
...
var SpeechRuleJS = {
  name: "多角色朗读2.121【情绪模块植入+称号可临时主名+旧主名自动入别名+修复别名丢失+同步shuming+别名合并发音人轮询+增强别名校验版v77+备用模型接力】",
  id: "mingwuyan",
  author: "命無言、萌新改",
  version: 120,
  zdfp: 1,
...
// 普通关键词：先处理重复，再处理单个（保留你原有注释）
      ...
      // v2.124：读取进度指针和章节缓存
      var v2124_progress = { bookName: "", chapterTitle: "", lastSeq: 0 };
      var v2124_chapterCache = { title: "", results: {} };
      if (ENABLE_PROGRESS_POINTER === 1 || ENABLE_CHAPTER_CACHE === 1) {
        try {
          var currentBook4 = v2124_getBookNameFromDataJson();
          if (currentBook4) {
   ...
   //   //console.log("\n" + AUDIO_CONFIG.logPrefix + "===== 解析yinxiao.json中的%标记（开头%提内容，结尾%提网址） =====");
      ...
      // ===================== 情绪桥接处理 =====================
              ...
              try {
                  ...
                  if (isSoundLikeItem) {
                      // 音效只清除情绪前缀，不添加
                      item.text = String(item.text || "").replace(/\[\[emo:[a-z\-]+\]\]/gi, "");
                  } else {
...
if (typeof debugTagData === "object" && debugTagData !== null) {
    ...
    try {
      ...
      // v2.124：超短对话去引号
      if (ENABLE_SHORT_QUOTE_REMOVE === 1) {
        for (var li = 0; li < list.length; li++) {
          if (list[li] && list[li].text) {
            list[li].text = v2124_applyShortQuoteRemove(list[li].text);
          }
        }
      }

      // v2.124：更新进度指针
      if (ENABLE_PROGRESS_POINTER === 1) {
        try {
          var currentBook3 = v2124_getBookNameFromDataJson();
          if (currentBook3) {
            var lastSeq = 0;
            for (var li2 = 0; li2 < list.length; li2++) {
              if (list[li2] && list[li2].tag && list[li2].tag !== "narration" && list[li2].tag.indexOf("localSound") !== 0 && list[li2].tag.indexOf("括号") !== 0) {
                lastSeq++;
              }
            }
